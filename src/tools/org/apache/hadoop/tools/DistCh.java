...
/**
 * A Map-reduce program to recursively change files properties such as owner,
...
public class DistCh extends DistTool {
	...
	static class FileOperation implements Writable {
		...
		/**
		 * path:owner:group:permission e.g. /user/foo:foo:bar:700
		 */
		FileOperation(String line) {
			try {
				...
				owner = t[1];
				group = t[2];
				permission = t[3] == null ? null : new FsPermission(Short
						.parseShort(t[3], 8));
		...
		static final FsPermission FILE_UMASK = FsPermission
	...
	static class ChangeInputFormat implements InputFormat<Text, FileOperation> {
		...
		public InputSplit[] getSplits(JobConf job, int numSplits)
			...
			try {
				for (in = new SequenceFile.Reader(fs, srcs, job); in.next(key,
						value);) {
					long curr = in.getPosition();
					long delta = curr - prev;
					if (++count > targetcount) {
						count = 0;
						splits.add(new FileSplit(srcs, prev, delta,
								(String[]) null));
						prev = curr;
					}
				}
			...
			if (remaining != 0) {
				splits
						.add(new FileSplit(srcs, prev, remaining,
								(String[]) null));
			}
			LOG.info("numSplits=" + numSplits + ", splits.size()="
					+ splits.size());
			return splits.toArray(new FileSplit[splits.size()]);
		}

		/** {@inheritDoc} */
		public RecordReader<Text, FileOperation> getRecordReader(
	...
	static class ChangeFilesMapper implements
			Mapper<Text, FileOperation, WritableComparable<?>, Text> {
		...
		public void configure(JobConf job) {
			this.jobconf = job;
			ignoreFailures = job.getBoolean(
					Option.IGNORE_FAILURES.propertyname, false);
		}
	...
	private static List<FileOperation> fetchList(Configuration conf,
	...
	public int run(String[] args) throws Exception {
		...
		try {
			...
			jobconf.setBoolean(Option.IGNORE_FAILURES.propertyname,
					isIgnoreFailures);
			check(jobconf, ops);
			...
			try {
				...
				try {
					if (logpath == null) {
						...
						final Path logdir = FileOutputFormat
	...
	private boolean setup(List<FileOperation> ops, Path log) throws IOException {
		...
		try {
			opWriter = SequenceFile.createWriter(fs, jobconf, opList,
					Text.class, FileOperation.class,
				...
				for (pathstack.push(op.src); !pathstack.empty();) {
					for (FileStatus stat : fs.listStatus(pathstack.pop())) {
						...
						if (op.isDifferent(stat)) {
							...
							Path f = stat.getPath();
							opWriter.append(new Text(f.toString()),
									new FileOperation(f, op));
						}
					}
				}
			}
