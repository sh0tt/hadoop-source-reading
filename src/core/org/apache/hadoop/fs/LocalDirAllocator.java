...
public class LocalDirAllocator {
	...
	private AllocatorPerContext obtainContext(String contextCfgItemName) {
		synchronized (contexts) {
			...
			if (l == null) {
				contexts.put(contextCfgItemName, (l = new AllocatorPerContext(
						contextCfgItemName)));
			}
			return l;
		}
	...
	private static class AllocatorPerContext {
					...
					try {
						...
						if (localFS.mkdirs(tmpDir) || localFS.exists(tmpDir)) {
							try {
								...
								dfList
										.add(new DF(new File(localDirs[i]),
												30000));
		...
		private Path createPath(String path) throws IOException {
			...
			try {
				DiskChecker.checkDir(new File(file.getParent().toUri()
						.getPath()));
				return file;
		 ...
		 */
		public synchronized Path getLocalPathForWrite(String pathStr,
				long size, Configuration conf) throws IOException {
			confChanged(conf);
			int numDirs = localDirs.length;
			...
			if (size == -1) { // do roulette selection: pick dir with
				...
				while (numDirsSearched < numDirs && returnPath == null) {
					long randomPosition = Math.abs(r.nextLong())
							% totalAvailable;
					int dir = 0;
		...
		public synchronized Path getLocalPathToRead(String pathStr,
			...
			// no path found
			throw new DiskErrorException("Could not find " + pathStr
