...
class KFSImpl implements IFSImpl {
	...
	public FileStatus[] readdirplus(Path path) throws IOException {
		...
		for (int i = 0; i < fattr.length; i++) {
			if ((fattr[i].filename.compareTo(".") == 0)
					|| (fattr[i].filename.compareTo("..") == 0))
				continue;
			numEntries++;
		}
		...
		for (int i = 0; i < fattr.length; i++) {
			...
			if (fattr[i].isDirectory)
				fstatus[j] = new FileStatus(0, true, 1, 0,
						fattr[i].modificationTime, fn);
			else
				fstatus[j] = new FileStatus(fattr[i].filesize,
						fattr[i].isDirectory, fattr[i].replication,
	...
	public FSDataOutputStream create(String path, short replication,
		...
		return new FSDataOutputStream(new KFSOutputStream(kfsAccess, path,
