...
/**
 * <p>
 * A block-based {@link FileSystem} backed by <a
...
public class S3FileSystem extends FileSystem {
	...
	public FileStatus[] listStatus(Path f) throws IOException {
		...
		if (inode.isFile()) {
			return new FileStatus[] { new S3FileStatus(f.makeQualified(this),
					inode) };
		}
	...
	public FSDataOutputStream create(Path file, FsPermission permission,
		...
		return new FSDataOutputStream(new S3OutputStream(getConf(), store,
				makeAbsolute(file), blockSize, progress, bufferSize),
				statistics);
	}
	...
	private boolean renameRecursive(Path src, Path dst) throws IOException {
		...
		if (srcINode.isDirectory()) {
			for (Path oldSrc : store.listDeepSubPaths(src)) {
				...
				Path newDst = new Path(oldSrcPath
						.replaceFirst(srcPath, dstPath));
