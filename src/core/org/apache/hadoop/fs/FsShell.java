...
public class FsShell extends Configured implements Tool {
	...
	public static final SimpleDateFormat dateForm = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm");
	protected static final SimpleDateFormat modifFmt = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");
   ...
   */
	...
	void copyToLocal(String[] argv, int pos) throws IOException {
		...
		if (dststr.equals("-")) {
			if (copyCrc) {
				System.err
						.println("-crc option is not valid when destination is stdout.");
			}
			...
			if (copyCrc && !(srcFS instanceof ChecksumFileSystem)) {
				System.err
						.println("-crc option is not valid when source file system "
	...
	private void copyToLocal(final FileSystem srcFS, final Path src,
			final File dst, final boolean copyCrc) throws IOException {
		/*
		...
		if (!srcFS.getFileStatus(src).isDir()) {
			...
			if (copyCrc) {
				if (!(srcFS instanceof ChecksumFileSystem)) {
					throw new IOException(
							"Source file system does not have crc files");
				}
	...
	void copyMergeToLocal(String srcf, Path dst, boolean endline)
		...
		for (int i = 0; i < srcs.length; i++) {
			if (endline) {
				FileUtil.copyMerge(srcFs, srcs[i], FileSystem
						.getLocal(getConf()), dst, false, getConf(), "\n");
			} else {
				FileUtil.copyMerge(srcFs, srcs[i], FileSystem
						.getLocal(getConf()), dst, false, getConf(), null);
			}
		}
	}
	...
	void cat(String src, boolean verifyChecksum) throws IOException {
		...
		}.globAndProcess(srcPattern, getSrcFileSystem(srcPattern,
				verifyChecksum));
	}
	...
	private void setReplication(String[] cmd, int pos) throws IOException {
		...
		try {
			...
			System.err
					.println("Illegal replication, a positive integer expected");
			throw nfe;
		} catch (IllegalArgumentException iae) {
	...
	void setReplication(short newRep, String srcf, boolean recursive,
			List<Path> waitingList) throws IOException {
		Path srcPath = new Path(srcf);
		FileSystem srcFs = srcPath.getFileSystem(getConf());
		Path[] srcs = FileUtil.stat2Paths(srcFs.globStatus(srcPath), srcPath);
		for (int i = 0; i < srcs.length; i++) {
	...
	private void setReplication(short newRep, FileSystem srcFs, Path src,
			boolean recursive, List<Path> waitingList) throws IOException {
		if (!srcFs.getFileStatus(src).isDir()) {
		...
		if (items == null) {
			...
			for (int i = 0; i < items.length; i++) {
				if (!items[i].isDir()) {
					setFileReplication(items[i].getPath(), srcFs, newRep,
							waitingList);
				} else if (recursive) {
					setReplication(newRep, srcFs, items[i].getPath(),
							recursive, waitingList);
				}
			}
		}
	}
	 ...
	 * @throws IOException
	 */
	private void setFileReplication(Path file, FileSystem srcFs, short newRep,
			List<Path> waitList) throws IOException {
	...
	void du(String src) throws IOException {
		...
		if ((items == null)
			...
			for (int i = 0; i < items.length; i++) {
				length[i] = items[i].isDir() ? srcFs.getContentSummary(
						items[i].getPath()).getLength() : items[i].getLen();
	...
	void dus(String src) throws IOException {
		...
		for (int i = 0; i < status.length; i++) {
			long totalSize = srcFs.getContentSummary(status[i].getPath())
					.getLength();
			String pathStr = status[i].getPath().toString();
			System.out.println(("".equals(pathStr) ? "." : pathStr) + "\t"
					+ totalSize);
		}
	}
	...
	void mkdir(String src) throws IOException {
		...
		try {
			...
			if (fstatus.isDir()) {
				throw new IOException("cannot create directory " + src
						+ ": File exists");
			} else {
			...
			for (int i = 0; i < fmt.length; ++i) {
				if (fmt[i] != '%') {
					...
					case 'r':
						...
						break;
					case 'y':
						buf.append(modifFmt.format(new Date(f
								.getModificationTime())));
						break;
					case 'Y':
	...
	void rename(String srcf, String dstf) throws IOException {
		...
		if (srcURI.compareTo(dstURI) != 0) {
			throw new IOException(
					"src and destination filesystems do not match.");
		}
		Path[] srcs = FileUtil.stat2Paths(srcFs.globStatus(srcPath), srcPath);
		...
		for (int i = 0; i < srcs.length; i++) {
			if (!srcFs.rename(srcs[i], dst)) {
				...
				throw new IOException("Failed to rename " + srcs[i] + " to "
						+ dst);
			}
		}
	}
	...
	private void delete(Path src, FileSystem srcFs, boolean recursive,
		...
		try {
			...
			throw new FileNotFoundException("cannot remove " + src
					+ ": No such file or directory.");
		}
	...
	static abstract class CmdHandler {
		...
		public abstract void run(FileStatus file, FileSystem fs)
				throws IOException;
	}
	...
	private static int runCmdHandler(CmdHandler handler, FileStatus stat,
			FileSystem srcFs, boolean recursive) throws IOException {
		int errors = 0;
	...
	int runCmdHandler(CmdHandler handler, String[] args, int startIndex,
		...
		for (int i = startIndex; i < args.length; i++) {
			...
			FileSystem srcFs = srcPath.getFileSystem(getConf());
			Path[] paths = FileUtil.stat2Paths(srcFs.globStatus(srcPath),
					srcPath);
			for (Path path : paths) {
				try {
					...
					String msg = (e.getMessage() != null ? e
							.getLocalizedMessage()
	...
	private static void printUsage(String cmd) {
		...
		if ("-fs".equals(cmd)) {
			...
			System.err.println("Usage: java FsShell"
					+ " [-D <[property=value>]");
		} else if ("-ls".equals(cmd) || "-lsr".equals(cmd) || "-du".equals(cmd)
				|| "-dus".equals(cmd) || "-touchz".equals(cmd)
			...
			System.err
					.println("Usage: java FsShell" + " [-test -[ezd] <path>]");
		} else if ("-stat".equals(cmd)) {
			System.err.println("Usage: java FsShell"
					+ " [-stat [format] <path>]");
		} else if ("-tail".equals(cmd)) {
			...
			System.err
					.println("           [-copyFromLocal <localsrc> ... <dst>]");
			System.err
					.println("           [-moveFromLocal <localsrc> ... <dst>]");
			System.err.println("           [" + GET_SHORT_USAGE + "]");
			System.err
					.println("           [-getmerge <src> <localdst> [addnl]]");
			System.err.println("           [-cat <src>]");
			System.err.println("           [-text <src>]");
			System.err.println("           [" + COPYTOLOCAL_SHORT_USAGE + "]");
			System.err
					.println("           [-moveToLocal [-crc] <src> <localdst>]");
	...
	public int run(String argv[]) throws Exception {
		...
		try {
			if ("-put".equals(cmd) || "-copyFromLocal".equals(cmd)) {
				...
				if (argv.length > i + 2)
					copyMergeToLocal(argv[i++], new Path(argv[i++]), Boolean
							.parseBoolean(argv[i++]));
				else
			...
			} else if ("-chmod".equals(cmd) || "-chown".equals(cmd)
					|| "-chgrp".equals(cmd)) {
