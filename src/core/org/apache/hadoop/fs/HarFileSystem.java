...
public class HarFileSystem extends FilterFileSystem {
	...
	public void initialize(URI name, Configuration conf) throws IOException {
		...
		try {
			...
			throw new IOException("Unable to "
					+ "read the version of the Har file system: "
					+ this.archivePath);
		}
		if (this.version != HarFileSystem.VERSION) {
			throw new IOException("Invalid version " + this.version
					+ " expected " + HarFileSystem.VERSION);
		}
	}
	...
	private URI decodeHarURI(URI rawURI, Configuration conf) throws IOException {
		...
		String auth = (underLyingHost == null && underLyingPort == -1) ? null
				: (underLyingHost + ":" + underLyingPort);
		URI tmp = null;
		...
		try {
			tmp = new URI(underLyingScheme, auth, rawURI.getPath(), rawURI
	...
	public BlockLocation[] getFileBlockLocations(FileStatus file, long start,
		...
		FileStatus fsFile = fs.getFileStatus(new Path(archivePath, harStatus
				.getPartName()));
		BlockLocation[] rawBlocks = fs.getFileBlockLocations(fsFile, harStatus
				.getStartIndex()
				+ start, len);
	...
	private String fileStatusInIndex(Path harPath) throws IOException {
		...
		while (read < masterStat.getLen()) {
			...
			if (startHash <= hashCode && hashCode <= endHash) {
				stores.add(new Store(Long.parseLong(readStr[2]), Long
						.parseLong(readStr[3]), startHash, endHash));
			}
			line.clear();
		}
		try {
	...
	public FileStatus getFileStatus(Path f) throws IOException {
		...
		if (readStr == null) {
			throw new FileNotFoundException("File: " + f
					+ " does not exist in " + uri);
		}
		HarStatus hstatus = null;
		hstatus = new HarStatus(readStr);
		return new FileStatus(hstatus.isDir() ? 0 : hstatus.getLength(),
				hstatus.isDir(), (int) archiveStatus.getReplication(),
				archiveStatus.getBlockSize(), archiveStatus
						.getModificationTime(), archiveStatus.getAccessTime(),
				new FsPermission(archiveStatus.getPermission()), archiveStatus
						.getOwner(), archiveStatus.getGroup(), makeRelative(
	...
	public FSDataInputStream open(Path f, int bufferSize) throws IOException {
		...
		return new HarFSDataInputStream(fs, new Path(archivePath, hstatus
				.getPartName()), hstatus.getStartIndex(), hstatus.getLength(),
				bufferSize);
	}
	...
	public FileStatus[] listStatus(Path f) throws IOException {
		...
		String readStr = fileStatusInIndex(harPath);
		if (readStr == null) {
			throw new FileNotFoundException("File " + f + " not found in "
					+ archivePath);
		}
		HarStatus hstatus = new HarStatus(readStr);
		if (!hstatus.isDir())
			statuses.add(new FileStatus(hstatus.getLength(), hstatus.isDir(),
					archiveStatus.getReplication(), archiveStatus
							.getBlockSize(), archiveStatus
							.getModificationTime(), archiveStatus
							.getAccessTime(), new FsPermission(archiveStatus
							.getPermission()), archiveStatus.getOwner(),
					archiveStatus.getGroup(), makeRelative(this.uri.toString(),
	...
	private static class HarFSDataInputStream extends FSDataInputStream {
		...
		private static class HarFsInputStream extends FSInputStream {
			...
			HarFsInputStream(FileSystem fs, Path path, long start, long length,
