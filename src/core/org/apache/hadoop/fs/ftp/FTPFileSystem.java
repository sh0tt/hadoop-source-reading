...
public class FTPFileSystem extends FileSystem {
	...
	private FTPClient connect() throws IOException {
		...
		if (!FTPReply.isPositiveCompletion(reply)) {
			...
			throw new IOException("Login failed on server - " + host
					+ ", port - " + port);
		}
	...
	public FSDataOutputStream create(Path file, FsPermission permission,
		...
		// FSDataOutputStream.
		client.changeWorkingDirectory(parent.toUri().getPath());
		FSDataOutputStream fos = new FSDataOutputStream(client
				.storeFileStream(file.getName()), statistics) {
			@Override
	...
	private FileStatus getFileStatus(FTPFile ftpFile, Path parentPath) {
		...
		return new FileStatus(length, isDir, blockReplication, blockSize,
				modTime, accessTime, permission, user, group, filePath
						.makeQualified(this));
	}
