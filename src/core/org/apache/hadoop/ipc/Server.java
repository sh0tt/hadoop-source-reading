...
public abstract class Server {
	...
	private class Handler extends Thread {
		public Handler(int instanceNumber) {
			this.setDaemon(true);
			this
					.setName("IPC Server handler " + instanceNumber + " on "
							+ port);
		}
   ...
   * the number of handler threads that will be used to process calls.
	...
	protected Server(String bindAddress, int port,
		...
		this.rpcMetrics = new RpcMetrics(serverName, Integer
				.toString(this.port), this);
	...
	private static int channelRead(ReadableByteChannel channel,
		...
		return (buffer.remaining() <= NIO_BUFFER_LIMIT) ? channel.read(buffer)
