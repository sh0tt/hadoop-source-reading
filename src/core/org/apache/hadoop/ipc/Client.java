...
public class Client {
			...
			try {
				...
				if (state == Status.SUCCESS.state) {
					...
					markClosed(new RemoteException(
							WritableUtils.readString(in), WritableUtils
									.readString(in)));
				}
			} catch (IOException e) {
				markClosed(e);
	...
	public Client(Class<? extends Writable> valueClass, Configuration conf,
		...
		this.maxIdleTime = conf.getInt("ipc.client.connection.maxidletime",
				10000); // 10s
	 ...
	 * 
	 * @deprecated Use
	 *             {@link #call(Writable, InetSocketAddress, Class, UserGroupInformation)}
	 *             instead
	 */
	@Deprecated
	 ...
	 * protocol, with the <code>ticket</code> credentials, returning the value.
	 ...
	 */
	private Connection getConnection(InetSocketAddress addr, Class<?> protocol,
			UserGroupInformation ticket, Call call) throws IOException {
		if (!running.get()) {
