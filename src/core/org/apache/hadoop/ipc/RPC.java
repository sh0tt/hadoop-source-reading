...
public class RPC {
	...
	private static class Invoker implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args)
			...
			ObjectWritable value = (ObjectWritable) client.call(new Invocation(
					method, args), address, method.getDeclaringClass(), ticket);
			if (logDebug) {
	 ...
	 */
	static VersionedProtocol waitForProxy(Class protocol, long clientVersion,
			InetSocketAddress addr, Configuration conf, long timeout)
			throws IOException {
	...
	public static Object[] call(Method method, Object[][] params,
		...
		try {
			Writable[] wrappedValues = client.call(invocations, addrs, method
					.getDeclaringClass(), ticket);

			if (method.getReturnType() == Void.TYPE) {
				return null;
			}

			Object[] values = (Object[]) Array.newInstance(method
					.getReturnType(), wrappedValues.length);
	...
	public static class Server extends org.apache.hadoop.ipc.Server {
		...
		public Writable call(Class<?> protocol, Writable param,
			...
			try {
				...
				Method method = protocol.getMethod(call.getMethodName(), call
						.getParameterClasses());
				...
				MetricsTimeVaryingRate m = (MetricsTimeVaryingRate) rpcMetrics.registry
				...
				if (m == null) {
					try {
						...
						m = (MetricsTimeVaryingRate) rpcMetrics.registry
								.get(call.getMethodName());
					}
				}
		...
		public void authorize(Subject user, ConnectionHeader connection)
			...
			if (authorize) {
				Class<?> protocol = null;
				try {
					protocol = getProtocolClass(connection.getProtocol(),
							getConf());
