...
public class TFile {
	...
	public static class Writer implements Closeable {
		...
		public void close() throws IOException {
			...
			try {
				...
				if (errorCount == 0) {
					...
					BlockAppender outMeta = writerBCF.prepareMetaBlock(
							TFileMeta.BLOCK_NAME, COMPRESSION_NONE);
					try {
					...
					BlockAppender outIndex = writerBCF
							.prepareMetaBlock(TFileIndex.BLOCK_NAME);
					try {
		 ...
		 *             would be close
		 */
		public void append(byte[] key, int koff, int klen, byte[] value,
				int voff, int vlen) throws IOException {
			...
			public void close() throws IOException {
				...
				try {
					...
					if (tfileMeta.isSorted()) {
						...
						if (tfileMeta.getComparator().compare(key, 0, len,
								lastKey, 0, lastLen) < 0) {
		...
		public DataOutputStream prepareAppendKey(int length) throws IOException {
			if (state != State.READY) {
				throw new IllegalStateException(
						"Incorrect state to start a new key: " + state.name());
			}
		...
		public DataOutputStream prepareAppendValue(int length)
			...
			if (length < 0) {
				...
				ret = new ValueRegister(new ChunkEncoder(blkAppender,
						valueBuffer));
			} else {
				ret = new ValueRegister(new Chunk.SingleChunkEncoder(
						blkAppender, length));
			}
		...
		public DataOutputStream prepareMetaBlock(String name,
				String compressName) throws IOException, MetaBlockAlreadyExists {
			if (state != State.READY) {
				throw new IllegalStateException(
						"Incorrect state to start a Meta Block: "
								+ state.name());
			}

			finishDataBlock(true);
			DataOutputStream outputStream = writerBCF.prepareMetaBlock(name,
					compressName);
			return outputStream;
		}
		...
		void finishDataBlock(boolean bForceFinish) throws IOException {
			...
			if (bForceFinish || blkAppender.getCompressedSize() >= sizeMinBlock) {
				...
				// for now
				TFileIndexEntry keyLast = new TFileIndexEntry(lastKeyBufferOS
						.getBuffer(), 0, lastKeyBufferOS.size(), blkRecordCount);
	...
	public static class Reader implements Closeable {
		 ...
		 * @throws IOException
		 */
		public Reader(FSDataInputStream fsdis, long fileLength,
		...
		synchronized void checkTFileDataIndex() throws IOException {
			if (tfileIndex == null) {
				BlockReader brIndex = readerBCF
						.getMetaBlock(TFileIndex.BLOCK_NAME);
				try {
					tfileIndex = new TFileIndex(readerBCF.getBlockCount(),
		...
		public Comparator<Scanner.Entry> getEntryComparator() {
			...
			return new Comparator<Scanner.Entry>() {
				...
				public int compare(Scanner.Entry o1, Scanner.Entry o2) {
					return comparator.compare(o1.getKeyBuffer(), 0, o1
							.getKeyLength(), o2.getKeyBuffer(), 0, o2
							.getKeyLength());
				}
			};
		}
		...
		Location getBlockContainsKey(RawComparable key, boolean greater)
			...
			checkTFileDataIndex();
			int blkIndex = (greater) ? tfileIndex.upperBound(key) : tfileIndex
					.lowerBound(key);
			if (blkIndex < 0)
				return end;
		...
		public Scanner createScannerByKey(byte[] beginKey, byte[] endKey)
				throws IOException {
			return createScannerByKey((beginKey == null) ? null
					: new ByteArray(beginKey, 0, beginKey.length),
					(endKey == null) ? null : new ByteArray(endKey, 0,
							endKey.length));
		}
		 ...
		 */
		@Deprecated
		public Scanner createScanner(RawComparable beginKey,
		 ...
		 * @throws IOException
		 */
		public Scanner createScannerByKey(RawComparable beginKey,
				RawComparable endKey) throws IOException {
		...
		public static class Scanner implements Closeable {
			 ...
			 * @throws IOException
			 */
			Scanner(Reader reader, Location begin, Location end)
					throws IOException {
				this.reader = reader;
				...
				valueDataInputStream = new DataInputStream(
						valueBufferInputStream);
			...
			private boolean seekTo(RawComparable key, boolean beyond)
				...
				// block.
				if (atEnd()
						|| (l.getBlockIndex() != currentLocation
								.getBlockIndex())
			...
			private void seekTo(Location l) throws IOException {
				...
				inBlockAdvance(l.getRecordIndex()
						- currentLocation.getRecordIndex());
			...
			int compareCursorKeyTo(RawComparable other) throws IOException {
				checkKey();
				return reader.compareKeys(keyBuffer, 0, klen, other.buffer(),
			...
			public class Entry implements Comparable<RawComparable> {
				...
				public long writeValue(OutputStream out) throws IOException {
					...
					try {
						...
						while ((chunkSize = valueBufferInputStream.getRemain()) > 0) {
							...
							dis
									.readFully(valTransferBuffer.get(), 0,
											chunkSize);
				...
				public int getValue(byte[] buf, int offset) throws IOException {
					...
					try {
						...
						while (nextOffset < buf.length) {
							int n = dis.read(buf, nextOffset, buf.length
									- nextOffset);
							if (n < 0) {
								break;
							}
				...
				public int compareTo(RawComparable key) {
					return reader.compareKeys(keyBuffer, 0, getKeyLength(), key
							.buffer(), key.offset(), key.size());
				}
				...
				public boolean equals(Object other) {
					...
					if (!(other instanceof Entry))
						return false;
					return ((Entry) other).compareTo(keyBuffer, 0,
							getKeyLength()) == 0;
				}

				@Override
				public int hashCode() {
					return WritableComparator.hashBytes(keyBuffer,
							getKeyLength());
				}
			}
	...
	static final class TFileMeta {
		...
		static BytesComparator makeComparator(String comparator) {
			...
			if (comparator.equals(COMPARATOR_MEMCMP)) {
				...
				String compClassName = comparator.substring(
						COMPARATOR_JCLASS.length()).trim();
				try {
					Class compClass = Class.forName(compClassName);
					// use its default ctor to create an instance
					return new BytesComparator(
							(RawComparator<Object>) compClass.newInstance());
				} catch (Exception e) {
					throw new IllegalArgumentException(
	...
	static class TFileIndex {
		 ...
		 * @throws IOException
		 */
		public TFileIndex(int entryCount, DataInput in,
			...
			if (size > 0) {
				...
				in.readFully(buffer);
				DataInputStream firstKeyInputStream = new DataInputStream(
		...
		public long getRecordNumByLocation(Reader.Location location) {
			...
			long lastRecNum = (blkIndex == 0) ? 0 : recordNumIndex
					.get(blkIndex - 1);
