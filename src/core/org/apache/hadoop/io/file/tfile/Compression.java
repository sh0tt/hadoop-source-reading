	...
	static enum Algorithm {
		LZO(TFile.COMPRESSION_LZO) {
			...
			public synchronized boolean isSupported() {
				if (!checked) {
					checked = true;
					String extClazz = (conf.get(CONF_LZO_CLASS) == null ? System
							.getProperty(CONF_LZO_CLASS)
							: null);
			...
			if (codec != null) {
				...
				if (compressor != null) {
					if (compressor.finished()) {
						...
						// it.
						LOG
								.warn("Compressor obtained from CodecPool already finished()");
					} else {
			...
			if (codec != null) {
				...
				if (decompressor != null) {
					if (decompressor.finished()) {
						...
						// it.
						LOG
								.warn("Deompressor obtained from CodecPool already finished()");
					} else {
