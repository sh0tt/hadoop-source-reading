...
final class BCFile {
	...
	static public class Writer implements Closeable {
		...
		private static final class WBlockState {
			 ...
			 * @throws IOException
			 */
			public WBlockState(Algorithm compressionAlgo,
					FSDataOutputStream fsOut, BytesWritable fsOutputBuffer,
				...
				this.fsBufferedOutput = new SimpleBufferedOutputStream(
						this.fsOut, fsOutputBuffer.get());
				this.compressor = compressAlgo.getCompressor();

				try {
					this.out = compressionAlgo.createCompressionStream(
		...
		public class BlockAppender extends DataOutputStream {
			...
			public void close() throws IOException {
				...
				try {
					++errorCount;
					wBlkState.finish();
					blockRegister.register(getRawSize(), wBlkState
							.getStartPos(), wBlkState.getCurrentPos());
					--errorCount;
				} finally {
					closed = true;
					blkInProgress = false;
		...
		public void close() throws IOException {
			...
			try {
				if (errorCount == 0) {
					...
					BlockAppender appender = prepareMetaBlock(
		...
		private BlockAppender prepareMetaBlock(String name,
				Algorithm compressAlgo) throws IOException,
			...
			WBlockState wbs = new WBlockState(compressAlgo, out,
					fsOutputBuffer, conf);
		...
		public BlockAppender prepareMetaBlock(String name,
				String compressionName) throws IOException,
				MetaBlockAlreadyExists {
		...
		public BlockAppender prepareDataBlock() throws IOException {
			...
			WBlockState wbs = new WBlockState(getDefaultCompressionAlgorithm(),
		...
		private class DataBlockRegister implements BlockRegister {
			...
			public void register(long raw, long begin, long end) {
				dataIndex.addBlockRegion(new BlockRegion(begin, end - begin,
						raw));
			}
		}
	}
	...
	static public class Reader implements Closeable {
		...
		static private final class RBlockState {
			...
			public RBlockState(Algorithm compressionAlgo,
					FSDataInputStream fsin, BlockRegion region,
					Configuration conf) throws IOException {
				this.compressAlgo = compressionAlgo;
				this.region = region;
				...
				try {
					this.in = compressAlgo.createDecompressionStream(
							new BoundedRangeFileInputStream(fsin, this.region
									.getOffset(), this.region
		...
		public BlockReader getDataBlock(int blockIndex) throws IOException {
			if (blockIndex < 0 || blockIndex >= getBlockCount()) {
				throw new IndexOutOfBoundsException(String.format(
						"blockIndex=%d, numBlocks=%d", blockIndex,
						getBlockCount()));
			}

			BlockRegion region = dataIndex.getBlockRegionList().get(blockIndex);
			return createReader(dataIndex.getDefaultCompressionAlgorithm(),
					region);
		}

		private BlockReader createReader(Algorithm compressAlgo,
				BlockRegion region) throws IOException {
		...
		public int getBlockIndexNear(long offset) {
			...
			int idx = Utils.lowerBound(list, new ScalarLong(offset),
	...
	static final class MetaIndexEntry {
		...
		public MetaIndexEntry(DataInput in) throws IOException {
			...
			compressionAlgorithm = Compression
					.getCompressionAlgorithmByName(Utils.readString(in));
	...
	static class DataIndex {
		...
		public DataIndex(DataInput in) throws IOException {
			defaultCompressionAlgorithm = Compression
					.getCompressionAlgorithmByName(Utils.readString(in));
	...
	static final class Magic {
		private final static byte[] AB_MAGIC_BCFILE = {
				// ... total of 16 bytes
				(byte) 0xd1, (byte) 0x11, (byte) 0xd3, (byte) 0x68,
				(byte) 0x91, (byte) 0xb5, (byte) 0xd7, (byte) 0xb6,
				(byte) 0x39, (byte) 0xdf, (byte) 0x41, (byte) 0x40,
				(byte) 0x92, (byte) 0xba, (byte) 0xe1, (byte) 0x50 };
