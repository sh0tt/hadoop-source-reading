...
public class BloomMapFile {
	...
	public static class Writer extends MapFile.Writer {
		...
		public Writer(Configuration conf, FileSystem fs, String dirName,
				...
				throws IOException {
			super(conf, fs, dirName, keyClass, valClass, compress, codec,
					progress);
			this.fs = fs;
		...
		public Writer(Configuration conf, FileSystem fs, String dirName,
			...
			super(conf, fs, dirName, comparator, valClass, compress, codec,
					progress);
			this.fs = fs;
			this.dir = new Path(dirName);
			initBloomFilter(conf);
		...
		private synchronized void initBloomFilter(Configuration conf) {
			...
			float errorRate = conf.getFloat("io.mapfile.bloom.error.rate",
					0.005f);
			vectorSize = (int) Math.ceil((double) (-HASH_COUNT * numKeys)
					/ Math.log(1.0 - Math.pow(errorRate, 1.0 / HASH_COUNT)));
			bloomFilter = new DynamicBloomFilter(vectorSize, HASH_COUNT, Hash
		...
		public synchronized void close() throws IOException {
			super.close();
			DataOutputStream out = fs.create(new Path(dir, BLOOM_FILE_NAME),
					true);
	...
	public static class Reader extends MapFile.Reader {
		...
		private void initBloomFilter(FileSystem fs, String dirName,
				Configuration conf) {
			try {
				DataInputStream in = fs
						.open(new Path(dirName, BLOOM_FILE_NAME));
