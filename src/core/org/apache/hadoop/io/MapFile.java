...
public class MapFile {
	...
	public static class Writer implements java.io.Closeable {
		...
		public Writer(Configuration conf, FileSystem fs, String dirName,
				...
				throws IOException {
			this(conf, fs, dirName, comparator, valClass, SequenceFile
		...
		public Writer(Configuration conf, FileSystem fs, String dirName,
			...
			this.data = SequenceFile.createWriter(fs, conf, dataFile, keyClass,
					valClass, compress, codec, progress);
			this.index = SequenceFile.createWriter(fs, conf, indexFile,
	...
	public static class Reader implements java.io.Closeable {
		 ...
		 */
		private synchronized int seekInternal(WritableComparable key,
				final boolean before) throws IOException {
	...
	public static long fix(FileSystem fs, Path dir,
		...
		if (!dataReader.getKeyClass().equals(keyClass)) {
			throw new Exception(dr + "Wrong key class in " + dir + ", expected"
					+ keyClass.getName() + ", got "
					+ dataReader.getKeyClass().getName());
		}
		if (!dataReader.getValueClass().equals(valueClass)) {
			throw new Exception(dr + "Wrong value class in " + dir
					+ ", expected" + valueClass.getName() + ", got "
		...
		if (!dryrun)
			indexWriter = SequenceFile.createWriter(fs, conf, index, keyClass,
					LongWritable.class);
		try {
			long pos = 0L;
			LongWritable position = new LongWritable();
	...
	public static void main(String[] args) throws Exception {
		...
		MapFile.Writer writer = new MapFile.Writer(conf, fs, out, reader
				.getKeyClass().asSubclass(WritableComparable.class), reader
				.getValueClass());

		WritableComparable key = ReflectionUtils.newInstance(reader
				.getKeyClass().asSubclass(WritableComparable.class), conf);
