...
public class UnixUserGroupInformation extends UserGroupInformation {
	...
	public UnixUserGroupInformation(String[] ugi) {
		if (ugi == null || ugi.length < 2) {
			throw new IllegalArgumentException(
					"Parameter does contain at least "
	...
	private void setUserGroupNames(String userName, String[] groupNames) {
		if (userName == null || userName.length() == 0 || groupNames == null
				|| groupNames.length == 0) {
			throw new IllegalArgumentException(
	...
	public static UnixUserGroupInformation readFromConf(Configuration conf,
			String attr) throws LoginException {
	 ...
	 * 
	 * If the current user's UNIX username or groups are configured in such a
	 * way to throw an Exception, for example if the user uses LDAP, then this
	 * method will use a the {@link #DEFAULT_USERNAME} and
	 * {@link #DEFAULT_GROUP} constants.
	 */
	 ...
	 */
	public static UnixUserGroupInformation login(Configuration conf,
			boolean save) throws LoginException {
	...
	public boolean equals(Object other) {
		...
		if (groupNames.length > 0
				&& !groupNames[0].equals(otherUGI.groupNames[0])) {
			return false;
		}
		// check all group names, ignoring the order
		return new TreeSet<String>(Arrays.asList(groupNames))
				.equals(new TreeSet<String>(Arrays.asList(otherUGI.groupNames)));
	}
