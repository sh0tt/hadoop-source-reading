...
public abstract class UserGroupInformation implements Writable, Principal {
	...
	private static final ThreadLocal<Subject> currentUser = new ThreadLocal<Subject>();
	...
	public static UserGroupInformation getCurrentUGI() {
		...
		Set<UserGroupInformation> ugiPrincipals = user
				.getPrincipals(UserGroupInformation.class);
		...
		if (ugiPrincipals != null && ugiPrincipals.size() == 1) {
			...
			if (ugi == null) {
				throw new RuntimeException(
						"Cannot find _current user_ UGI in the Subject!");
			}
		} else {
			throw new RuntimeException(
					"Cannot resolve current user from subject, " + "which had "
	...
	/**
	 * Set the {@link UserGroupInformation} for the current thread WARNING -
	 * This method should be used only in test cases and other exceptional
	 * cases!
	 * 
	 * @param ugi
	 *            {@link UserGroupInformation} for the current thread
	 */
