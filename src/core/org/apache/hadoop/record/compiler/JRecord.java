...
public class JRecord extends JCompType {
	...
	class JavaRecord extends JavaCompType {
		...
		JavaRecord(String name, ArrayList<JField<JType>> flist) {
			...
			for (Iterator<JField<JType>> iter = flist.iterator(); iter
					.hasNext();) {
				JField<JType> f = iter.next();
				fields.add(new JField<JavaType>(f.getName(), f.getType()
						.getJavaType()));
			}
		}

		String getTypeIDObjectString() {
			return "new org.apache.hadoop.record.meta.StructTypeID(" + fullName
					+ ".getTypeInfo())";
		}
		...
		void genSetupRtiFields(CodeBuffer cb) {
			...
			cb
					.append("if (null != " + Consts.RTI_FILTER_FIELDS
							+ ") return;\n");
			cb.append("int " + Consts.RIO_PREFIX + "i, " + Consts.RIO_PREFIX
					+ "j;\n");
			cb.append(Consts.RTI_FILTER_FIELDS + " = new int ["
					+ Consts.RIO_PREFIX
					+ "rtiFilter.getFieldTypeInfos().size()];\n");
			cb.append("for (" + Consts.RIO_PREFIX + "i=0; " + Consts.RIO_PREFIX
					+ "i<" + Consts.RTI_FILTER_FIELDS + ".length; "
					+ Consts.RIO_PREFIX + "i++) {\n");
			cb.append(Consts.RTI_FILTER_FIELDS + "[" + Consts.RIO_PREFIX
					+ "i] = 0;\n");
			cb.append("}\n");
			cb.append("java.util.Iterator<org.apache.hadoop.record.meta."
					+ "FieldTypeInfo> " + Consts.RIO_PREFIX + "itFilter = "
					+ Consts.RIO_PREFIX
					+ "rtiFilter.getFieldTypeInfos().iterator();\n");
			cb.append(Consts.RIO_PREFIX + "i=0;\n");
			cb
					.append("while (" + Consts.RIO_PREFIX
							+ "itFilter.hasNext()) {\n");
			cb.append("org.apache.hadoop.record.meta.FieldTypeInfo "
					+ Consts.RIO_PREFIX + "tInfoFilter = " + Consts.RIO_PREFIX
					+ "itFilter.next();\n");
			cb.append("java.util.Iterator<org.apache.hadoop.record.meta."
					+ "FieldTypeInfo> " + Consts.RIO_PREFIX + "it = "
					+ Consts.RTI_VAR + ".getFieldTypeInfos().iterator();\n");
			cb.append(Consts.RIO_PREFIX + "j=1;\n");
			cb.append("while (" + Consts.RIO_PREFIX + "it.hasNext()) {\n");
			cb.append("org.apache.hadoop.record.meta.FieldTypeInfo "
					+ Consts.RIO_PREFIX + "tInfo = " + Consts.RIO_PREFIX
					+ "it.next();\n");
			cb.append("if (" + Consts.RIO_PREFIX + "tInfo.equals("
					+ Consts.RIO_PREFIX + "tInfoFilter)) {\n");
			cb.append(Consts.RTI_FILTER_FIELDS + "[" + Consts.RIO_PREFIX
					+ "i] = " + Consts.RIO_PREFIX + "j;\n");
			cb.append("break;\n");
			cb.append("}\n");
		...
		void genReadMethod(CodeBuffer cb, String fname, String tag, boolean decl) {
			...
			cb.append(fname + ".deserialize(" + Consts.RECORD_INPUT + ",\""
					+ tag + "\");\n");
		}

		void genWriteMethod(CodeBuffer cb, String fname, String tag) {
			cb.append(fname + ".serialize(" + Consts.RECORD_OUTPUT + ",\""
					+ tag + "\");\n");
		}

		void genSlurpBytes(CodeBuffer cb, String b, String s, String l) {
			cb.append("{\n");
			cb.append("int r = " + fullName + ".Comparator.slurpRaw(" + b + ","
					+ s + "," + l + ");\n");
			cb.append(s + "+=r; " + l + "-=r;\n");
			cb.append("}\n");
		}
		...
		void genCode(String destDir, ArrayList<String> options)
				throws IOException {
			String pkg = module;
			...
			if (!pkgdir.exists()) {
				...
				if (!ret) {
					throw new IOException("Cannnot create directory: "
							+ pkgpath);
				}
			...
			cb
					.append("// File generated by hadoop record compiler. Do not edit.\n");
			...
			cb
					.append("public static org.apache.hadoop.record.meta.RecordTypeInfo"
							+ " getTypeInfo() {\n");
			...
			cb
					.append(Consts.RECORD_INPUT + ".endRecord(" + Consts.TAG
							+ ");\n");
			cb.append("}\n");
			...
			cb.append("}\n");
			cb
					.append("// if we're here, we need to read based on version info\n");
			cb.append(Consts.RECORD_INPUT + ".startRecord(" + Consts.TAG
					+ ");\n");
			cb.append("setupRtiFields();\n");
			cb.append("for (int " + Consts.RIO_PREFIX + "i=0; "
					+ Consts.RIO_PREFIX + "i<" + Consts.RTI_FILTER
					+ ".getFieldTypeInfos().size(); " + Consts.RIO_PREFIX
					+ "i++) {\n");
			int ct = 0;
			for (Iterator<JField<JavaType>> i = fields.iterator(); i.hasNext();) {
				JField<JavaType> jf = i.next();
			...
			if (0 != ct) {
				...
				cb.append("org.apache.hadoop.record.meta.Utils.skip("
						+ Consts.RECORD_INPUT + ", " + "typeInfos.get("
						+ Consts.RIO_PREFIX + "i).getFieldID(), typeInfos.get("
			...
			cb.append("if (!(" + Consts.RIO_PREFIX + "peer_ instanceof " + name
					+ ")) {\n");
			cb
					.append("throw new ClassCastException(\"Comparing different types of records.\");\n");
			cb.append("}\n");
			...
			// clone()
			cb
					.append("public Object clone() throws CloneNotSupportedException {\n");
			...
			for (Iterator<JField<JavaType>> i = fields.iterator(); i.hasNext();) {
				...
				cb.append(Consts.RIO_PREFIX + "result = 37*"
						+ Consts.RIO_PREFIX + "result + " + Consts.RIO_PREFIX
						+ "ret;\n");
			}
			...
			cb
					.append("static public int compareRaw(byte[] b1, int s1, int l1,\n");
			cb
					.append("                             byte[] b2, int s2, int l2) {\n");
			cb.append("try {\n");
	...
	class CppRecord extends CppCompType {
		...
		CppRecord(String name, ArrayList<JField<JType>> flist) {
			...
			for (Iterator<JField<JType>> iter = flist.iterator(); iter
					.hasNext();) {
				JField<JType> f = iter.next();
				fields.add(new JField<CppType>(f.getName(), f.getType()
						.getCppType()));
			}
		}

		String getTypeIDObjectString() {
			return "new ::hadoop::StructTypeID(" + fullName
					+ "::getTypeInfo().getFieldTypeInfos())";
		}
		...
		void genSetRTIFilter(CodeBuffer cb) {
			// we set the RTI filter here
			cb.append(fullName
					+ "::setTypeFilter(rti.getNestedStructTypeInfo(\"" + name
					+ "\"));\n");
		}
		...
		void genSetupRTIFields(CodeBuffer cb) {
			...
			cb.append("for (unsigned int " + Consts.RIO_PREFIX + "j=0; "
					...
					+ "j++) {\n");
			cb
					.append("if (*(p" + Consts.RTI_FILTER
							+ "->getFieldTypeInfos()[" + Consts.RIO_PREFIX
							+ "i]) == *(p" + Consts.RTI_VAR
							+ "->getFieldTypeInfos()[" + Consts.RIO_PREFIX
							+ "j])) {\n");
			cb.append("p" + Consts.RTI_FILTER_FIELDS + "[" + Consts.RIO_PREFIX
		...
		void genCode(FileWriter hh, FileWriter cc, ArrayList<String> options)
			...
			hb
					.append("virtual void deserializeWithoutFilter(::hadoop::IArchive& "
							+ Consts.RECORD_INPUT
							+ ", const char* "
							+ Consts.TAG + ");\n");
			hb.append("public:\n");
			...
			hb
					.append("static void setTypeFilter(const ::hadoop::RecordTypeInfo& rti);\n");
			hb
					.append("static void setTypeFilter(const ::hadoop::RecordTypeInfo* prti);\n");
			hb.append("virtual void serialize(::hadoop::OArchive& "
					+ Consts.RECORD_OUTPUT + ", const char* " + Consts.TAG
					+ ") const;\n");
			...
			hb.append("virtual bool operator<(const " + name
					+ "& peer_) const;\n");
			hb.append("virtual bool operator==(const " + name
					+ "& peer_) const;\n");
			...
			cb.append("::hadoop::RecordTypeInfo* " + fullName + "::p"
					+ Consts.RTI_VAR + " = " + fullName
					+ "::setupTypeInfo();\n");
			cb.append("::hadoop::RecordTypeInfo* " + fullName + "::p"
					+ Consts.RTI_FILTER + " = NULL;\n");
			cb.append("int* " + fullName + "::p" + Consts.RTI_FILTER_FIELDS
					+ " = NULL;\n\n");

			// setupTypeInfo()
			cb.append("::hadoop::RecordTypeInfo* " + fullName
					+ "::setupTypeInfo() {\n");
			cb
					.append("::hadoop::RecordTypeInfo* p = new ::hadoop::RecordTypeInfo(\""
							+ name + "\");\n");
			for (Iterator<JField<CppType>> i = fields.iterator(); i.hasNext();) {
				JField<CppType> jf = i.next();
				String name = jf.getName();
			...
			cb.append("}\n");
			cb.append("p" + Consts.RTI_FILTER
					+ " = new ::hadoop::RecordTypeInfo(rti);\n");
			cb.append("if (NULL != p" + Consts.RTI_FILTER_FIELDS + ") {\n");
			...
			for (Iterator<JField<CppType>> i = fields.iterator(); i.hasNext();) {
				...
				CppType type = jf.getType();
				if (type instanceof JBuffer.CppBuffer) {
					cb.append("{\nsize_t len=0; " + Consts.RECORD_INPUT
			...
			if (0 != ct) {
				cb.append("else {\n");
				cb
						.append("const std::vector< ::hadoop::FieldTypeInfo* >& typeInfos = p"
								+ Consts.RTI_FILTER
								+ "->getFieldTypeInfos();\n");
				cb.append("::hadoop::Utils::skip(" + Consts.RECORD_INPUT
						+ ", typeInfos[" + Consts.RIO_PREFIX
						+ "i]->getFieldID()->c_str()" + ", *(typeInfos["
						+ Consts.RIO_PREFIX + "i]->getTypeID()));\n");
				cb.append("}\n");
			}
			cb.append("}\n");
			...
			// operator <
			cb.append("bool " + fullName + "::operator< (const " + fullName
					+ "& peer_) const {\n");
			cb.append("return (1\n");
			for (Iterator<JField<CppType>> i = fields.iterator(); i.hasNext();) {
				JField<CppType> jf = i.next();
			...
			cb.append("const ::std::string&" + fullName
					+ "::signature() const {\n");
