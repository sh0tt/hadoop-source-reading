		...
		try {
			...
			while (true) {
				try {
					...
					LOG
							.info("listener.getLocalPort() returned "
									+ listener.getLocalPort()
									+ " webServer.getConnectors()[0].getLocalPort() returned "
									+ webServer.getConnectors()[0]
											.getLocalPort());
					...
					port = listener.getLocalPort();
					if (port < 0) {
						LOG.warn("Bounds port is " + port
								+ " after webserver start");
					...
					if (ex instanceof BindException) {
						...
						LOG
								.info("HttpServer.start() threw a non Bind IOException");
						throw ex;
					}
