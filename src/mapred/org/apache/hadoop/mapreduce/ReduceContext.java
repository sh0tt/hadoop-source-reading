 ...
 * @param <VALUEIN>
 *            the class of the input values
 * @param <KEYOUT>
 *            the class of the output keys
 * @param <VALUEOUT>
 *            the class of the output values
 */
public class ReduceContext<KEYIN, VALUEIN, KEYOUT, VALUEOUT> extends
	...
	public ReduceContext(Configuration conf, TaskAttemptID taskid,
			...
			Counter inputValueCounter, RecordWriter<KEYOUT, VALUEOUT> output,
		...
		this.valueDeserializer = serializationFactory
	...
	protected class ValueIterator implements Iterator<VALUEIN> {
		...
		public VALUEIN next() {
			...
			// if this isn't the first record and the next key is different,
			// they
			...
			try {
				...
				// this is bad, but we can't modify the exception list of
				// java.util
				throw new RuntimeException("next value iterator interrupted",
						ie);
			}
		}
