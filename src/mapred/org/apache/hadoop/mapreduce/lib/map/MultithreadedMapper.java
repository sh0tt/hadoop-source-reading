...
public class MultithreadedMapper<K1, V1, K2, V2> extends Mapper<K1, V1, K2, V2> {
	...
	public static <K1, V1, K2, V2> Class<Mapper<K1, V1, K2, V2>> getMapperClass(
			JobContext job) {
		return (Class<Mapper<K1, V1, K2, V2>>) job.getConfiguration().getClass(
				"mapred.map.multithreadedrunner.class", Mapper.class);
	}
	...
	private class SubMapRecordReader extends RecordReader<K1, V1> {
		...
		public boolean nextKeyValue() throws IOException, InterruptedException {
			synchronized (outer) {
				...
				key = ReflectionUtils.copy(outer.getConfiguration(), outer
						.getCurrentKey(), key);
	...
	private class MapRunner extends Thread {
		...
		MapRunner(Context context) throws IOException, InterruptedException {
			mapper = ReflectionUtils.newInstance(mapClass, context
					.getConfiguration());
			subcontext = new Context(outer.getConfiguration(), outer
					.getTaskAttemptID(), new SubMapRecordReader(),
