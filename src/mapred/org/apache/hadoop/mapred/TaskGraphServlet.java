...
public class TaskGraphServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		JobTracker tracker = (JobTracker) getServletContext().getAttribute(
				"job.tracker");
		...
		final boolean isMap = "map".equalsIgnoreCase(request
				.getParameter("type"));
		...
		// borderlines
		printLine(out, w + xmargin + 1, w + xmargin + 1, height + ymargin + 1,
				ymargin - 1, "#CCCCCC");
		printLine(out, xmargin - 1, w + xmargin + 1, ymargin - 1, ymargin - 1,
				"#CCCCCC");
		...
		for (int i = 0, barCnt = 0;; i += tasksPerBar, barCnt++) {
			...
			if (i >= xNotchInterval * xNotchCount) {
				...
				printText(out, x, height + ymargin + 15, String
						.valueOf(xNotchInterval * xNotchCount++), "middle");
			}
			if (i >= reports.length)
				break;
			...
			if (isMap) {
				...
				float[] progresses = getReduceAvarageProgresses(tasksPerBar, i,
						reports);
		...
		// y axis notches
		for (int i = 0; i <= 10; i++) {
			printLine(out, xmargin - 3, xmargin + 2, ymargin + (i * height)
					/ 10, ymargin + (i * height) / 10, "black");
	...
	private void printRect(PrintWriter out, int width, int height, int x,
			int y, String color) throws IOException {
		if (height > 0) {
	...
	private void printLine(PrintWriter out, int x1, int x2, int y1, int y2,
