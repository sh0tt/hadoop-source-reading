...
public class Submitter extends Configured implements Tool {
	...
	static Class<? extends Partitioner> getJavaPartitioner(JobConf conf) {
		return conf.getClass("hadoop.pipes.partitioner", HashPartitioner.class,
				Partitioner.class);
	}
	...
	private static void setupPipesJob(JobConf conf) throws IOException {
		...
		if (!getIsJavaRecordReader(conf) && !getIsJavaMapper(conf)) {
			conf.setClass("mapred.pipes.user.inputformat", conf
					.getInputFormat().getClass(), InputFormat.class);
	...
	public int run(String[] args) throws Exception {
		...
		cli.addOption("writer", false, "java classname of OutputFormat",
				"class");
		...
		try {
			...
			GenericOptionsParser genericParser = new GenericOptionsParser(
					getConf(), args);
			CommandLine results = parser.parse(cli.options, genericParser
					.getRemainingArgs());

			JobConf job = new JobConf(getConf());

			if (results.hasOption("input")) {
				FileInputFormat.setInputPaths(job, (String) results
						.getOptionValue("input"));
			}
			if (results.hasOption("output")) {
				FileOutputFormat.setOutputPath(job, new Path((String) results
			...
			if (results.hasOption("reduce")) {
				...
				job.setReducerClass(getClass(results, "reduce", job,
						Reducer.class));
			}
			if (results.hasOption("reduces")) {
				job.setNumReduceTasks(Integer.parseInt((String) results
						.getOptionValue("reduces")));
			}
