...
class IndexCache {
	...
	public IndexCache(JobConf conf) {
		this.conf = conf;
		totalMemoryAllowed = conf
				.getInt("mapred.tasktracker.indexcache.mb", 10) * 1024 * 1024;
	...
	public IndexRecord getIndexInformation(String mapId, int reduce,
				...
				while (null == info.mapSpillRecord) {
					try {
						...
						throw new IOException(
								"Interrupted waiting for construction", e);
					}
				}
			}
		...
		if (info.mapSpillRecord.size() == 0
			...
			throw new IOException("Invalid request " + " Map Id = " + mapId
					+ " Reducer = " + reduce + " Index Info Length = "
					+ info.mapSpillRecord.size());
		}
	...
	private static class IndexInformation {
		...
		int getSize() {
			return mapSpillRecord == null ? 0 : mapSpillRecord.size()
