...
public class IsolationRunner {
	private static final Log LOG = LogFactory.getLog(IsolationRunner.class
			.getName());
	...
	private static class FakeUmbilical implements TaskUmbilicalProtocol {
		...
		public void fsError(TaskAttemptID taskId, String message)
				throws IOException {
			LOG.info("Task " + taskId + " reporting file system error: "
					+ message);
		}
		...
		public MapTaskCompletionEventsUpdate getMapCompletionEvents(
			...
			return new MapTaskCompletionEventsUpdate(
					TaskCompletionEvent.EMPTY_ARRAY, false);
		}
	...
	private static ClassLoader makeClassLoader(JobConf conf, File workDir)
			throws IOException {
	...
	private static void fillInMissingMapOutputs(FileSystem fs,
			TaskAttemptID taskId, int numMaps, JobConf conf) throws IOException {
		Class<? extends WritableComparable> keyClass = conf
				.getMapOutputKeyClass().asSubclass(WritableComparable.class);
		Class<? extends Writable> valueClass = conf.getMapOutputValueClass()
	...
	public static void main(String[] args) throws ClassNotFoundException,
		...
		File workDirName = new File(lDirAlloc.getLocalPathToRead(
				TaskTracker.getLocalTaskDir(taskId.getJobID().toString(),
						taskId.toString())
						+ Path.SEPARATOR + "work", conf).toString());
		...
		Task task;
		if (isMap) {
			Path localSplit = new Path(new Path(jobFilename.toString())
					.getParent(), "split.dta");
			DataInputStream splitFile = FileSystem.getLocal(conf).open(
					localSplit);
