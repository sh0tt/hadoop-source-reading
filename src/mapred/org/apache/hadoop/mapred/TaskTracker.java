...
public class TaskTracker implements MRConstants, TaskUmbilicalProtocol,
	...
	synchronized void initialize() throws IOException {
		...
		if (localHostname == null) {
			this.localHostname = DNS.getDefaultHost(fConf.get(
					"mapred.tasktracker.dns.interface", "default"), fConf.get(
		...
		this.fConf.set("mapred.task.tracker.report.address", taskReportAddress
				.getHostName()
				...
				try {
					...
					for (FetchStatus f : fList) {
						...
						try {
							...
							LOG
									.warn("Ignoring exception that fetch for map completion"
											+ " events threw for "
											+ f.jobId
											+ " threw: "
	...
	private void localizeJob(TaskInProgress tip) throws IOException {
		...
		Path localJobFile = lDirAlloc.getLocalPathForWrite(getLocalJobDir(jobId
				.toString())
		...
		if (rjob == null) {
			...
			synchronized (rjob) {
				...
				if (!rjob.keepJobFiles) {
					directoryCleanupThread.addToQueue(fConf, getLocalFiles(
							fConf, getLocalJobDir(rjob.getJobID().toString())));
				}
				// Remove this job
				rjob.tasks.clear();
			}
     ...
     */
		...
		public TaskInProgress(Task task, JobConf conf, TaskLauncher launcher) {
			...
			localJobConf = null;
			taskStatus = TaskStatus.createTaskStatus(task.isMapTask(), task
					.getTaskID(), 0.0f, task.getState(), diagnosticInfo
					.toString(), "initializing", getName(), task
					.isTaskCleanupTask() ? TaskStatus.Phase.CLEANUP : task
		...
		private void localizeTask(Task task) throws IOException {
			...
			Path cwd = lDirAlloc.getLocalPathForWrite(getLocalTaskDir(task
			...
			if (fConf.get("slave.host.name") != null) {
				localJobConf.set("slave.host.name", fConf
						.get("slave.host.name"));
			}
			...
			if (staticResolutions != null && staticResolutions.size() > 0) {
				...
				localJobConf.set("hadoop.net.static.resolutions", str
						.toString());
			}
			if (task.isMapTask()) {
						...
						if (debugCommand != null) {
							...
							try {
								...
								taskStdout = FileUtil.makeShellPath(TaskLog
										.getRealTaskLogFileLocation(task
												.getTaskID(),
								...
								LOG
										.warn("Exception finding task's stdout/err/syslog files");
							}
							File workDir = null;
							try {
								workDir = new File(
							...
							if (executable != null) {
								try {
									...
									LOG
											.warn("Problem in the URI fragment for pipes executable");
								}
							}
							...
							try {
								...
								LOG
										.warn("Error in preparing wrapped debug command");
							}
			...
			synchronized (this) {
				try {
					...
					String taskDir = getLocalTaskDir(
							task.getJobID().toString(), taskId.toString(), task
									.isTaskCleanupTask());
					if (needCleanup) {
						if (runner != null) {
	...
	public static class MapOutputServlet extends HttpServlet {
		...
		public void doGet(HttpServletRequest request,
			...
			try {
				...
				// Index file
				Path indexFileName = lDirAlloc.getLocalPathToRead(TaskTracker
						.getIntermediateOutputDir(jobId, mapId)
						+ "/file.out.index", conf);
				...
				response.setHeader(RAW_MAP_OUTPUT_LENGTH, Long
						.toString(info.rawLength));

				// set the custom "Map-Output-Length" http header to
				// the actual number of bytes being transferred
				response.setHeader(MAP_OUTPUT_LENGTH, Long
						.toString(info.partLength));
