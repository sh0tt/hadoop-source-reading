...
class JobInProgress {
	...
	synchronized void addRunningTaskToTIP(TaskInProgress tip, TaskAttemptID id,
		...
		if (tip.isFirstAttempt(id)) {
			JobHistory.Task.logStarted(tip.getTIPId(), name, tip
					.getExecStartTime(), splits);
		}
	...
	private synchronized TaskInProgress findSpeculativeTask(
		...
		while (iter.hasNext()) {
			...
			if (!tip.hasRunOnMachine(ttStatus.getHost(), ttStatus
					.getTrackerName())) {
	...
	public synchronized boolean completedTask(TaskInProgress tip,
		...
		if (status.getIsMap()) {
			JobHistory.MapAttempt.logStarted(status.getTaskID(), status
					.getStartTime(), status.getTaskTracker(), ttStatus
					.getHttpPort(), taskType);
			JobHistory.MapAttempt.logFinished(status.getTaskID(), status
					.getFinishTime(), trackerHostname, taskType, status
					.getStateString(), status.getCounters());
		} else {
	...
	synchronized void fetchFailureNotification(TaskInProgress tip,
		...
		if (fetchFailures >= MAX_FETCH_FAILURES_NOTIFICATIONS && isMapFaulty) {
			...
			failedTask(tip, mapTaskId, "Too many fetch-failures", (tip
