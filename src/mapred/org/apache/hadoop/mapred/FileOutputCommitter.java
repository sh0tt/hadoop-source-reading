...
public class FileOutputCommitter extends OutputCommitter {
	...
	public static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.mapred.FileOutputCommitter");
	/**
	...
	public void abortTask(TaskAttemptContext context) throws IOException {
		...
		try {
			if (taskOutputPath != null) {
				FileSystem fs = taskOutputPath.getFileSystem(context
						.getJobConf());
				context.getProgressible().progress();
	...
	Path getTempTaskOutputPath(TaskAttemptContext taskContext) {
		...
		Path outputPath = FileOutputFormat.getOutputPath(conf);
		if (outputPath != null) {
			Path p = new Path(outputPath, (FileOutputCommitter.TEMP_DIR_NAME
