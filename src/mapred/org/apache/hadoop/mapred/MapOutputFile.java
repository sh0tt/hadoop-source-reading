...
class MapOutputFile {
	...
	public Path getOutputFile(TaskAttemptID mapTaskId) throws IOException {
		return lDirAlloc.getLocalPathToRead(TaskTracker
				.getIntermediateOutputDir(jobId.toString(), mapTaskId
						.toString())
				+ "/file.out", conf);
	}
	...
	public Path getSpillFileForWrite(TaskAttemptID mapTaskId, int spillNumber,
		...
		String dir = TaskTracker.getIntermediateOutputDir(jobId.toString(),
				mapTaskId.toString());
		return lDirAlloc.getLocalPathForWrite(dir + "/spill" + spillNumber
				+ ".out", size, conf);
	}
