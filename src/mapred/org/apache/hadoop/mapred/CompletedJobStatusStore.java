...
class CompletedJobStatusStore implements Runnable {
	...
	public static final Log LOG = LogFactory
			.getLog(CompletedJobStatusStore.class);
	...
	CompletedJobStatusStore(Configuration conf) throws IOException {
		active = conf.getBoolean("mapred.job.tracker.persist.jobstatus.active",
				false);

		if (active) {
			retainTime = conf.getInt(
			...
			LOG
					.info("Completed job store activated/configured with retain-time : "
	...
	private void deleteJobStatusDirs() {
		try {
			...
			FileStatus[] jobInfoFiles = fs.listStatus(new Path[] { new Path(
					jobInfoDir) });
			...
			for (FileStatus jobInfo : jobInfoFiles) {
				try {
					...
					LOG.warn("Could not do housekeeping for [ "
							+ jobInfo.getPath() + "] job info : "
							+ ie.getMessage(), ie);
				}
			}
		} catch (IOException ie) {
			LOG
					.warn("Could not obtain job info files : "
							+ ie.getMessage(), ie);
		}
	}
	...
	private JobStatus readJobStatus(FSDataInputStream dataIn)
			throws IOException {
	...
	public JobStatus readJobStatus(JobID jobId) {
		...
		if (active) {
			try {
				...
				LOG.warn("Could not read [" + jobId + "] job status : " + ex,
						ex);
			}
		}
		return jobStatus;
	...
	public JobProfile readJobProfile(JobID jobId) {
		...
		if (active) {
			try {
				...
				LOG.warn("Could not read [" + jobId + "] job profile : " + ex,
						ex);
			}
		}
		return jobProfile;
	...
	public Counters readCounters(JobID jobId) {
		...
		if (active) {
			try {
				...
				LOG.warn("Could not read [" + jobId + "] job counters : " + ex,
						ex);
			}
		}
		return counters;
	...
	public TaskCompletionEvent[] readJobTaskCompletionEvents(JobID jobId,
		...
		if (active) {
			try {
				...
				LOG.warn("Could not read [" + jobId + "] job events : " + ex,
						ex);
			}
		}
		return events;
