...
class ReduceTask extends Task {
	...
	public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
		...
		Path[] mapFiles = getMapFiles(rfs, true);
		boolean deleteInputs = !conf.getKeepFailedTaskFiles();
		int int1 = job.getInt("io.sort.factor", 100);
		Path tmpDir = new Path(getTaskID().toString());
		RawComparator outputKeyComparator = job.getOutputKeyComparator();
		RawKeyValueIterator rIter = isLocal ? Merger.merge(job, rfs,
				mapOutputKeyClass, mapOutputValueClass, codec, mapFiles,
				deleteInputs, int1, tmpDir, outputKeyComparator, reporter,
		...
		statusUpdate(umbilical);
		Class keyClass = mapOutputKeyClass;
		Class valueClass = mapOutputValueClass;
			...
			private int getMapCompletionEvents() throws IOException {
				...
				for (TaskCompletionEvent event : events) {
					...
					case SUCCEEDED: {
						...
						URL mapOutputLocation = new URL(event
								.getTaskTrackerHttp()
								+ "/mapOutput?job="
								+ taskId.getJobID()
								+ "&map="
