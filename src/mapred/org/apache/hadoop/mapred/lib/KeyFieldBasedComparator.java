...
public class KeyFieldBasedComparator<K, V> extends WritableComparator implements
		JobConfigurable {
	...
	public int compare(byte[] b1, int s1, int l1, byte[] b2, int s2, int l2) {
		...
		int[] lengthIndicesFirst = keyFieldHelper.getWordLengths(b1, s1 + n1,
				s1 + l1);
		int[] lengthIndicesSecond = keyFieldHelper.getWordLengths(b2, s2 + n2,
				s2 + l2);
		for (KeyDescription keySpec : allKeySpecs) {
			int startCharFirst = keyFieldHelper.getStartOffset(b1, s1 + n1, s1
					+ l1, lengthIndicesFirst, keySpec);
			int endCharFirst = keyFieldHelper.getEndOffset(b1, s1 + n1,
					s1 + l1, lengthIndicesFirst, keySpec);
			int startCharSecond = keyFieldHelper.getStartOffset(b2, s2 + n2, s2
					+ l2, lengthIndicesSecond, keySpec);
			int endCharSecond = keyFieldHelper.getEndOffset(b2, s2 + n2, s2
					+ l2, lengthIndicesSecond, keySpec);
			int result;
			if ((result = compareByteSequence(b1, startCharFirst, endCharFirst,
					b2, startCharSecond, endCharSecond, keySpec)) != 0) {
				return result;
			}
		}
		return 0;
	...
	private int compareByteSequence(byte[] first, int start1, int end1,
		...
		int compareResult = 0;
		if (!key.numeric) {
			compareResult = compareBytes(first, start1, end1 - start1 + 1,
					second, start2, end2 - start2 + 1);
		}
		if (key.numeric) {
			compareResult = numericalCompare(first, start1, end1, second,
					start2, end2);
		}
		if (key.reverse) {
	...
	private int numericalCompare(byte[] a, int start1, int end1, byte[] b,
			int start2, int end2) {
		int i = start1;
		int j = start2;
		int mul = 1;
	...
	private int decimalCompare(byte[] a, int i, int end1, byte[] b, int j,
			int end2) {
		if (i > end1) {
	...
	private int oneNegativeCompare(byte[] a, int start1, int end1, byte[] b,
			int start2, int end2) {
