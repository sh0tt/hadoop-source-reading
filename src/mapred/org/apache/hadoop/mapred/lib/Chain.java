...
class Chain {
	...
	private static JobConf getChainElementConf(JobConf jobConf, String confKey) {
		JobConf conf;
		try {
			Stringifier<JobConf> stringifier = new DefaultStringifier<JobConf>(
					jobConf, JobConf.class);
	 ...
	 *            FALSE.
	 */
	public static <K1, V1, K2, V2> void addMapper(boolean isMap,
			JobConf jobConf, Class<? extends Mapper<K1, V1, K2, V2>> klass,
			Class<? extends K1> inputKeyClass,
			Class<? extends V1> inputValueClass,
			Class<? extends K2> outputKeyClass,
			Class<? extends V2> outputValueClass, boolean byValue,
			JobConf mapperConf) {
		...
		jobConf.setClass(prefix + CHAIN_MAPPER_CLASS + index, klass,
				Mapper.class);
		...
		if (!isMap && index == 0) {
			...
			if (!inputValueClass.isAssignableFrom(reducerConf.getClass(
				...
				throw new IllegalArgumentException(
						"The Reducer output value class"
		...
		mapperConf
				.setClass(MAPPER_INPUT_KEY_CLASS, inputKeyClass, Object.class);
		mapperConf.setClass(MAPPER_INPUT_VALUE_CLASS, inputValueClass,
				Object.class);
		mapperConf.setClass(MAPPER_OUTPUT_KEY_CLASS, outputKeyClass,
				Object.class);
		...
		try {
			jobConf.set(prefix + CHAIN_MAPPER_CONFIG + index, stringifier
					.toString(new JobConf(mapperConf)));
		} catch (IOException ioEx) {
			throw new RuntimeException(ioEx);
		}
	...
	public static <K1, V1, K2, V2> void setReducer(JobConf jobConf,
		...
		reducerConf.setClass(REDUCER_INPUT_KEY_CLASS, inputKeyClass,
				Object.class);
		...
		try {
			jobConf.set(prefix + CHAIN_REDUCER_CONFIG, stringifier
					.toString(new JobConf(reducerConf)));
		} catch (IOException ioEx) {
			throw new RuntimeException(ioEx);
		}
	}
	...
	public void configure(JobConf jobConf) {
		...
		for (int i = 0; i < index; i++) {
			...
			JobConf mConf = getChainElementConf(jobConf, prefix
					+ CHAIN_MAPPER_CONFIG + i);
			Mapper mapper = ReflectionUtils.newInstance(klass, mConf);
			mappers.add(mapper);

			if (mConf.getBoolean(MAPPER_BY_VALUE, true)) {
				mappersKeySerialization.add(serializationFactory
						.getSerialization(mConf.getClass(
								MAPPER_OUTPUT_KEY_CLASS, null)));
				mappersValueSerialization.add(serializationFactory
						.getSerialization(mConf.getClass(
								MAPPER_OUTPUT_VALUE_CLASS, null)));
			} else {
		...
		if (klass != null) {
			JobConf rConf = getChainElementConf(jobConf, prefix
					+ CHAIN_REDUCER_CONFIG);
			...
			if (rConf.getBoolean(REDUCER_BY_VALUE, true)) {
				reducerKeySerialization = serializationFactory
						.getSerialization(rConf.getClass(
								REDUCER_OUTPUT_KEY_CLASS, null));
	...
	public OutputCollector getMapperCollector(int mapperIndex,
		...
		Serialization keySerialization = mappersKeySerialization
				.get(mapperIndex);
		Serialization valueSerialization = mappersValueSerialization
				.get(mapperIndex);
	...
	public OutputCollector getReducerCollector(OutputCollector output,
		...
		return new ChainOutputCollector(reducerKeySerialization,
				reducerValueSerialization, output, reporter);
	}
	...
	private class ChainOutputCollector<K, V> implements OutputCollector<K, V> {
		 ...
		 */
		public ChainOutputCollector(int index,
				Serialization<K> keySerialization,
				Serialization<V> valueSerialization, OutputCollector output,
				Reporter reporter) {
		...
		public void collect(K key, V value) throws IOException {
			if (nextMapperIndex < mappers.size()) {
				...
				Serialization nextKeySerialization = mappersKeySerialization
						.get(nextMapperIndex);
				Serialization nextValueSerialization = mappersValueSerialization
						.get(nextMapperIndex);
				Mapper nextMapper = mappers.get(nextMapperIndex);

				// invokes next mapper in chain
				nextMapper.map(key, value, new ChainOutputCollector(
						nextMapperIndex, nextKeySerialization,
		...
		private <E> E makeCopyForPassByValue(Serialization<E> serialization,
			...
			Serializer<E> ser = serialization.getSerializer(GenericsUtil
					.getClass(obj));
			Deserializer<E> deser = serialization.getDeserializer(GenericsUtil
					.getClass(obj));
			...
			obj = ReflectionUtils.newInstance(GenericsUtil.getClass(obj),
					getChainJobConf());
			ByteArrayInputStream bais = new ByteArrayInputStream(dof.getData(),
					0, dof.getLength());
			deser.open(bais);
