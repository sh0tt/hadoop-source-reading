...
public class MultipleOutputs {
	 ...
	 */
	public static Class<? extends Writable> getNamedOutputValueClass(
			JobConf conf, String namedOutput) {
		checkNamedOutput(conf, namedOutput, false);
	...
	private static void addNamedOutput(JobConf conf, String namedOutput,
		...
		conf
				.set(NAMED_OUTPUTS, conf.get(NAMED_OUTPUTS, "") + " "
						+ namedOutput);
		conf.setClass(MO_PREFIX + namedOutput + FORMAT, outputFormatClass,
				OutputFormat.class);
		conf.setClass(MO_PREFIX + namedOutput + KEY, keyClass, Object.class);
		conf
				.setClass(MO_PREFIX + namedOutput + VALUE, valueClass,
						Object.class);
	...
	public MultipleOutputs(JobConf job) {
		...
		namedOutputs = Collections.unmodifiableSet(new HashSet<String>(
				MultipleOutputs.getNamedOutputsList(job)));
	...
	private synchronized RecordWriter getRecordWriter(String namedOutput,
			String baseFileName, final Reporter reporter) throws IOException {
		...
		if (writer == null) {
			...
			jobConf.set(InternalFileOutputFormat.CONFIG_NAMED_OUTPUT,
					namedOutput);
			FileSystem fs = FileSystem.get(conf);
			writer = outputFormat.getRecordWriter(fs, jobConf, baseFileName,
					reporter);
	...
	public OutputCollector getCollector(String namedOutput, String multiName,
		...
		final RecordWriter writer = getRecordWriter(namedOutput, baseFileName,
				reporter);
	...
	private static class InternalFileOutputFormat extends
		...
		@SuppressWarnings( { "unchecked" })
		public RecordWriter<Object, Object> getRecordWriter(FileSystem fs,
				JobConf job, String baseFileName, Progressable progress)
				throws IOException {
			...
			outputConf.setOutputFormat(getNamedOutputFormatClass(job,
					nameOutput));
			outputConf
					.setOutputKeyClass(getNamedOutputKeyClass(job, nameOutput));
			outputConf.setOutputValueClass(getNamedOutputValueClass(job,
					nameOutput));
			OutputFormat outputFormat = outputConf.getOutputFormat();
			return outputFormat.getRecordWriter(fs, outputConf, fileName,
					progress);
		}
	}
