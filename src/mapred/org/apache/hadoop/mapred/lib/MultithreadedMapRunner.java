 ...
 *       <p>
 */
public class MultithreadedMapRunner<K1, V1, K2, V2> implements
		MapRunnable<K1, V1, K2, V2> {

	private static final Log LOG = LogFactory
			.getLog(MultithreadedMapRunner.class.getName());
	...
	public void configure(JobConf jobConf) {
		int numberOfThreads = jobConf.getInt(
		...
		executorService = new ThreadPoolExecutor(numberOfThreads,
				numberOfThreads, 0L, TimeUnit.MILLISECONDS,
	 ...
	 */
	private static class BlockingArrayQueue extends
	...
	private void checkForExceptionsFromProcessingThreads() throws IOException,
			RuntimeException {
	...
	public void run(RecordReader<K1, V1> input, OutputCollector<K2, V2> output,
		...
		try {
			...
			while (input.next(key, value)) {
				...
				executorService.execute(new MapperInvokeRunable(key, value,
						output, reporter));
