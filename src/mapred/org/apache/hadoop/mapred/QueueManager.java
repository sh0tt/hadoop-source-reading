...
class QueueManager {
	...
	static enum QueueOperation {
		SUBMIT_JOB("acl-submit-job", false), ADMINISTER_JOBS(
				"acl-administer-jobs", true);
	...
	public synchronized boolean hasAccess(String queueName, JobInProgress job,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("checking access for : "
					+ toFullPropertyName(queueName, oper.getAclName()));
		}
		...
		AccessControlList acl = aclsMap.get(toFullPropertyName(queueName, oper
				.getAclName()));
		if (acl == null) {
			return false;
		}
	...
	synchronized JobQueueInfo[] getJobQueueInfos() {
		...
		for (String queue : queueNames) {
			...
			if (schedulerInfo != null) {
				queueInfoList.add(new JobQueueInfo(queue, schedulerInfo
						.toString()));
			} else {
				queueInfoList.add(new JobQueueInfo(queue, null));
			}
		}
		return (JobQueueInfo[]) queueInfoList
				.toArray(new JobQueueInfo[queueInfoList.size()]);
	}
