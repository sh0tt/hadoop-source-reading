...
public class SequenceFileInputFilter<K, V> extends
	...
	public static class PercentFilter extends FilterBase {
		...
		public void setConf(Configuration conf) {
			...
			if (this.frequency <= 0) {
				throw new RuntimeException("Negative " + FILTER_FREQUENCY
						+ ": " + this.frequency);
			}
			this.conf = conf;
		}
	...
	public static class MD5Filter extends FilterBase {
		...
		synchronized private long MD5Hashcode(byte[] bytes, int start,
	...
	private static class FilterRecordReader<K, V> extends
		...
		public FilterRecordReader(Configuration conf, FileSplit split)
			...
			super(conf, split);
			// instantiate filter
			filter = (Filter) ReflectionUtils.newInstance(conf.getClass(
					FILTER_CLASS, PercentFilter.class), conf);
		}
