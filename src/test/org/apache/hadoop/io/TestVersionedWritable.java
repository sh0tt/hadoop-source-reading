...
public class TestVersionedWritable extends TestCase {
	...
	public static class SimpleVersionedWritable extends VersionedWritable {
		...
		public static SimpleVersionedWritable read(DataInput in)
				throws IOException {
	...
	public static class AdvancedVersionedWritable extends
			SimpleVersionedWritable {
		...
		String[] testStringArray = { "The", "Quick", "Brown", "Fox", "Jumped",
				"Over", "The", "Lazy", "Dog" };
		...
		public void write(DataOutput out) throws IOException {
			...
			int comp = WritableUtils.writeCompressedString(out,
		...
		public boolean equals(Object o) {
			...
			if (!shortTestString
					.equals(((AdvancedVersionedWritable) o).shortTestString)) {
				return false;
			}
			if (!longTestString
					.equals(((AdvancedVersionedWritable) o).longTestString)) {
				return false;
			}
			if (!compressableTestString
					.equals(((AdvancedVersionedWritable) o).compressableTestString)) {
				return false;
			}
			...
			for (int i = 0; i < testStringArray.length; i++) {
				if (!testStringArray[i]
						.equals(((AdvancedVersionedWritable) o).testStringArray[i])) {
					return false;
				}
			}

			if (!containedObject
					.equals(((AdvancedVersionedWritable) o).containedObject)) {
				return false;
			}
	...
	public static class SimpleVersionedWritableV2 extends
			SimpleVersionedWritable {
	...
	public void testSimpleVersionedWritableMismatch() throws Exception {
		TestVersionedWritable.testVersionedWritable(
				new SimpleVersionedWritable(), new SimpleVersionedWritableV2());
	}
