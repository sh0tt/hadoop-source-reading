...
public class TestSequenceFile extends TestCase {
	...
	public void compressedSeqFileTest(CompressionCodec codec) throws Exception {
		...
		Path file = new Path(System.getProperty("test.build.data", ".")
				+ "/test.seq");
		Path recordCompressedFile = new Path(System.getProperty(
				"test.build.data", ".")
				+ "/test.rc.seq");
		Path blockCompressedFile = new Path(System.getProperty(
				"test.build.data", ".")
				+ "/test.bc.seq");
		...
		try {
			...
			writeTest(fs, count, seed, recordCompressedFile,
			...
			writeTest(fs, count, seed, blockCompressedFile,
			...
			mergeTest(fs, count, seed, blockCompressedFile,
					CompressionType.BLOCK, false, factor, megabytes);
			checkSort(fs, count, seed, blockCompressedFile);

			mergeTest(fs, count, seed, blockCompressedFile,
					CompressionType.BLOCK, true, factor, megabytes);
			checkSort(fs, count, seed, blockCompressedFile);
	...
	private static void writeTest(FileSystem fs, int count, int seed,
			Path file, CompressionType compressionType, CompressionCodec codec)
			throws IOException {
		fs.delete(file, true);
	...
	private static void sortTest(FileSystem fs, int count, int megabytes,
			int factor, boolean fast, Path file) throws IOException {
	...
	private static void checkSort(FileSystem fs, int count, int seed, Path file)
		...
		RandomDatum k = new RandomDatum();
		RandomDatum v = new RandomDatum();
		Iterator<Map.Entry<RandomDatum, RandomDatum>> iterator = map.entrySet()
				.iterator();
	...
	private static void mergeTest(FileSystem fs, int count, int seed,
			Path file, CompressionType compressionType, boolean fast,
			int factor, int megabytes) throws IOException {

		LOG.debug("creating " + factor + " files with " + count / factor
				+ " records");
		...
		for (int i = 0; i < factor; i++) {
			LOG.debug("sorting file " + i + " with " + count / factor
					+ " records");
			newSorter(fs, fast, megabytes, factor).sort(names[i],
					sortedNames[i]);
		}

		LOG.info("merging " + factor + " files with " + count / factor
				+ " debug");
		fs.delete(new Path(file + ".sorted"), true);
		newSorter(fs, fast, megabytes, factor).merge(sortedNames,
				file.suffix(".sorted"));
	}

	private static SequenceFile.Sorter newSorter(FileSystem fs, boolean fast,
			int megabytes, int factor) {
		SequenceFile.Sorter sorter = fast ? new SequenceFile.Sorter(fs,
				new RandomDatum.Comparator(), RandomDatum.class,
				RandomDatum.class, conf) : new SequenceFile.Sorter(fs,
	...
	public void testSequenceFileMetadata() throws Exception {
		...
		try {
			// SequenceFile.Writer
			writeMetadataTest(fs, count, seed, file, CompressionType.NONE,
					null, theMetadata);
			...
			// SequenceFile.RecordCompressWriter
			writeMetadataTest(fs, count, seed, recordCompressedFile,
					CompressionType.RECORD, codec, theMetadata);
			...
			// SequenceFile.BlockCompressWriter
			writeMetadataTest(fs, count, seed, blockCompressedFile,
					CompressionType.BLOCK, codec, theMetadata);
	...
	private static void writeMetadataTest(FileSystem fs, int count, int seed,
			Path file, CompressionType compressionType, CompressionCodec codec,
			SequenceFile.Metadata metadata) throws IOException {
		fs.delete(file, true);
		LOG.info("creating " + count + " records with metadata and with"
	...
	public void testClose() throws IOException {
		...
		Path path1 = new Path(System.getProperty("test.build.data", ".")
				+ "/test1.seq");
		...
		Path path2 = new Path(System.getProperty("test.build.data", ".")
				+ "/test2.seq");
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			CompressionCodec codec = (CompressionCodec) ReflectionUtils
					.newInstance(conf.getClassByName(compressionCodec), conf);
			...
			if (!rwonly) {
				if (merge) {
					mergeTest(fs, count, seed, file, compressionType, fast,
							factor, megabytes);
				} else {
