...
public class TestTFileByteArrays extends TestCase {
	...
	public void testNoDataEntry() throws IOException {
		...
		Reader reader = new Reader(fs.open(path), fs.getFileStatus(path)
				.getLen(), conf);
	...
	public void testThreeBlocks() throws IOException {
		...
		checkBlockIndex(2 * records1stBlock + 5, records1stBlock
				+ records2ndBlock - 1, 1);
		checkBlockIndex(2 * records1stBlock + 5, records1stBlock
				+ records2ndBlock, 2);
		readValueBeforeKey(2 * records1stBlock + 5, records1stBlock
	...
	public void testLocate() throws IOException {
		...
		Reader reader = new Reader(fs.open(path), fs.getFileStatus(path)
				.getLen(), conf);
		Scanner scanner = reader.createScanner();
		Location loc2 = locate(scanner, composeSortedKey(KEY,
				3 * records1stBlock, 2).getBytes());
		Location locLastIn1stBlock = locate(scanner, composeSortedKey(KEY,
				3 * records1stBlock, records1stBlock - 1).getBytes());
		Location locFirstIn2ndBlock = locate(scanner, composeSortedKey(KEY,
				3 * records1stBlock, records1stBlock).getBytes());
	...
	public void testFailureWriterNotClosed() throws IOException {
		...
		Reader reader = null;
		try {
			reader = new Reader(fs.open(path), fs.getFileStatus(path).getLen(),
					conf);
	...
	public void testFailureWriteMetaBlocksWithSameName() throws IOException {
		...
		DataOutputStream outMeta = writer.prepareMetaBlock("testX",
		...
		try {
			DataOutputStream outMeta2 = writer.prepareMetaBlock("testX",
					Compression.Algorithm.GZ.getName());
	...
	static long writeRecords(Writer writer, int count) throws IOException {
		...
		for (nx = 0; nx < count; nx++) {
			...
			rawDataSize += WritableUtils.getVIntSize(key.length) + key.length
	...
	static void readRecords(FileSystem fs, Path path, int count,
		...
		try {
			for (int nx = 0; nx < count; nx++, scanner.advance()) {
				...
				Assert.assertEquals(new String(kbuf, 0, klen),
						composeSortedKey(KEY, count, nx));
	...
	private void readValueBeforeKey(int count, int recordIndex)
		...
		Reader reader = new Reader(fs.open(path), fs.getFileStatus(path)
				.getLen(), conf);
		Scanner scanner = reader.createScannerByKey(composeSortedKey(KEY,
				count, recordIndex).getBytes(), null);
		...
		try {
			...
			int klen = scanner.entry().getKeyLength();
			scanner.entry().getKey(kbuf);
			Assert.assertEquals(new String(kbuf, 0, klen), composeSortedKey(
					KEY, count, recordIndex));
		} finally {
			scanner.close();
			reader.close();
		}
	...
	private void readKeyWithoutValue(int count, int recordIndex)
		...
		try {
			...
			Assert.assertEquals(new String(kbuf1, 0, klen1), composeSortedKey(
					KEY, count, recordIndex));
			...
			if (scanner.advance() && !scanner.atEnd()) {
				...
				Assert.assertEquals(new String(kbuf2, 0, klen2),
						composeSortedKey(KEY, count, recordIndex + 1));
			}
		} finally {
			scanner.close();
			reader.close();
	...
	private void readKeyManyTimes(int count, int recordIndex)
			throws IOException {
		Reader reader = new Reader(fs.open(path), fs.getFileStatus(path)
				.getLen(), conf);
