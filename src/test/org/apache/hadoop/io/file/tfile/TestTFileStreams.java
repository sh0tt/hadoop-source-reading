...
public class TestTFileStreams extends TestCase {
	...
	public void testOneEntryUnknownLength() throws IOException {
		...
		// TODO: will throw exception at getValueLength, it's inconsistent
		// though;
	...
	private long writeRecords(int count, boolean knownKeyLength,
		...
		for (int nx = 0; nx < count; nx++) {
			...
			DataOutputStream outKey = writer
					.prepareAppendKey(knownKeyLength ? key.length() : -1);
			outKey.write(key.getBytes());
			outKey.close();
			String value = "value" + nx;
			DataOutputStream outValue = writer
					.prepareAppendValue(knownValueLength ? value.length() : -1);
			outValue.write(value.getBytes());
			outValue.close();
			rawDataSize += WritableUtils.getVIntSize(key.getBytes().length)
					+ key.getBytes().length
