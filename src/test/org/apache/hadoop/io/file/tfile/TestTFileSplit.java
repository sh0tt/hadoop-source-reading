...
public class TestTFileSplit extends TestCase {
	...
	void readFile() throws IOException {
		...
		for (int i = 0; i < numSplit; ++i, offset += splitSize) {
			Scanner scanner = reader
					.createScannerByByteRange(offset, splitSize);
			int count = 0;
	...
	/*
	 * Similar to readFile(), tests the scanner created by record numbers rather
	 * than the offsets.
	 */
	void readRowSplits(int numSplits) throws IOException {
		...
		for (int i = 0; i < numSplits; i++) {
			...
			long x = startRec;
			while (!scanner.atEnd()) {
				assertEquals("Incorrect RecNum returned by scanner", scanner
						.getRecordNum(), x);
				scanner.entry().get(key, value);
				++count;
				assertEquals("Incorrect RecNum returned by scanner", scanner
	...
	void checkRecNums() throws IOException {
		...
		for (long i = 0; i < 100; ++i) {
			assertEquals("Locaton to RecNum conversion not symmetric", i,
					reader.getRecordNumByLocation(reader
							.getLocationByRecordNum(i)));
		}

		for (long i = 1; i < 100; ++i) {
			long x = totalRecs - i;
			assertEquals("Locaton to RecNum conversion not symmetric", x,
					reader.getRecordNumByLocation(reader
							.getLocationByRecordNum(x)));
		}
