...
public class TestTFileSeqFileComparison extends TestCase {
	...
	private void setUpDictionary() {
		...
		for (int i = 0; i < options.dictSize; ++i) {
			int len = rng.nextInt(options.maxWordLen - options.minWordLen)
					+ options.minWordLen;
	...
	static class TFileAppendable implements KVAppendable {
		...
		public TFileAppendable(FileSystem fs, Path path, String compress,
			...
			this.writer = new TFile.Writer(fsdos, minBlkSize, compress, null,
					conf);
		}
	...
	static class TFileReadable implements KVReadable {
		...
		public TFileReadable(FileSystem fs, Path path, int osBufferSize,
			...
			this.reader = new TFile.Reader(fsdis, fs.getFileStatus(path)
					.getLen(), conf);
		...
		private void checkKeyBuffer(int size) {
			...
			keyBuffer = new byte[Math.max(2 * keyBuffer.length, 2 * size
					- keyBuffer.length)];
		}
		...
		private void checkValueBuffer(int size) {
			...
			valueBuffer = new byte[Math.max(2 * valueBuffer.length, 2 * size
					- valueBuffer.length)];
		}
	...
	static class SeqFileAppendable implements KVAppendable {
		...
		public SeqFileAppendable(FileSystem fs, Path path, int osBufferSize,
			...
			if (!"none".equals(compress)) {
				writer = SequenceFile.createWriter(conf, fsdos,
	...
	private void reportStats(Path path, long totalBytes) throws IOException {
		...
		printlnWithTimestamp(String.format(
				"Duration: %dms...total size: %.2fMB...raw thrpt: %.2fMB/s",
				duration, (double) totalBytes / 1024 / 1024,
				(double) totalBytes / duration * 1000 / 1024 / 1024));
		printlnWithTimestamp(String.format(
				"Compressed size: %.2fMB...compressed thrpt: %.2fMB/s.",
				(double) fsize / 1024 / 1024, (double) fsize / duration * 1000
						/ 1024 / 1024));
	}
	...
	private void createTFile(String parameters, String compress)
		...
		KVAppendable appendable = new TFileAppendable(fs, path, compress,
				options.minBlockSize, options.osOutputBufferSize, conf);
		timeWrite(path, appendable, options.keyLength, options.valueLength,
				options.fileSize);
	}

	private void readTFile(String parameters, boolean delFile)
			throws IOException {
		System.out.println("=== TFile: Reading (" + parameters + ") === ");
		{
			Path path = new Path(options.rootDir, "TFile.Performance");
			KVReadable readable = new TFileReadable(fs, path,
					options.osInputBufferSize, conf);
	...
	private void createSeqFile(String parameters, String compress)
		...
		KVAppendable appendable = new SeqFileAppendable(fs, path,
				options.osOutputBufferSize, compress, options.minBlockSize);
		timeWrite(path, appendable, options.keyLength, options.valueLength,
				options.fileSize);
	}
	...
	private void readSeqFile(String parameters, boolean delFile)
		...
		Path path = new Path(options.rootDir, "SeqFile.Performance");
		KVReadable readable = new SeqFileReadable(fs, path,
				options.osInputBufferSize);
		timeRead(path, readable);
		if (delFile) {
			if (fs.exists(path)) {
				fs.delete(path, true);
	...
	public void testRunComparisons() throws IOException {
		...
		for (String compress : compresses) {
			if (compress.equals("none")) {
				conf.setInt("tfile.fs.input.buffer.size",
						options.fsInputBufferSizeNone);
				conf.setInt("tfile.fs.output.buffer.size",
						options.fsOutputBufferSizeNone);
			} else if (compress.equals("lzo")) {
				conf.setInt("tfile.fs.input.buffer.size",
						options.fsInputBufferSizeLzo);
				conf.setInt("tfile.fs.output.buffer.size",
						options.fsOutputBufferSizeLzo);
			} else {
				conf.setInt("tfile.fs.input.buffer.size",
						options.fsInputBufferSizeGz);
				conf.setInt("tfile.fs.output.buffer.size",
						options.fsOutputBufferSizeGz);
			}
			compareRun(compress);
		}
	}
	...
	private static String parameters2String(MyOptions options) {
		return String
				.format(
						...
						options.keyLength, options.keyLength * 2,
						options.valueLength, options.valueLength * 2,
						(double) options.minBlockSize / 1024,
						(double) options.fileSize / 1024 / 1024,
						options.compress);
	}

	private static class MyOptions {
		String rootDir = System.getProperty("test.build.data",
				"/tmp/tfile-test");
		String compress = "gz";
		String format = "tfile";
		int dictSize = 1000;
		...
		private Options buildOptions() {
			Option compress = OptionBuilder.withLongOpt("compress")
					.withArgName("[none|lzo|gz]").hasArg().withDescription(
							"compression scheme").create('c');

			Option ditSize = OptionBuilder.withLongOpt("dict").withArgName(
					"size").hasArg().withDescription(
					"number of dictionary entries").create('d');

			Option fileSize = OptionBuilder.withLongOpt("file-size")
					.withArgName("size-in-MB").hasArg().withDescription(
							"target size of the file (in MB).").create('s');

			Option format = OptionBuilder.withLongOpt("format").withArgName(
					"[tfile|seqfile]").hasArg().withDescription(
					"choose TFile or SeqFile").create('f');

			Option fsInputBufferSz = OptionBuilder.withLongOpt(
					"fs-input-buffer").withArgName("size").hasArg()
					.withDescription(
							"size of the file system input buffer (in bytes).")
					.create('i');

			Option fsOutputBufferSize = OptionBuilder
					.withLongOpt("fs-output-buffer")
					.withArgName("size")
					.hasArg()
					.withDescription(
							"size of the file system output buffer (in bytes).")
					.create('o');

			Option keyLen = OptionBuilder
			...
			Option wordLen = OptionBuilder.withLongOpt("word-length")
					.withArgName("min,max").hasArg().withDescription(
							"range of dictionary word length (in bytes)")
					.create('w');

			Option blockSz = OptionBuilder.withLongOpt("block").withArgName(
					"size-in-KB").hasArg().withDescription(
					"minimum block size (in KB)").create('b');

			Option seed = OptionBuilder.withLongOpt("seed").withArgName(
					"long-int").hasArg().withDescription("specify the seed")
					.create('S');
			...
			Option operation = OptionBuilder
					...
					.hasArg()
					.withDescription(
							"action: read-only, create-only, read-after-create")
					.create('x');

			Option rootDir = OptionBuilder.withLongOpt("root-dir").withArgName(
					"path").hasArg().withDescription(
			...
			return new Options().addOption(compress).addOption(ditSize)
					.addOption(fileSize).addOption(format).addOption(
							fsInputBufferSz).addOption(fsOutputBufferSize)
					.addOption(keyLen).addOption(wordLen).addOption(blockSz)
					.addOption(rootDir).addOption(valueLen)
					.addOption(operation).addOption(help);
		...
		private void processOptions(CommandLine line, Options opts)
			...
			if (line.hasOption('w')) {
				...
				if (st.countTokens() != 2) {
					throw new ParseException("Bad word length specification: "
							+ min_max);
				}
			...
			if (line.hasOption('x')) {
				...
				if (strOp.equals("r")) {
					...
					throw new ParseException("Unknown action specifier: "
							+ strOp);
				}
			}
		...
		private void validateOptions() throws ParseException {
			if (!compress.equals("none") && !compress.equals("lzo")
					&& !compress.equals("gz")) {
				throw new ParseException("Unknown compression scheme: "
						+ compress);
			}
