...
public class TestTFile extends TestCase {
	...
	private int readAndCheckbytes(Scanner scanner, int start, int n)
		...
		for (int i = start; i < (start + n); i++) {
			...
			assertTrue("bytes for vals do not match " + valStr + " "
					+ new String(val), Arrays.equals(valStr.getBytes(), val));
			assertTrue(scanner.advance());
			key = readKey(scanner);
			val = readValue(scanner);
			assertTrue("btyes for keys do not match", Arrays.equals(keyStr
					.getBytes(), key));
			assertTrue("bytes for vals do not match", Arrays.equals(valStr
					.getBytes(), val));
			assertTrue(scanner.advance());
		}
		return (start + n);
	...
	private void readEmptyRecords(Scanner scanner, int n) throws IOException {
		...
		for (int i = 0; i < n; i++) {
			...
			assertTrue("failed to match values", Arrays
					.equals(readValue, value));
	...
	private int readPrepWithKnownLength(Scanner scanner, int start, int n)
		...
		for (int i = start; i < (start + n); i++) {
			...
			assertTrue("values not equal", Arrays
					.equals(value.getBytes(), read));
			scanner.advance();
		}
		return (start + n);
	}
	...
	private int readPrepWithUnknownLength(Scanner scanner, int start, int n)
		...
		for (int i = start; i < start; i++) {
			...
			assertTrue("values nto equal", Arrays
					.equals(read, value.getBytes()));
			scanner.advance();
		}
		return (start + n);
	}
	...
	void basicWithSomeCodec(String codec) throws IOException {
		...
		Reader reader = new Reader(fs.open(ncTFile), fs.getFileStatus(ncTFile)
				.getLen(), conf);
	...
	void unsortedWithSomeCodec(String codec) throws IOException {
		...
		Reader reader = new Reader(fs.open(uTfile), fs.getFileStatus(uTfile)
				.getLen(), conf);
	...
	private void writeNumMetablocks(Writer writer, String compression, int n)
			throws IOException {
		for (int i = 0; i < n; i++) {
			DataOutputStream dout = writer.prepareMetaBlock("TfileMeta" + i,
					compression);
