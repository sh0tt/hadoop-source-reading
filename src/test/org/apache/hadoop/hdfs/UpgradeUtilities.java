...
public class UpgradeUtilities {
	...
	private static File namenodeStorage = new File(TEST_ROOT_DIR,
			"namenodeMaster");
	...
	private static File datanodeStorage = new File(TEST_ROOT_DIR,
			"datanodeMaster");
	...
	public static void initialize() throws Exception {
		...
		try {
			...
			namenodeStorageNamespaceID = namenode.versionRequest()
					.getNamespaceID();
		...
		namenodeStorageChecksum = checksumContents(NAME_NODE, new File(
				namenodeStorage, "current"));
		datanodeStorageChecksum = checksumContents(DATA_NODE, new File(
				datanodeStorage, "current"));
	}
	...
	public static Configuration initializeStorageStateConf(int numDirs,
			Configuration conf) {
		StringBuffer nameNodeDirs = new StringBuffer(new File(TEST_ROOT_DIR,
				"name1").toString());
		StringBuffer dataNodeDirs = new StringBuffer(new File(TEST_ROOT_DIR,
				"data1").toString());
	 ...
	 * given node type.
	 */
	public static long checksumMasterContents(NodeType nodeType)
			throws IOException {
		...
		for (int i = 0; i < parents.length; i++) {
			...
			case NAME_NODE:
				localFS.copyToLocalFile(new Path(namenodeStorage.toString(),
						"current"), new Path(newDir.toString()), false);
				Path newImgDir = new Path(newDir.getParent(), "image");
				if (!localFS.exists(newImgDir))
					localFS.copyToLocalFile(new Path(
							namenodeStorage.toString(), "image"), newImgDir,
							false);
				break;
			case DATA_NODE:
				localFS.copyToLocalFile(new Path(datanodeStorage.toString(),
						"current"), new Path(newDir.toString()), false);
				Path newStorageFile = new Path(newDir.getParent(), "storage");
				if (!localFS.exists(newStorageFile))
					localFS.copyToLocalFile(new Path(
							datanodeStorage.toString(), "storage"),
							newStorageFile, false);
				break;
			}
			retVal[i] = newDir;
		}
	...
	public static File[] createVersionFile(NodeType nodeType, File[] parent,
		...
		for (int i = 0; i < parent.length; i++) {
			...
			StorageDirectory sd = storage.new StorageDirectory(parent[i]
					.getParentFile());
	...
	public static void corruptFile(File file) throws IOException {
		if (!file.isFile()) {
			throw new IllegalArgumentException("Given argument is not a file:"
					+ file);
		}
