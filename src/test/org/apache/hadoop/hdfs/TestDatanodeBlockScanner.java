...
public class TestDatanodeBlockScanner extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog(TestDatanodeBlockScanner.class);

	private static Pattern pattern = Pattern
			.compile(".*?(blk_[-]*\\d+).*?scan time\\s*:\\s*(\\d+)");

	private static Pattern pattern_blockVerify = Pattern
			.compile(".*?(SCAN_PERIOD)\\s*:\\s*(\\d+.*?)");
	...
	private static long waitForVerification(DatanodeInfo dn, FileSystem fs,
		...
		while (verificationTime <= 0) {
			...
			if (blocksValidated >= 0) {
				for (Matcher matcher = pattern_blockVerify.matcher(response); matcher
						.find();) {
					if (block.equals(matcher.group(1))) {
						assertEquals(
								"Wrong number of blocks reported for validation.",
								blocksValidated, Long.parseLong(matcher
										.group(2)));
						break;
					}
				}
	...
	public void testDatanodeBlockScanner() throws IOException {
		...
		IOUtils.copyBytes(fs.open(file2), new IOUtils.NullOutputStream(), conf,
				true);
	...
	public static boolean corruptReplica(String blockName, int replica)
			throws IOException {
	...
	private void blockCorruptionRecoveryPolicy(int numDataNodes,
			short numReplicas, int numCorruptReplicas) throws IOException {
		Configuration conf = new Configuration();
		...
		while (replicaCount != numReplicas) {
			try {
				LOG.info("Looping until expected replicaCount of "
						+ numReplicas + "is reached");
				Thread.sleep(1000);
			} catch (InterruptedException ignore) {
			}
			blocks = dfsClient.namenode.getBlockLocations(file1.toString(), 0,
		...
		int corruptReplicaSize = cluster.getNameNode().namesystem.corruptReplicas
				.numCorruptReplicas(blk);
		while (corruptReplicaSize != numCorruptReplicas) {
			try {
				IOUtils.copyBytes(fs.open(file1),
						new IOUtils.NullOutputStream(), conf, true);
			} catch (IOException e) {
			}
			try {
				LOG.info("Looping until expected " + numCorruptReplicas
						+ " are " + "reported. Current reported "
						+ corruptReplicaSize);
				Thread.sleep(1000);
			} catch (InterruptedException ignore) {
			}
			corruptReplicaSize = cluster.getNameNode().namesystem.corruptReplicas
					.numCorruptReplicas(blk);
		}
		...
		while (replicaCount != numReplicas) {
			try {
				LOG.info("Looping until block gets rereplicated to "
						+ numReplicas);
				Thread.sleep(1000);
			} catch (InterruptedException ignore) {
			}
	...
	public void testTruncatedBlockReport() throws Exception {
		...
		try {
			...
			String block = DFSTestUtil.getFirstBlock(fs, fileName)
					.getBlockName();
			...
			// restart the cluster
			cluster = new MiniDFSCluster(0, conf, REPLICATION_FACTOR, false,
					true, null, null, null);
