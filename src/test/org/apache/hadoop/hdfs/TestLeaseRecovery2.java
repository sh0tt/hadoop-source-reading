...
public class TestLeaseRecovery2 extends junit.framework.TestCase {
	...
	public void testBlockSynchronization() throws Exception {
		...
		try {
			...
			FSDataOutputStream stm = dfs.create(filepath, true, bufferSize,
					repl, BLOCK_SIZE);
			...
			// should fail but will trigger lease recovery.
			{
				...
				String username = UserGroupInformation.getCurrentUGI()
						.getUserName()
						+ "_1";
				...
				for (int i = 0; i < 10 && !done; i++) {
					...
					try {
						dfs2.create(filepath, false, bufferSize, repl,
								BLOCK_SIZE);
						...
						if (message.contains("file exists")) {
							AppendTestUtil.LOG.info("done", ioe);
								...
								.contains(AlreadyBeingCreatedException.class
										.getSimpleName())) {
							AppendTestUtil.LOG.info("GOOD! got " + message);
						} else {
							AppendTestUtil.LOG.warn("UNEXPECTED IOException",
									ioe);
						}
					}
			...
			AppendTestUtil.LOG.info("Lease for file " + filepath
					+ " is recovered. " + "Validating its contents now...");
