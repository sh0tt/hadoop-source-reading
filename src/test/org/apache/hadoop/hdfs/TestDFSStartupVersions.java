...
public class TestDFSStartupVersions extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.hdfs.TestDFSStartupVersions");
	...
	void log(String label, NodeType nodeType, Integer testCase,
		...
		LOG
				.info("============================================================");
		LOG.info("***TEST*** " + label + ":" + testCaseLine + " nodeType="
				+ nodeType + " layoutVersion=" + version.getLayoutVersion()
				+ " namespaceID=" + version.getNamespaceID() + " fsscTime="
				+ version.getCTime());
	}
	...
	boolean isVersionCompatible(StorageInfo namenodeVer, StorageInfo datanodeVer) {
		...
		if (softwareLV == storedLV
			...
			LOG
					.info("layoutVersions and cTimes are equal: isVersionCompatible=true");
			return true;
		}
		// check #2
		...
		if (absSoftwareLV > absStoredLV
				|| (softwareLV == storedLV && datanodeVer.getCTime() < namenodeVer
						.getCTime())) {
			LOG
					.info("softwareLayoutVersion is newer OR namenode cTime is newer: isVersionCompatible=true");
			return true;
		}
		// check #4
	...
	public void testVersions() throws Exception {
		...
		UpgradeUtilities.createStorageDirs(NAME_NODE, conf
				.getStrings("dfs.name.dir"), "current");
		cluster = new MiniDFSCluster(conf, 0, StartupOption.REGULAR);
		StorageInfo nameNodeVersion = new StorageInfo(UpgradeUtilities
				.getCurrentLayoutVersion(), UpgradeUtilities
				.getCurrentNamespaceID(cluster), UpgradeUtilities
				.getCurrentFsscTime(cluster));
		log("NameNode version info", NAME_NODE, null, nameNodeVersion);
		for (int i = 0; i < versions.length; i++) {
			File[] storage = UpgradeUtilities.createStorageDirs(DATA_NODE, conf
					.getStrings("dfs.data.dir"), "current");
			log("DataNode version info", DATA_NODE, i, versions[i]);
			UpgradeUtilities.createVersionFile(DATA_NODE, storage, versions[i]);
			try {
				cluster.startDataNodes(conf, 1, false, StartupOption.REGULAR,
						null);
