...
public class TestDFSUpgradeFromImage extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.hdfs.TestDFSUpgradeFromImage");
	...
	public void unpackStorage() throws IOException {
		...
		BufferedReader reader = new BufferedReader(new FileReader(System
				.getProperty("test.cache.data", "build/test/cache")
				+ "/hadoop-dfs-dir.txt"));
		String line;
	...
	private void verifyChecksum(String path, long checksum) throws IOException {
		...
		if (printChecksum) {
			LOG.info("CRC info for reference file : " + path + " \t "
					+ checksum);
		} else {
			if (!refIter.hasNext()) {
			...
			assertEquals("Checking checksum for " + path, info.checksum,
					checksum);
		}
	}
