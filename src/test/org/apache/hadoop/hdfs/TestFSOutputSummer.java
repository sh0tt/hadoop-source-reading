...
public class TestFSOutputSummer extends TestCase {
	...
	private void writeFile1(Path name) throws Exception {
		FSDataOutputStream stm = fileSys.create(name, true, fileSys.getConf()
				.getInt("io.file.buffer.size", 4096), NUM_OF_DATANODES,
				BLOCK_SIZE);
		stm.write(expected);
		stm.close();
		checkFile(name);
	...
	private void writeFile3(Path name) throws Exception {
		...
		stm.write(expected, BLOCK_SIZE + BYTES_PER_CHECKSUM, FILE_SIZE - 3
				* BYTES_PER_CHECKSUM);
		stm.close();
		checkFile(name);
		cleanupFile(name);
	...
	public void testFSOutputSummer() throws Exception {
		...
		MiniDFSCluster cluster = new MiniDFSCluster(conf, NUM_OF_DATANODES,
				true, null);
