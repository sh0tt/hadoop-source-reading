...
public class TestCrcCorruption extends TestCase {
		...
		try {
				...
				if (blocks[idx].getName().startsWith("blk_")
					...
					if (num % 3 == 0) {
						...
						System.out.println("Deliberately truncating file "
								+ blocks[idx].getName() + " to size " + newsize
								+ " bytes.");
						...
						//
						if (num != 2) {
							position = (long) random.nextInt((int) channel
									.size());
						}
						int length = random.nextInt((int) (channel.size()
								- position + 1));
						byte[] buffer = new byte[length];
						random.nextBytes(buffer);
						channel.write(ByteBuffer.wrap(buffer), position);
						System.out.println("Deliberately corrupting file "
								+ blocks[idx].getName() + " at offset "
			...
			for (int idx = 0; idx < blocks.length; idx++) {
				if (blocks[idx].getName().startsWith("blk_")
					...
					if (count % 2 == 0) {
						System.out
								.println("Deliberately insertimg bad crc into files "
										+ blocks[idx].getName()
										+ " "
										+ previous.getName());
						assertTrue("Cannot remove file.", blocks[idx].delete());
						assertTrue("Cannot corrupt meta file.", previous
								.renameTo(blocks[idx]));
						assertTrue("Cannot recreate empty meta file.", previous
								.createNewFile());
						previous = null;
					} else {
						previous = blocks[idx];
			...
			// be good. Verify.
			//
			assertTrue("Corrupted replicas not handled properly.", util
					.checkFiles(fs, "/srcdat"));
	...
	public void testCrcCorruption() throws Exception {
		...
		DFSTestUtil util1 = new DFSTestUtil("TestCrcCorruption", 40, 3,
				8 * 1024);
	...
	/**
	 * Make a single-DN cluster, corrupt a block, and make sure there's no
	 * infinite loop, but rather it eventually reports the exception to the
	 * client.
	 */
	...
	/**
	 * Same thing with multiple datanodes - in history, this has behaved
	 * differently than the above.
	 * 
	 * This test usually completes in around 15 seconds - if it times out, this
	 * suggests that the client is retrying indefinitely.
	 */
	...
	private void doTestEntirelyCorruptFile(int numDataNodes) throws Exception {
		...
		try {
			...
			DFSTestUtil.createFile(fs, file, fileSize, (short) numDataNodes,
					12345L /* seed */);
			...
			try {
				IOUtils.copyBytes(fs.open(file),
