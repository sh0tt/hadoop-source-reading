...
public class TestDFSClientRetries extends TestCase {
	public static final Log LOG = LogFactory.getLog(TestDFSClientRetries.class
			.getName());
	...
	class TestNameNode implements ClientProtocol {
		...
		public final String ADD_BLOCK_EXCEPTION = "Testing exception thrown from"
				+ "TestDFSClientRetries::" + "TestNameNode::addBlock";
		...
		public LocatedBlock addBlock(String src, String clientName)
			...
			if (num_calls > num_calls_allowed) {
				...
				throw new RemoteException(NotReplicatedYetException.class
						.getName(), ADD_BLOCK_EXCEPTION);
			}
		}

		// The following methods are stub methods that are not needed by this
		// mock class
		...
		public DatanodeInfo[] getDatanodeReport(
				FSConstants.DatanodeReportType type) throws IOException {
			return null;
		}
	...
	public void testNotYetReplicatedErrors() throws IOException {
		...
		try {
			...
			assertTrue("Retries are not being stopped correctly", e
					.getMessage().equals(tnn.ADD_BLOCK_EXCEPTION));
		}
	}
	...
	public void testFailuresArePerOperation() throws Exception {
		...
		try {
			...
			DFSTestUtil
					.createFile(fs, file, fileSize, (short) 1, 12345L /* seed */);
			...
			doAnswer(new FailNTimesAnswer(preSpyNN, maxBlockAcquires + 1))
					.when(spyNN).getBlockLocations(anyString(), anyLong(),
							anyLong());
			try {
			...
			doAnswer(new FailNTimesAnswer(preSpyNN, maxBlockAcquires)).when(
					spyNN).getBlockLocations(anyString(), anyLong(), anyLong());
			IOUtils.copyBytes(client.open(file.toString()),
					new IOUtils.NullOutputStream(), conf, true);

			DFSClient.LOG.info("Starting test case for failure reset");

			// Now the tricky case - if we fail a few times on one read, then
			// succeed,
			...
			// Further reads at this point will succeed since it has the good
			// block locations.
			// So, force the block locations on this stream to be refreshed from
			// bad info.
			// When reading again, it should start from a fresh failure count,
			// since
			// we're starting a new operation on the user level.
			doAnswer(new FailNTimesAnswer(preSpyNN, maxBlockAcquires)).when(
					spyNN).getBlockLocations(anyString(), anyLong(), anyLong());
			is.openInfo();
			// Seek to beginning forces a reopen of the BlockReader - otherwise
			// it'll
			// just keep reading on the existing stream and the fact that we've
			// poisoned
	...
	/**
	 * Mock Answer implementation of NN.getBlockLocations that will return a
	 * poisoned block list a certain number of times before returning a proper
	 * one.
	 */
	private static class FailNTimesAnswer implements Answer<LocatedBlocks> {
		...
		private LocatedBlocks makeBadBlockList(LocatedBlocks goodBlockList) {
			...
			LocatedBlock badLocatedBlock = new LocatedBlock(goodLocatedBlock
					.getBlock(), new DatanodeInfo[] { new DatanodeInfo(
					new DatanodeID("255.255.255.255:234")) }, goodLocatedBlock
