...
public class DataNodeCluster {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) { // parse command line
			if (args[i].equals("-n")) {
				...
				conf.setBoolean(SimulatedFSDataset.CONFIG_PROPERTY_SIMULATED,
						true);
			} else if (args[i].equals("-inject")) {
				if (!conf.getBoolean(
						SimulatedFSDataset.CONFIG_PROPERTY_SIMULATED, false)) {
		...
		boolean simulated = conf.getBoolean(
				SimulatedFSDataset.CONFIG_PROPERTY_SIMULATED, false);
		...
		if (numRacks > 0) {
			...
			for (int i = 0; i < numDataNodes; ++i) {
				...
				System.out.println("Data Node " + i + " using "
						+ rack4DataNode[i]);
			...
			if (inject) {
				...
				for (int i_dn = 0; i_dn < numDataNodes; ++i_dn) {
					...
					for (int i = 1; i <= replication; ++i) {
						...
						System.out.println("Injecting blocks of dn " + i_dn
								+ " into dn" + ((i_dn + i - 1) % numDataNodes));
					}
				}
	...
	static private String getUniqueRackPrefix() {
		...
		try {
			...
			System.out
					.println("Could not find ip address of \"default\" inteface.");
		}
