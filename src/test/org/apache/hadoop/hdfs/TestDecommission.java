...
public class TestDecommission extends TestCase {
	...
	private void writeConfigFile(FileSystem fs, Path name,
	...
	private void checkFile(FileSystem fileSys, Path name, int repl,
		...
		DFSClient.DFSDataInputStream dis = (DFSClient.DFSDataInputStream) ((DistributedFileSystem) fileSys)
				.open(name);
		...
		for (LocatedBlock blk : dinfo) { // for each block
			...
			assertEquals("Number of replicas for block" + blk.getBlock(), Math
					.min(numDatanodes, repl + hasdown), nodes.length);
		}
	}
	 ...
	 */
	private String decommissionNode(NameNode namenode, Configuration conf,
			DFSClient client, FileSystem localFileSys) throws IOException {
	 ...
	 */
	private boolean checkNodeState(FileSystem filesys, String node,
		...
		for (int i = 0; i < datanodes.length; i++) {
			...
			if (dn.getName().equals(node)) {
				if (state == NodeState.DECOMMISSIONED) {
					...
					done = (!dn.isDecommissionInProgress() && !dn
							.isDecommissioned());
				}
	 ...
	 */
	private void waitNodeState(FileSystem filesys, String node, NodeState state)
			throws IOException {
