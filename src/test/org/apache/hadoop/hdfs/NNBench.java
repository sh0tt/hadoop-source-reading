...
public class NNBench {
	private static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.hdfs.NNBench");
	...
	private static void createControlFiles() throws IOException {
		...
		for (int i = 0; i < numberOfMaps; i++) {
			...
			try {
				writer = SequenceFile.createWriter(tempFS, config, filePath,
	...
	private static void analyzeResults() throws IOException {
		...
		double totalTimeTPS = (longestMapTimeTPmS == 0) ? (1000 * successfulFileOps)
				: (double) (1000 * successfulFileOps)
						/ (double) longestMapTimeTPmS;
		...
		if (operation.equals(OP_CREATE_WRITE)) {
			...
			resultALLine1 = "            Avg Lat (ms): Create/Write: "
					+ avgLatency1;
			resultALLine2 = "                   Avg Lat (ms): Close: "
					+ avgLatency2;
			...
			resultTPSLine2 = "         Avg Exec time (ms): Open/Read: "
					+ (double) AverageExecutionTime;
			resultALLine1 = "                    Avg Lat (ms): Open: "
					+ avgLatency1;
			if (readFileAfterOpen) {
				resultALLine2 = "                  Avg Lat (ms): Read: "
						+ avgLatency2;
			}
			...
			resultTPSLine2 = "            Avg Exec time (ms): Rename: "
					+ (double) AverageExecutionTime;
			resultALLine1 = "                  Avg Lat (ms): Rename: "
					+ avgLatency1;
			...
			resultTPSLine2 = "            Avg Exec time (ms): Delete: "
					+ (double) AverageExecutionTime;
			resultALLine1 = "                  Avg Lat (ms): Delete: "
					+ avgLatency1;
		}
	...
	public static void validateInputs() {
		...
		if (!operation.equals(OP_CREATE_WRITE)
				...
				&& !operation.equals(OP_RENAME) && !operation.equals(OP_DELETE)) {
		...
		if (numberOfMaps < 0) {
			System.err
					.println("Error: Number of maps must be a positive number");
			displayUsage();
			System.exit(-1);
		}
		...
		if (numberOfReduces <= 0) {
			System.err
					.println("Error: Number of reduces must be a positive number");
			displayUsage();
			System.exit(-1);
		}
		...
		if (bytesToWrite < 0) {
			System.err
					.println("Error: Bytes to write must be a positive number");
			displayUsage();
			System.exit(-1);
		}
		...
		if (bytesPerChecksum < 0) {
			System.err
					.println("Error: Bytes per checksum must be a positive number");
			displayUsage();
			System.exit(-1);
		}
		...
		if (numberOfFiles < 0) {
			System.err
					.println("Error: Number of files must be a positive number");
			displayUsage();
			System.exit(-1);
		}
		...
		if (replicationFactorPerFile < 0) {
			System.err
					.println("Error: Replication factor must be a positive number");
			displayUsage();
			System.exit(-1);
		}
		...
		if (blockSize % bytesPerChecksum != 0) {
			System.err
					.println("Error: Block Size in bytes must be a multiple of "
							+ "bytes per checksum: ");
			displayUsage();
			System.exit(-1);
		}
	 ...
	 * Mapper class
	 */
	static class NNBenchMapper extends Configured implements
		...
		public void map(Text key, LongWritable value,
			...
			replFactor = (short) (conf.getInt("test.nnbench.replicationfactor",
					1));
			...
			output.collect(new Text("l:totalTimeAL1"), new Text(String
					.valueOf(totalTimeAL1)));
			output.collect(new Text("l:totalTimeAL2"), new Text(String
					.valueOf(totalTimeAL2)));
			output.collect(new Text("l:numOfExceptions"), new Text(String
					.valueOf(numOfExceptions)));
			output.collect(new Text("l:successfulFileOps"), new Text(String
					.valueOf(successfulFileOps)));
			output.collect(new Text("l:totalTimeTPmS"), new Text(String
					.valueOf(totalTimeTPmS)));
			output.collect(new Text("min:mapStartTimeTPmS"), new Text(String
					.valueOf(startTimeTPmS)));
			output.collect(new Text("max:mapEndTimeTPmS"), new Text(String
					.valueOf(endTimeTPms)));
		}
		...
		private void doCreateWriteOp(String name,
			...
			for (long l = 0l; l < numberOfFiles; l++) {
				Path filePath = new Path(new Path(baseDir, dataDirName), name
						+ "_" + l);

				boolean successfulOp = false;
				while (!successfulOp
						&& numOfExceptions < MAX_OPERATION_EXCEPTIONS) {
					try {
						// Set up timer for measuring AL (transaction #1)
						startTimeAL = System.currentTimeMillis();
						// Create the file
						// Use a buffer size of 512
						out = filesystem.create(filePath, true, 512,
								replFactor, blkSize);
						out.write(buffer);
		...
		private void doRenameOp(String name,
			...
			for (long l = 0l; l < numberOfFiles; l++) {
				Path filePath = new Path(new Path(baseDir, dataDirName), name
						+ "_" + l);
				Path filePathR = new Path(new Path(baseDir, dataDirName), name
						+ "_r_" + l);
	 ...
	 * Reducer class
	 */
	static class NNBenchReducer extends MapReduceBase implements
