...
class AppendTestUtil {
	...
	static {
		final long seed = RANDOM_NUMBER_GENERATOR_SEED == null ? SEED
	...
	static void write(OutputStream out, int offset, int length)
			throws IOException {
	...
	static void check(FileSystem fs, Path p, long length) throws IOException {
		...
		try {
			...
			throw new IOException("p=" + p + ", length=" + length + ", i=" + i,
					ioe);
		}
	}
}