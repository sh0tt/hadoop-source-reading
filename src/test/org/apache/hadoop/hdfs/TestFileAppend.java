...
public class TestFileAppend extends TestCase {
	...
	private void checkFile(FileSystem fileSys, Path name, int repl)
		...
		while (!done) {
			...
			if (locations.length < numBlocks) {
				System.out
						.println("Number of blocks found " + locations.length);
				done = false;
				continue;
			}
			for (int idx = 0; idx < numBlocks; idx++) {
				if (locations[idx].getHosts().length < repl) {
					System.out.println("Block index " + idx
							+ " not yet replciated.");
					done = false;
					break;
				}
	...
	public void testCopyOnWrite() throws IOException {
		...
		try {
			...
			assertTrue("There should be only one datanode but found "
					+ dn.length, dn.length == 1);
			...
			for (int i = 0; i < blocks.size(); i = i + 2) {
				...
				System.out.println("Creating hardlink for File " + f + " to "
						+ link);
			...
			for (int i = 0; i < blocks.size(); i++) {
				...
				assertTrue("Detaching block " + b
						+ " should have returned true", dataset.detachBlock(b,
						1) == true);
			}
			...
			for (int i = 0; i < blocks.size(); i++) {
				...
				System.out.println("testCopyOnWrite detaching block " + b);
				assertTrue("Detaching block " + b
						+ " should have returned false", dataset.detachBlock(b,
						1) == false);
			}
