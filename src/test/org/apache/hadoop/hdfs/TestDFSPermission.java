...
public class TestDFSPermission extends TestCase {
	...
	final private static FsPermission DEFAULT_PERMISSION = FsPermission
			.createImmutable((short) 0777);
	final static private int NUM_TEST_PERMISSIONS = conf.getInt(
			"test.dfs.permission.num", 10)
	...
	private void create(OpType op, Path name, short umask,
		...
		switch (op) {
		case CREATE:
			FSDataOutputStream out = fs.create(name, permission, true, conf
					.getInt("io.file.buffer.size", 4096), fs
					.getDefaultReplication(), fs.getDefaultBlockSize(), null);
			out.close();
			break;
		case MKDIRS:
	...
	private void testOwnership(OpType op) throws Exception {
		...
		fs = FileSystem.get(conf);
		create(op, FILE_DIR_PATH, DEFAULT_UMASK, new FsPermission(
				DEFAULT_PERMISSION));
	...
	public void testPermissionChecking() throws Exception {
		...
		try {
			...
			// inode
			PermissionGenerator ancestorPermissionGenerator = new PermissionGenerator(
					r);
			PermissionGenerator dirPermissionGenerator = new PermissionGenerator(
					r);
			PermissionGenerator filePermissionGenerator = new PermissionGenerator(
					r);
			...
			testPermissionCheckingPerUser(USER1, ancestorPermissions,
					parentPermissions, permissions, parentPaths, filePaths,
					dirPaths);
			/* group owner */
	...
	private void testPermissionCheckingPerUser(UnixUserGroupInformation ugi,
		...
		for (int i = 0; i < NUM_TEST_PERMISSIONS; i++) {
			...
			testDeleteFile(ugi, files[i], ancestorPermission[i],
					parentPermission[i]);
			testDeleteDir(ugi, dirs[i], ancestorPermission[i],
					parentPermission[i], filePermission[i], null);
		}
	...
	abstract class PermissionVerifier {
		...
		void verifyPermission(UnixUserGroupInformation ugi)
		...
		protected void logPermissions() {
			...
			LOG.info("parent permission: "
					+ Integer.toOctalString(parentPermission));
	...
	private void testOpen(UnixUserGroupInformation ugi, Path path,
		...
		openVerifier.set(path, ancestorPermission, parentPermission,
				filePermission);
	...
	private class RenamePermissionVerifier extends PermissionVerifier {
		...
		/* initialize */
		void set(Path src, short srcAncestorPermission,
				short srcParentPermission, Path dst,
				short dstAncestorPermission, short dstParentPermission) {
			super.set(src, srcAncestorPermission, srcParentPermission,
					NULL_MASK);
			this.dst = dst;
	...
	private void testRename(UnixUserGroupInformation ugi, Path src, Path dst,
			...
			short dstAncestorPermission, short dstParentPermission)
			throws Exception {
		renameVerifier.set(src, srcAncestorPermission, srcParentPermission,
				dst, dstAncestorPermission, dstParentPermission);
