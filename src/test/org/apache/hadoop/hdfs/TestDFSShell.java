...
public class TestDFSShell extends TestCase {
	...
	public void testZeroSizeFile() throws IOException {
		...
		try {
			...
			dfs
					.copyFromLocalFile(false, false, new Path(f1.getPath()),
							remotef);
	...
	public void testPut() throws IOException {
		...
		try {
			...
			final Thread copy2ndFileThread = new Thread() {
				public void run() {
					try {
						...
						dfs.copyFromLocalFile(false, false, new Path(f2
								.getPath()), dst);
			...
			final Path destmultiple2 = mkdir(dfs,
					new Path("/test/movemultiple"));
			srcs[0] = new Path(f1.getPath());
			srcs[1] = new Path(f2.getPath());
	...
	public void testErrOutPut() throws Exception {
		...
		try {
			...
			assertTrue("cat does not print exceptions ", (returned
					.lastIndexOf("Exception") == -1));
			out.reset();
			argv[0] = "-rm";
			...
			out.reset();
			assertTrue("rm prints reasonable error ", (returned
					.lastIndexOf("No such file or directory") != -1));
			argv[0] = "-rmr";
			argv[1] = root.toString();
			ret = ToolRunner.run(shell, argv);
			assertTrue(" -rmr returned -1", 0 >= ret);
			returned = out.toString();
			assertTrue("rmr prints reasonable error ", (returned
					.lastIndexOf("No such file or directory") != -1));
			out.reset();
			argv[0] = "-du";
			...
			ret = ToolRunner.run(shell, argv);
			returned = out.toString();
			assertTrue(" -du prints reasonable error ", (returned
					.lastIndexOf("No such file or directory") != -1));
			out.reset();
			argv[0] = "-dus";
			argv[1] = "/nonexistentfile";
			ret = ToolRunner.run(shell, argv);
			returned = out.toString();
			assertTrue(" -dus prints reasonable error", (returned
					.lastIndexOf("No such file or directory") != -1));
			out.reset();
			argv[0] = "-ls";
			...
			ret = ToolRunner.run(shell, argv);
			returned = out.toString();
			assertTrue(" -ls does not return Found 0 items", (returned
					.lastIndexOf("Found 0") == -1));
			out.reset();
			argv[0] = "-ls";
			argv[1] = "/nonexistentfile";
			ret = ToolRunner.run(shell, argv);
			...
			ret = ToolRunner.run(shell, argv);
			returned = out.toString();
			assertTrue(" -ls does not print out anything ", (returned
					.lastIndexOf("Found 0") == -1));
			out.reset();
			argv[0] = "-ls";
			...
			assertTrue(" -mkdir returned File exists", (returned
					.lastIndexOf("File exists") != -1));
			...
			returned = out.toString();
			assertTrue(" -mkdir returned -1", (ret < 0));
			assertTrue(" -mkdir returned this is a file ", (returned
					.lastIndexOf("not a directory") != -1));
			out.reset();
			argv = new String[3];
			argv[0] = "-mv";
			argv[1] = "/testfile";
			...
			ret = ToolRunner.run(shell, argv);
			returned = out.toString();
			assertTrue("no output from rename", (returned
					.lastIndexOf("Renamed") == -1));
			out.reset();
			argv[0] = "-mv";
			argv[1] = "/testfile";
			argv[2] = "/testfiletmp";
			ret = ToolRunner.run(shell, argv);
			returned = out.toString();
			assertTrue(" unix like output", (returned
					.lastIndexOf("No such file or") != -1));
			out.reset();
			argv = new String[1];
			argv[0] = "-du";
			...
			assertTrue("empty path specified", (returned
					.lastIndexOf("empty string") == -1));
		} finally {
			if (bak != null) {
				System.setErr(bak);
			}
	...
	public void testURIPaths() throws Exception {
		...
		try {
			...
			runCmd(shell, "-chgrp", "-R", "herbivores", dstFs.getUri()
					.toString()
					+ "/*");
			confirmOwner(null, "herbivores", dstFs, parent, path);
			runCmd(shell, "-chown", "-R", ":reptiles", dstFs.getUri()
					.toString()
					+ "/");
	...
	public void testText() throws Exception {
		...
		try {
			...
			fs.mkdirs(root);
			OutputStream zout = new GZIPOutputStream(fs.create(new Path(root,
					"file.gz")));
			Random r = new Random();
			...
			file.reset();
			out.reset();
			assertTrue("Output doesn't match input", Arrays.equals(file
	...
	public void testCount() throws Exception {
		...
		try {
			...
			assertEquals(0, new Count(new String[] { root, localstr }, 0, conf)
					.runAll());
		} finally {
			try {
				dfs.close();
	...
	void testChmod(Configuration conf, FileSystem fs, String chmodDir)
		...
		try {
			...
			assertEquals("rwxrw----", fs.getFileStatus(dir).getPermission()
					.toString());
			...
			assertEquals("rw-r--r--", fs.getFileStatus(file).getPermission()
					.toString());

			// test recursive
			runCmd(shell, "-chmod", "-R", "a+rwX", chmodDir);
			assertEquals("rwxrwxrwx", fs.getFileStatus(dir).getPermission()
					.toString());
			assertEquals("rw-rw-rw-", fs.getFileStatus(file).getPermission()
					.toString());
	...
	public void testFilePermissions() throws IOException {
		...
		testChmod(conf, fs, (new File(TEST_ROOT_DIR, "chmodTest"))
				.getAbsolutePath());
		...
		runCmd(shell, "-chown", "hdfs/hadoop-core@apache.org:asf-projects",
				file);
	...
	public void testDFSShell() throws IOException {
		...
		try {
			...
			// Verify that we can get with and without crc
			{
				...
				assertTrue("Checksum file " + checksumFile + " is copied.",
						!checksumFile.exists());
				testFile.delete();
			}
			{
				...
				assertTrue("Checksum file " + checksumFile + " not copied.",
						checksumFile.exists());
				testFile.delete();
				checksumFile.delete();
			}
	...
	public void testRemoteException() throws Exception {
		...
		try {
			...
			assertTrue("permission denied printed", str
	...
	public void testGet() throws IOException {
		...
		try {
			...
			dfs.copyFromLocalFile(false, false, new Path(localf.getPath()),
					remotef);
			...
			TestGetRunner runner = new TestGetRunner() {
				...
				public String run(int exitcode, String... options)
						throws IOException {
					...
					return exitcode == 0 ? DFSTestUtil.readFile(new File(dst))
							: null;
				}
			};
			...
			assertEquals(localfcontent.substring(1), corruptedcontent
					.substring(1));
			assertEquals(localfcontent.charAt(0) + 1, corruptedcontent
					.charAt(0));
