...
public class TestDistributedFileSystem extends junit.framework.TestCase {
	...
	public void testFileChecksum() throws IOException {
		...
		for (int n = 0; n < 5; n++) {
			...
			final byte[] data = new byte[RAN.nextInt(block_size / 2 - 1) + n
					* block_size + 1];
			RAN.nextBytes(data);
			...
			{
				final FSDataOutputStream out = hdfs.create(foo, false,
			...
			{
				final FSDataOutputStream out = hdfs.create(bar, false,
						buffer_size, (short) 2, block_size);
				out.write(data);
				out.close();
			}
