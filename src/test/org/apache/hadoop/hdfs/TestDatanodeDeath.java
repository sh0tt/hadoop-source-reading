...
public class TestDatanodeDeath extends TestCase {
	{
		...
		((Log4JLogger) InterDatanodeProtocol.LOG).getLogger().setLevel(
				Level.ALL);
	}
	...
	static class Workload extends Thread {
		...
		Workload(long myseed, FileSystem fs, int threadIndex,
				int numberOfFiles, short replication, long stamp) {
			this.myseed = myseed;
			id = threadIndex;
			this.fs = fs;
		...
		public void run() {
			...
			for (int i = 0; i < numberOfFiles; i++) {
				...
				try {
					...
					FSDataOutputStream stm = createFile(fs, filename,
							replication);
					...
					stm.close();
					checkFile(fs, filename, replication, numBlocks, fileSize,
							myseed);
				} catch (Throwable e) {
				...
				if (locations[idx].getHosts().length < repl) {
					if (attempt > 100) {
						System.out.println("File " + name + " has "
								+ locations.length + " blocks: " + " The "
								+ idx + " block has only "
								+ locations[idx].getHosts().length
								+ " replicas but is expected to have " + repl
								+ " replicas.");
					}
					done = false;
					break;
			...
			while (running) {
				...
				for (int i = 0; i < replication - 1; i++) {
					...
					try {
						...
						System.out
								.println("TestDatanodeDeath Modify exception "
										+ e);
						assertTrue("TestDatanodeDeath Modify exception " + e,
								false);
						running = false;
					}
				}
	...
	private void complexTest() throws IOException {
		...
		try {
			...
			for (int i = 0; i < numThreads; i++) {
				workload[i] = new Workload(AppendTestUtil.nextLong(), fs, i,
						numberOfFiles, replication, 0);
				workload[i].start();
			}
	...
	private void simpleTest(int datanodeToKill) throws IOException {
		...
		System.out.println("SimpleTest starting with DataNode to Kill "
				+ datanodeToKill);
		MiniDFSCluster cluster = new MiniDFSCluster(conf, myMaxNodes, true,
				null);
		cluster.waitActive();
		FileSystem fs = cluster.getFileSystem();
		short repl = 3;
		...
		try {
			...
			while (count-- > 0 && targets == null) {
				try {
					System.out
							.println("SimpleTest: Waiting for pipeline to be created.");
					Thread.sleep(1000);
				} catch (InterruptedException e) {
				}
			...
			if (targets == null) {
				...
				System.out.println("SimpleTest stopping datanode random "
						+ victim);
