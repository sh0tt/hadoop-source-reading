...
public class TestFileAppend2 extends TestCase {
	...
	private void checkFile(FileSystem fs, Path name, int len)
			throws IOException {
	...
	public void testSimpleAppend() throws IOException {
		...
		try {
			{ // test appending to a file.
				...
				System.out
						.println("Writing " + mid + " bytes to file " + file1);
			...
			{ // test append permission.
				...
				final UserGroupInformation superuser = UserGroupInformation
						.getCurrentUGI();
				String username = "testappenduser";
				String group = "testappendgroup";
				assertFalse(superuser.getUserName().equals(username));
				assertFalse(Arrays.asList(superuser.getGroupNames()).contains(
						group));
				UnixUserGroupInformation appenduser = UnixUserGroupInformation
						.createImmutable(new String[] { username, group });
		...
		public void run() {
			...
			for (int i = 0; i < numAppendsPerThread; i++) {
				...
				synchronized (testFiles) {
					if (testFiles.size() == 0) {
						System.out
								.println("Completed write to almost all files.");
						return;
					}
				...
				try {
					...
					System.out.println("Workload thread " + id + " appending "
							+ sizeToAppend + " bytes " + " to file " + testfile
							+ " of size " + len);
					...
					assertTrue("Workload exception " + id + " testfile "
							+ testfile + " expected size "
	...
	public void testComplexAppend() throws IOException {
		...
		try {
			...
			for (int i = 0; i < numberOfFiles; i++) {
				short replication = (short) (AppendTestUtil
						.nextInt(numDatanodes) + 1);
			...
			for (int i = 0; i < numThreads; i++) {
				try {
					...
					workload[i].join();
					System.out
							.println("Waiting for thread " + i + " complete.");
				} catch (InterruptedException e) {
					i--; // retry
				}
			}
