...
public class DFSTestUtil extends TestCase {
	...
	void createFiles(FileSystem fs, String topdir, short replicationFactor)
		...
		for (int idx = 0; idx < nFiles; idx++) {
			createFile(fs, new Path(root, files[idx].getName()), files[idx]
					.getSize(), replicationFactor, files[idx].getSeed());
		}
	}
	...
	public static void waitReplication(FileSystem fs, Path fileName,
		...
		do {
			good = true;
			BlockLocation locs[] = fs.getFileBlockLocations(fs
					.getFileStatus(fileName), 0, Long.MAX_VALUE);
