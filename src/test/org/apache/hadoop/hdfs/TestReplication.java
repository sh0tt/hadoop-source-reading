...
public class TestReplication extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.hdfs.TestReplication");
	...
	private void checkFile(FileSystem fileSys, Path name, int repl)
		...
		waitForBlockReplication(name.toString(), namenode, Math.min(
				numDatanodes, repl), -1);

		LocatedBlocks locations = namenode.getBlockLocations(name.toString(),
				0, Long.MAX_VALUE);
		FileStatus stat = fileSys.getFileStatus(name);
		BlockLocation[] blockLocations = fileSys.getFileBlockLocations(stat,
				0L, Long.MAX_VALUE);
	...
	public void testBadBlockReportOnTransfer() throws Exception {
		...
		dfsClient = new DFSClient(new InetSocketAddress("localhost", cluster
				.getNameNodePort()), conf);
		...
		blocks = dfsClient.namenode.getBlockLocations(file1.toString(), 0,
				Long.MAX_VALUE);
	...
	private void waitForBlockReplication(String filename,
			ClientProtocol namenode, int expected, long maxWaitSec)
			throws IOException {
		...
		while (true) {
			...
			for (Iterator<LocatedBlock> iter = blocks.getLocatedBlocks()
			...
			if (maxWaitSec > 0
				...
				throw new IOException(
						"Timedout while waiting for all blocks to "
	...
	public void testPendingReplicationRetry() throws IOException {
		...
		try {
			...
			cluster = new MiniDFSCluster(0, conf, numDataNodes, true, true,
					null, null);
			cluster.waitActive();
			DFSClient dfsClient = new DFSClient(new InetSocketAddress(
					"localhost", cluster.getNameNodePort()), conf);

			OutputStream out = cluster.getFileSystem().create(testPath);
			out.write(buffer);
			out.close();

			waitForBlockReplication(testFile, dfsClient.namenode, numDataNodes,
					-1);

			// get first block of the file.
			String block = dfsClient.namenode.getBlockLocations(testFile, 0,
					Long.MAX_VALUE).get(0).getBlock().getBlockName();
			...
			for (int i = 0; i < 6; i++) {
				...
				if (blockFile.exists()) {
					if (fileCount == 0) {
						...
						assertTrue(len > 50);
						RandomAccessFile blockOut = new RandomAccessFile(
								blockFile, "rw");
						try {
							blockOut.seek(len / 3);
			...
			LOG
					.info("Restarting minicluster after deleting a replica and corrupting 2 crcs");
			conf = new Configuration();
			conf.set("dfs.replication", Integer.toString(numDataNodes));
			conf
					.set("dfs.replication.pending.timeout.sec", Integer
							.toString(2));
			conf.set("dfs.datanode.block.write.timeout.sec", Integer
					.toString(5));
	...
	public void testReplicateLenMismatchedBlock() throws Exception {
		MiniDFSCluster cluster = new MiniDFSCluster(new Configuration(), 2,
				true, null);
		try {
			cluster.waitActive();
	...
	private void changeBlockLen(MiniDFSCluster cluster, int lenDelta)
		...
		for (int i = 0; i < cluster.getDataNodes().size(); i++) {
			if (TestDatanodeBlockScanner
					.changeReplicaLength(block, i, lenDelta)) {
				break;
			}
		}
		...
		LocatedBlocks blocks = dfsClient.namenode.getBlockLocations(fileName
				.toString(), 0, fileLen);
		if (lenDelta < 0) { // replica truncated
			while (!blocks.get(0).isCorrupt()
					|| REPLICATION_FACTOR != blocks.get(0).getLocations().length) {
				Thread.sleep(100);
				blocks = dfsClient.namenode.getBlockLocations(fileName
						.toString(), 0, fileLen);
			}
