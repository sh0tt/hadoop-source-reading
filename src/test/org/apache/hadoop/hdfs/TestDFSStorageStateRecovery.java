...
public class TestDFSStorageStateRecovery extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.hdfs.TestDFSStorageStateRecovery");
	...
	void log(String label, int numDirs, int testCaseNum, boolean[] state) {
		LOG
				.info("============================================================");
		LOG.info("***TEST " + (testCounter++) + "*** " + label + ":"
				+ " numDirs=" + numDirs + " testCase=" + testCaseNum
				+ " current=" + state[0] + " previous=" + state[1]
				+ " previous.tmp=" + state[2] + " removed.tmp=" + state[3]);
	}
	 ...
	 *         directories
	 */
	String[] createStorageState(NodeType nodeType, boolean[] state)
			throws Exception {
		String[] baseDirs = (nodeType == NAME_NODE ? conf
				.getStrings("dfs.name.dir") : conf.getStrings("dfs.data.dir"));
		...
		if (state[2]) // previous.tmp
			UpgradeUtilities.createStorageDirs(nodeType, baseDirs,
					"previous.tmp");
	...
	void checkResult(NodeType nodeType, String[] baseDirs,
		...
		case NAME_NODE:
			if (currentShouldExist) {
				for (int i = 0; i < baseDirs.length; i++) {
					...
					assertTrue(new File(baseDirs[i], "current/VERSION")
							.isFile());
					assertTrue(new File(baseDirs[i], "current/edits").isFile());
					assertTrue(new File(baseDirs[i], "current/fsimage")
							.isFile());
		...
		case DATA_NODE:
			if (currentShouldExist) {
				for (int i = 0; i < baseDirs.length; i++) {
					assertEquals(UpgradeUtilities.checksumContents(nodeType,
							new File(baseDirs[i], "current")), UpgradeUtilities
							.checksumMasterContents(nodeType));
				}
			}
			break;
		}
		if (previousShouldExist) {
			for (int i = 0; i < baseDirs.length; i++) {
				...
				assertEquals(UpgradeUtilities.checksumContents(nodeType,
						new File(baseDirs[i], "previous")), UpgradeUtilities
						.checksumMasterContents(nodeType));
			}
		}
	}
		...
		for (int numDirs = 1; numDirs <= 2; numDirs++) {
			...
			for (int i = 0; i < testCases.length; i++) {
				...
				if (shouldRecover) {
					...
					checkResult(NAME_NODE, baseDirs, curAfterRecover,
							prevAfterRecover);
					cluster.shutdown();
				} else {
					try {
				...
				createStorageState(NAME_NODE, new boolean[] { true, true,
						false, false });
				cluster = new MiniDFSCluster(conf, 0, StartupOption.REGULAR);
				...
				if (!testCase[0] && !testCase[1] && !testCase[2]
					...
					// exist
					cluster.startDataNodes(conf, 1, false,
							StartupOption.REGULAR, null);
				} else {
					if (shouldRecover) {
						cluster.startDataNodes(conf, 1, false,
								StartupOption.REGULAR, null);
						checkResult(DATA_NODE, baseDirs, curAfterRecover,
								prevAfterRecover);
					} else {
						try {
							cluster.startDataNodes(conf, 1, false,
									StartupOption.REGULAR, null);
							throw new AssertionError(
									"DataNode should have failed to start");
