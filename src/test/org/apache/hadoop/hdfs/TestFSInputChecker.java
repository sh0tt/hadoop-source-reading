...
public class TestFSInputChecker extends TestCase {
	...
	private void writeFile(FileSystem fileSys, Path name) throws IOException {
		...
		FSDataOutputStream stm = fileSys.create(name, new FsPermission(
				(short) 0777), true, fileSys.getConf().getInt(
				"io.file.buffer.size", 4096), (short) NUM_OF_DATANODES,
				BLOCK_SIZE, null);
		stm.write(expected);
		stm.close();
	}
	...
	private void checkReadAndGetPos() throws Exception {
		...
		stm.readFully(actual, 2 * BLOCK_SIZE - HALF_CHUNK_SIZE, FILE_SIZE
				- (2 * BLOCK_SIZE - HALF_CHUNK_SIZE));
		assertEquals(stm.getPos(), FILE_SIZE);
		checkAndEraseData(actual, 0, expected, "Read Sanity Test");
		...
		stm.readFully(actual, BLOCK_SIZE + HALF_CHUNK_SIZE, FILE_SIZE
				- BLOCK_SIZE - HALF_CHUNK_SIZE);
		assertEquals(stm.getPos(), FILE_SIZE);
		checkAndEraseData(actual, 0, expected, "Read Sanity Test");
	}
	...
	private void checkFileCorruption(LocalFileSystem fileSys, Path file,
		...
		// corrupt the file
		RandomAccessFile out = new RandomAccessFile(new File(fileToCorrupt
				.toString()), "rw");

		byte[] buf = new byte[(int) fileSys.getFileStatus(file).getLen()];
		int corruptFileLen = (int) fileSys.getFileStatus(fileToCorrupt)
				.getLen();
	...
	public void testFSInputChecker() throws Exception {
		...
		ChecksumFileSystem fileSys = (ChecksumFileSystem) cluster
				.getFileSystem();
		try {
	...
	private void testSeekAndRead(ChecksumFileSystem fileSys) throws IOException {
		Path file = new Path("try.dat");
		writeFile(fileSys, file);
		stm = fileSys.open(file, fileSys.getConf().getInt(
