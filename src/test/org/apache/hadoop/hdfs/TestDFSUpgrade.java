...
public class TestDFSUpgrade extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.hdfs.TestDFSUpgrade");
	...
	void checkResult(NodeType nodeType, String[] baseDirs) throws IOException {
		...
		for (int i = 0; i < baseDirs.length; i++) {
			...
			assertEquals(UpgradeUtilities.checksumContents(nodeType, new File(
					baseDirs[i], "previous")), UpgradeUtilities
					.checksumMasterContents(nodeType));
		}
	}
	...
	public void testUpgrade() throws Exception {
		...
		for (int numDirs = 1; numDirs <= 2; numDirs++) {
			...
			log(
					"DataNode upgrade with future stored layout version in current",
					numDirs);
			UpgradeUtilities.createStorageDirs(NAME_NODE, nameNodeDirs,
					"current");
			cluster = new MiniDFSCluster(conf, 0, StartupOption.UPGRADE);
			baseDirs = UpgradeUtilities.createStorageDirs(DATA_NODE,
					dataNodeDirs, "current");
			...
			log("NameNode upgrade with future layout version in current",
					numDirs);
			baseDirs = UpgradeUtilities.createStorageDirs(NAME_NODE,
					nameNodeDirs, "current");
			UpgradeUtilities.createVersionFile(NAME_NODE, baseDirs,
