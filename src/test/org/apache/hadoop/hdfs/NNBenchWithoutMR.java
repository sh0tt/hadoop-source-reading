...
public class NNBenchWithoutMR {
	...
	static int createWrite() {
		...
		for (int index = 0; index < numFiles; index++) {
			...
			do { // create file until is succeeds or max exceptions reached
				try {
					out = fileSys.create(new Path(taskDir, "" + index), false,
							512, (short) 1, bytesPerBlock);
					success = true;
	...
	static int openRead() {
		...
		for (int index = 0; index < numFiles; index++) {
			...
			try {
				...
				totalExceptions++;
				handleException("opening file #" + index, ioe,
						++singleFileExceptions);
			}
		}
		return totalExceptions;
	...
	static int rename() {
		...
		for (int index = 0; index < numFiles; index++) {
			...
			do { // rename file until is succeeds
				try {
					boolean result = fileSys.rename(new Path(taskDir, ""
							+ index), new Path(taskDir, "A" + index));
					success = true;
				} catch (IOException ioe) {
					success = false;
					totalExceptions++;
	...
	static int delete() {
		...
		for (int index = 0; index < numFiles; index++) {
			...
			do { // delete file until is succeeds
				try {
					boolean result = fileSys.delete(new Path(taskDir, "A"
							+ index), true);
					success = true;
				} catch (IOException ioe) {
					success = false;
	...
	public static void main(String[] args) throws IOException {
		...
		jobConf
				.set("io.bytes.per.checksum", Integer
						.toString(bytesPerChecksum));
		...
		if (operation == null
				|| // verify args
				baseDir == null || numFiles < 1 || blocksPerFile < 1
		...
		System.out.println("The " + operation + " job took " + duration
				+ " seconds.");
