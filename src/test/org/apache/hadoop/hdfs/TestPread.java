...
public class TestPread extends TestCase {
	...
	private void doPread(FSDataInputStream stm, long position, byte[] buffer,
		...
		int nread = 0;
		while (nread < length) {
			int nbytes = stm.read(position + nread, buffer, offset + nread,
					length - nread);
	...
	private void pReadFile(FileSystem fileSys, Path name) throws IOException {
		...
		checkAndEraseData(actual, (int) (blockSize - 2048), expected,
				"Pread Test 3");
		...
		checkAndEraseData(actual, (int) (10 * blockSize - 2048), expected,
				"Pread Test 5");
