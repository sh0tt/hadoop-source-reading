...
public class TestBalancer extends TestCase {
	...
	private void createFile(long fileLen, short replicationFactor)
		...
		DFSTestUtil.createFile(fs, filePath, fileLen, replicationFactor, r
				.nextLong());
	...
	private Block[] generateBlocks(long size, short numNodes)
		...
		try {
			...
			List<LocatedBlock> locatedBlocks = client.getBlockLocations(
					fileName, 0, fileLen).getLocatedBlocks();

			int numOfBlocks = locatedBlocks.size();
			Block[] blocks = new Block[numOfBlocks];
			for (int i = 0; i < numOfBlocks; i++) {
				Block b = locatedBlocks.get(i).getBlock();
				blocks[i] = new Block(b.getBlockId(), b.getNumBytes(), b
						.getGenerationStamp());
			}
	...
	Block[][] distributeBlocks(Block[] blocks, short replicationFactor,
		...
		List<List<Block>> blockReports = new ArrayList<List<Block>>(
				usedSpace.length);
	...
	private void testUnevenDistribution(long distribution[], long capacities[],
		...
		// redistribute blocks
		Block[][] blocksDN = distributeBlocks(blocks,
				(short) (numDatanodes - 1), distribution);

		// restart the cluster: do NOT format the cluster
		CONF.set("dfs.safemode.threshold.pct", "0.0f");
		cluster = new MiniDFSCluster(0, CONF, numDatanodes, false, true, null,
				racks, capacities);
		cluster.waitActive();
		client = DFSClient.createNamenode(CONF);
	...
	private void waitForHeartBeat(long expectedUsedSpace,
			long expectedTotalSpace) throws IOException {
		long[] status = client.getStats();
		while (status[0] != expectedTotalSpace
				|| status[1] != expectedUsedSpace) {
			try {
				Thread.sleep(100L);
	 ...
	 * @param new
	 */
	private void test(long[] capacities, String[] racks, long newCapacity,
			String newRack) throws Exception {
		int numOfDatanodes = capacities.length;
		assertEquals(numOfDatanodes, racks.length);
		cluster = new MiniDFSCluster(0, CONF, capacities.length, true, true,
	...
	public void testBalancer0() throws Exception {
		...
		test(new long[] { CAPACITY }, new String[] { RACK0 }, CAPACITY / 2,
				RACK0);
	...
	public void testBalancer1() throws Exception {
		testUnevenDistribution(new long[] { 50 * CAPACITY / 100,
				10 * CAPACITY / 100 }, new long[] { CAPACITY, CAPACITY },
