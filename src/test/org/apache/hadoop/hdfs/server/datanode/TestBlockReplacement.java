...
public class TestBlockReplacement extends TestCase {
	private static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.hdfs.TestBlockReplacement");
	...
	public void testThrottler() throws IOException {
		...
		long start = Util.now();
		BlockTransferThrottler throttler = new BlockTransferThrottler(
				bandwidthPerSec);
		long totalBytes = 0L;
	...
	public void testBlockReplacement() throws IOException {
		...
		cluster = new MiniDFSCluster(CONF, REPLICATION_FACTOR, true,
				INITIAL_RACKS);
		try {
			cluster.waitActive();
			...
			DFSTestUtil.createFile(fs, fileName, DEFAULT_BLOCK_SIZE,
					REPLICATION_FACTOR, r.nextLong());
			...
			DatanodeInfo[] datanodes = client
					.datanodeReport(DatanodeReportType.ALL);
			...
			LOG.info("Testcase 3: Proxy=" + source.getName() + " source="
					+ proxies.get(0).getName() + " destination="
					+ newNode.getName());
			assertTrue(replaceBlock(b, source, proxies.get(0), newNode));
			// block locations should contain two proxies and newNode
			checkBlocks(new DatanodeInfo[] { newNode, proxies.get(0),
					proxies.get(1) }, fileName.toString(), DEFAULT_BLOCK_SIZE,
					REPLICATION_FACTOR, client);
			// case 4: proxies.get(0) is not a valid del hint
			LOG
					.info("Testcase 4: invalid del hint "
							+ proxies.get(0).getName());
			...
			checkBlocks(proxies.toArray(new DatanodeInfo[proxies.size()]),
					fileName.toString(), DEFAULT_BLOCK_SIZE,
					REPLICATION_FACTOR, client);
		} finally {
			cluster.shutdown();
		}
	...
	private void checkBlocks(DatanodeInfo[] includeNodes, String fileName,
		...
		do {
			...
			List<LocatedBlock> blocks = client.namenode.getBlockLocations(
			...
			if (notDone) {
				LOG
						.info("Expected replication factor is " + replFactor
								+ " but the real replication factor is "
								+ nodes.length);
			} else {
	...
	private boolean replaceBlock(Block block, DatanodeInfo source,
		...
		sock.connect(NetUtils.createSocketAddr(destination.getName()),
