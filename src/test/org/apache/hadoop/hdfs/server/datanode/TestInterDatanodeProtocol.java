	...
	public void testBlockMetaDataInfo() throws Exception {
		...
		try {
			...
			LocatedBlock locatedblock = getLastLocatedBlock(
					dfs.getClient().namenode, filestr);
			DatanodeInfo[] datanodeinfo = locatedblock.getLocations();
			assertTrue(datanodeinfo.length > 0);

			// connect to a data node
			InterDatanodeProtocol idp = DataNode
					.createInterDataNodeProtocolProxy(datanodeinfo[0], conf);
			DataNode datanode = cluster.getDataNode(datanodeinfo[0]
					.getIpcPort());
			...
			// verify updateBlock
			Block newblock = new Block(b.getBlockId(), b.getNumBytes() / 2, b
