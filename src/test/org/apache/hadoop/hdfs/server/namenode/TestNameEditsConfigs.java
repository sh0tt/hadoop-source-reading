...
public class TestNameEditsConfigs extends TestCase {
	...
	void checkImageAndEditsFilesExistence(File dir, boolean imageMustExist,
	...
	public void testNameEditsConfigs() throws IOException {
		...
		File checkpointNameAndEdits = new File(base_dir,
				"second_name_and_edits");
		...
		cluster = new MiniDFSCluster(0, conf, NUM_DATA_NODES, true, false,
		...
		new File(nameAndEdits, FILE_EDITS).renameTo(new File(newNameDir,
				FILE_EDITS));
		new File(nameAndEdits, FILE_IMAGE).renameTo(new File(newEditsDir,
				FILE_IMAGE));
		new File(checkpointNameAndEdits, FILE_EDITS).renameTo(new File(
				checkpointNameDir, FILE_EDITS));
		new File(checkpointNameAndEdits, FILE_IMAGE).renameTo(new File(
				checkpointEditsDir, FILE_IMAGE));
		conf = new Configuration();
		...
		assertTrue(FileUtil.fullyDelete(new File(checkpointNameAndEdits,
				"current")));
		conf = new Configuration();
		conf.set("dfs.name.dir", nameAndEdits.getPath() + ","
				+ newNameDir.getPath());
	...
	public void testNameEditsConfigsFailure() throws IOException {
		...
		try {
			...
			System.out
					.println("cluster start failed due to missing latest name dir");
		} finally {
			cluster = null;
		}
	}
