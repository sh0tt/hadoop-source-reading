...
public class TestCheckpoint extends TestCase {
	...
	private void testSecondaryNamenodeError1(Configuration conf)
		...
		for (Iterator<StorageDirectory> it = image
				.dirIterator(NameNodeDirType.IMAGE); it.hasNext();) {
			StorageDirectory sd = it.next();
			assertFalse(FSImage.getImageFile(sd, NameNodeFile.IMAGE_NEW)
					.exists());
		}
		for (Iterator<StorageDirectory> it = image
	...
	void testSecondaryFailsToReturnImage(Configuration conf) throws IOException {
		...
		try {
			...
			for (Iterator<StorageDirectory> it = image
				...
				sd = it.next();
			assertTrue(sd != null);
			long fsimageLength = FSImage.getImageFile(sd, NameNodeFile.IMAGE)
					.length();
			//
			// Make the checkpoint
			//
			...
			try {
				...
				System.out
						.println("testSecondaryFailsToReturnImage: doCheckpoint() "
			...
			for (Iterator<StorageDirectory> it = image
					.dirIterator(NameNodeDirType.IMAGE); it.hasNext();) {
				assertTrue(FSImage.getImageFile(it.next(), NameNodeFile.IMAGE)
						.length() == fsimageLength);
			}
	...
	void testStartup(Configuration conf) throws IOException {
		System.out
				.println("Startup of the name-node in the checkpoint directory.");
		...
		System.out
				.println("Startup of secondary in the same dir as the primary.");
		...
		System.out
				.println("Startup of primary in the same dir as the secondary.");
		...
		for (File dir : secondaryDirs) {
			Storage.rename(new File(dir, "current"), new File(dir,
					"lastcheckpoint.tmp"));
		}
		secondary = startSecondaryNameNode(conf);
		secondary.shutdown();
		...
		for (File dir : secondaryDirs) {
			Storage.rename(new File(dir, "previous.checkpoint"), new File(dir,
					"lastcheckpoint.tmp"));
		}
		secondary = startSecondaryNameNode(conf);
		secondary.shutdown();
	...
	NameNode startNameNode(Configuration conf, String imageDirs,
			String editsDirs, StartupOption start) throws IOException {
	...
	public void testSaveNamespace() throws IOException {
		...
		try {
			...
			try {
				...
				assertTrue(eIO.getLocalizedMessage().contains(
						"Safe mode should be turned ON"));
			} catch (Exception e) {
			...
			for (File ed : editsDirs) {
				assertTrue(new File(ed, "current/edits").length() > Integer.SIZE
						/ Byte.SIZE);
			}
