...
public class TestStartup extends TestCase {
	...
	private static final Log LOG = LogFactory.getLog(TestStartup.class
			.getName());
	...
	protected void setUp() throws Exception {
		...
		config.set("fs.checkpoint.dir", new File(hdfsDir, "secondary")
				.getPath());
	...
	public void createCheckPoint() throws IOException {
		...
		try {
			cluster = new MiniDFSCluster(0, config, 1, true, false, false,
					null, null, null, null);
			cluster.waitActive();
	...
	private void corruptNameNodeFiles() throws IOException {
		...
		List<File> nameDirs = (List<File>) FSNamesystem
				.getNamespaceDirs(config);
		List<File> nameEditsDirs = (List<File>) FSNamesystem
				.getNamespaceEditsDirs(config);
	...
	public void testChkpointStartup2() throws IOException {
		LOG
				.info("--starting checkpointStartup2 - same directory for checkpoint");
		// different name dirs
		config.set("dfs.name.dir", new File(hdfsDir, "name").getPath());
		config.set("dfs.name.edits.dir", new File(hdfsDir, "edits").getPath());
		// same checkpoint dirs
		config.set("fs.checkpoint.edits.dir", new File(hdfsDir, "chkpt")
				.getPath());
	...
	public void testChkpointStartup1() throws IOException {
		...
		config.set("dfs.name.edits.dir", new File(hdfsDir, "edits").getPath());
		// same checkpoint dirs
		config.set("fs.checkpoint.edits.dir", new File(hdfsDir, "chkpt_edits")
				.getPath());
		config.set("fs.checkpoint.dir", new File(hdfsDir, "chkpt").getPath());
	...
	/**
	 * secnn-7 secondary node copies fsimage and edits into correct separate
	 * directories.
	 * 
	 * @throws IOException
	 */
	public void testSNNStartup() throws IOException {
		...
		try {
			...
			assertEquals(sd.getStorageDirType(),
