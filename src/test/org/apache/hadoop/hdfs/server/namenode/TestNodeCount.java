...
public class TestNodeCount extends TestCase {
	public void testNodeCount() throws Exception {
		...
		final MiniDFSCluster cluster = new MiniDFSCluster(conf,
				REPLICATION_FACTOR, true, null);
		try {
			...
			DataNodeProperties dnprop = cluster
					.stopDataNode(datanode.getName());
			...
			Iterator<DatanodeDescriptor> iter = namesystem.blocksMap
					.nodeIterator(block);
			DatanodeDescriptor nonExcessDN = null;
			while (iter.hasNext()) {
				DatanodeDescriptor dn = iter.next();
				Collection<Block> blocks = namesystem.excessReplicateMap.get(dn
						.getStorageID());
