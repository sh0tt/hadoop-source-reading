...
public class NNThroughputBenchmark {
		...
		void benchmark() throws IOException {
			...
			try {
				...
				for (tIdx = 0; tIdx < numThreads; tIdx++)
					daemons
							.add(new StatsDaemon(tIdx, opsPerThread[tIdx], this));
	...
	class CreateFileStats extends OperationStatsBase {
		...
		long executeOp(int daemonId, int inputIdx, String clientName)
			...
			nameNode.create(fileNames[daemonId][inputIdx], FsPermission
					.getDefault(), clientName, true, replication, BLOCK_SIZE);
			long end = System.currentTimeMillis();
	...
	class OpenFileStats extends CreateFileStats {
		...
		void generateInputs(int[] opsPerThread) throws IOException {
			...
			String[] createArgs = new String[] { "-op", "create", "-threads",
					...
					"-close" };
			CreateFileStats opCreate = new CreateFileStats(Arrays
					.asList(createArgs));
			...
			if (cmds != null) {
				for (DatanodeCommand cmd : cmds) {
					if (cmd.getAction() == DatanodeProtocol.DNA_TRANSFER) {
						...
						return transferBlocks(bcmd.getBlocks(), bcmd
								.getTargets());
					}
				}
			}
	...
	class BlockReportStats extends OperationStatsBase {
		...
		private void addBlocks(String fileName, String clientName)
			...
			for (int jdx = 0; jdx < blocksPerFile; jdx++) {
				...
				for (DatanodeInfo dnInfo : loc.getLocations()) {
					...
					nameNode
							.blockReceived(datanodes[dnIdx].dnRegistration,
		...
		long executeOp(int daemonId, int inputIdx, String ignore)
			...
			long start = System.currentTimeMillis();
			nameNode.blockReport(dn.dnRegistration, BlockListAsLongs
					.convertToArrayLongs(dn.blocks));
			long end = System.currentTimeMillis();
			return end - start;
		}
	...
	class ReplicationStats extends OperationStatsBase {
		...
		ReplicationStats(List<String> args) {
			...
			blockReportObject = new BlockReportStats(Arrays
	...
	public static void main(String[] args) throws Exception {
		runBenchmark(new Configuration(), new ArrayList<String>(Arrays
				.asList(args)));
	}
}
