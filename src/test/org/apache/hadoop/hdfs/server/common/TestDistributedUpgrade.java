...
public class TestDistributedUpgrade extends TestCase {
	private static final Log LOG = LogFactory
			.getLog(TestDistributedUpgrade.class);
	...
	void startNameNodeShouldFail(StartupOption operation) {
		try {
			...
			cluster = new MiniDFSCluster(0, conf, 0, false, true, operation,
					null); // Should fail
   ...
   */
	public void testDistributedUpgrade() throws Exception {
		...
		log(
				"NameNode start in regular mode when dustributed upgrade is required",
				numDirs);
		...
		log(
				"NameNode start in regular mode when dustributed upgrade has been started",
				numDirs);
		startNameNodeShouldFail(StartupOption.REGULAR);

		log(
				"NameNode rollback to the old version that require a dustributed upgrade",
				numDirs);
...
class UO_Datanode extends UpgradeObjectDatanode {
	...
	public void doUpgrade() throws IOException {
		...
		getDatanode().namenode.processUpgradeCommand(new UpgradeCommand(
				UpgradeCommand.UC_ACTION_REPORT_STATUS, getVersion(),
				getUpgradeStatus()));
	}
