...
public class TestQuota extends TestCase {
	...
	public void testQuotaCommands() throws Exception {
		...
		try {
			...
			assertEquals(2L << 40, dfs.getContentSummary(parent)
					.getSpaceQuota());

			// set diskspace quota to 10000
			runCommand(admin, false, "-setSpaceQuota", Long
					.toString(spaceQuota), parent.toString());
			...
			// 8: clear quota /test
			runCommand(admin, new String[] { "-clrQuota", parent.toString() },
					false);
			c = dfs.getContentSummary(parent);
			assertEquals(c.getQuota(), -1);
			assertEquals(c.getSpaceQuota(), spaceQuota);
			...
			runCommand(admin,
					new String[] { "-clrQuota", childDir0.toString() }, false);
			c = dfs.getContentSummary(childDir0);
			assertEquals(c.getQuota(), -1);
	...
	public void testNamespaceCommands() throws Exception {
		...
		try {
			...
			try {
				assertFalse(dfs
						.rename(tempPath, new Path(quotaDir3, "nqdir32")));
			} catch (NSQuotaExceededException e) {
				hasException = true;
			}
			assertTrue(hasException);
	...
	public void testSpaceCommands() throws Exception {
		...
		try {
			...
			DFSTestUtil.createFile(dfs, new Path(tempPath, "fileDir/file1"),
					fileLen, replication, 0);
			c = dfs.getContentSummary(quotaDir21);
			...
			boolean hasException = false;
			try {
				DFSTestUtil.createFile(dfs, new Path(quotaDir21,
						"nqdir33/file2"), 2 * fileLen, replication, 0);
			} catch (DSQuotaExceededException e) {
				hasException = true;
			}
			assertTrue(hasException);
			...
			dfs
					.setQuota(quotaDir20, FSConstants.QUOTA_DONT_SET,
							10 * fileSpace);
