...
public class TestHDFSServerPorts extends TestCase {
	 ...
	 */
	private boolean canStartSecondaryNode(Configuration conf)
			throws IOException {
	...
	public void testDataNodePorts() throws Exception {
		...
		try {
			...
			conf2.set("dfs.datanode.address", FileSystem.getDefaultUri(config)
					.getAuthority());
			...
			conf2.set("dfs.datanode.http.address", config
	...
	public void testSecondaryNodePorts() throws Exception {
		...
		try {
			...
			// bind http server to the same port as name-node
			Configuration conf2 = new Configuration(config);
			conf2.set("dfs.secondary.http.address", config
					.get("dfs.http.address"));
