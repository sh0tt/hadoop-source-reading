...
public class TestFileAppend3 extends junit.framework.TestCase {
	...
	public void testTC7() throws Exception {
		...
		{
			FSDataOutputStream out = fs.create(p, false, buffersize, repl,
					BLOCK_SIZE);
			AppendTestUtil.write(out, 0, len1);
			out.close();
		}
		...
		final LocatedBlocks locatedblocks = fs.dfs.namenode.getBlockLocations(p
				.toString(), 0L, len1);
		...
		final RandomAccessFile raf = new RandomAccessFile(data
				.getBlockFile(blk), "rw");
	...
	public void testTC11() throws Exception {
		...
		final LocatedBlocks locatedblocks = fs.dfs.namenode.getBlockLocations(
				pnew.toString(), 0L, len);
		...
		for (int i = 0; i < numblock; i++) {
			...
			for (DatanodeInfo datanodeinfo : lb.getLocations()) {
				final DataNode dn = cluster.getDataNode(datanodeinfo
						.getIpcPort());
