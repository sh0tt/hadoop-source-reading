...
public class TestIPC extends TestCase {
		...
		public void run() {
			for (int i = 0; i < count; i++) {
				try {
					...
					Writable[] values = client.call(params, addresses, null,
							null);
					for (int j = 0; j < addresses.length; j++) {
	...
	public void testSerial(int handlerCount, boolean handlerSleep,
		...
		for (int i = 0; i < callerCount; i++) {
			callers[i] = new SerialCaller(clients[i % clientCount], addr,
					callCount);
			callers[i].start();
		}
		for (int i = 0; i < callerCount; i++) {
	...
	public void testParallel(int handlerCount, boolean handlerSleep,
			int serverCount, int addressCount, int clientCount,
			int callerCount, int callCount) throws Exception {
		...
		for (int i = 0; i < callerCount; i++) {
			callers[i] = new ParallelCaller(clients[i % clientCount],
					addresses, callCount);
			callers[i].start();
		}
		for (int i = 0; i < callerCount; i++) {
	...
	public void testStandAloneClient() throws Exception {
		...
		try {
			client.call(new LongWritable(RANDOM.nextLong()), address, null,
					null);
	...
	public void testErrorClient() throws Exception {
		...
		try {
			client.call(new LongErrorWritable(RANDOM.nextLong()), addr, null,
					null);
			fail("Expected an exception to have been thrown");
		} catch (IOException e) {
			// check error
