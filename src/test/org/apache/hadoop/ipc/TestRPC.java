...
public class TestRPC extends TestCase {
	...
	public void testSlowRpc() throws Exception {
		...
		Server server = RPC.getServer(new TestImpl(), ADDRESS, 0, 2, false,
				conf);
	...
	public void testCalls() throws Exception {
		...
		try {
			...
			assertTrue(Arrays.equals(stringResults,
					new String[] { "foo", "bar" }));
			...
			String[] strings = (String[]) RPC.call(echo, new String[][] {
					{ "a" }, { "b" } }, new InetSocketAddress[] { addr, addr },
					conf);
			assertTrue(Arrays.equals(strings, new String[] { "a", "b" }));

			Method ping = TestProtocol.class.getMethod("ping", new Class[] {});
			Object[] voids = (Object[]) RPC.call(ping,
					new Object[][] { {}, {} }, new InetSocketAddress[] { addr,
							addr }, conf);
	...
	public void testStandaloneClient() throws IOException {
		try {
			RPC.waitForProxy(TestProtocol.class, TestProtocol.versionID,
	...
	private void doRPCs(Configuration conf, boolean expectFailure)
			throws Exception {
		SecurityUtil.setPolicy(new ConfiguredPolicy(conf,
				new TestPolicyProvider()));

		Server server = RPC
				.getServer(new TestImpl(), ADDRESS, 0, 5, true, conf);
