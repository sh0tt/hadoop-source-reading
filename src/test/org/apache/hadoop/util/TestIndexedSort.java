...
public class TestIndexedSort extends TestCase {
	...
	public void sortWritable(IndexedSorter sorter) throws Exception {
		...
		assertTrue("seed: " + seed + "\ndoesn't match", Arrays.equals(values,
				check));
	}
	...
	public void testQuickSort() throws Exception {
		...
		System.out.println("QuickSort degen cmp/swp: " + m.getCmp() + "/"
				+ m.getSwp() + "(" + sorter.getClass().getName() + ")");
		Arrays.sort(values);
		int[] check = s.getSorted();
	...
	private static class SampleSortable implements IndexedSortable {
		...
		public int compare(int i, int j) {
			// assume positive
			return values[valindirect[valindex[i]]]
					- values[valindirect[valindex[j]]];
		}
	...
	private static class WritableSortable implements IndexedSortable {
		...
		public int compare(int i, int j) {
			...
			return comparator.compare(bytes, offsets[ii],
					((ii + 1 == indices.length) ? eob : offsets[ii + 1])
							- offsets[ii], bytes, offsets[ij],
					((ij + 1 == indices.length) ? eob : offsets[ij + 1])
							- offsets[ij]);
		}
		...
		public String[] getSorted() throws IOException {
			...
			for (int i = 0; i < ret.length; ++i) {
				int ii = indices[i];
				dib.reset(bytes, offsets[ii], ((ii + 1 == indices.length) ? eob
						: offsets[ii + 1])
						- offsets[ii]);
				t.readFields(dib);
				ret[i] = t.toString();
