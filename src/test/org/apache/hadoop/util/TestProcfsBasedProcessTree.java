...
public class TestProcfsBasedProcessTree extends TestCase {
	...
	public void testProcessTree() {
		...
		File tempFile = new File(this.getName() + "_shellScript_"
				+ rm.nextInt() + ".sh");
		tempFile.deleteOnExit();
		shellScript = tempFile.getName();

		// create pid file
		tempFile = new File(this.getName() + "_pidFile_" + rm.nextInt()
				+ ".pid");
		tempFile.deleteOnExit();
	 ...
	 * files. Verifies that the virtual memory is correctly computed.
	 * 
	 * @throws IOException
	...
	public void testVirtualMemoryForProcessTree() throws IOException {
		...
		try {
			...
			assertEquals("Cumulative memory does not match", Long
					.parseLong("600000"), processTree.getCumulativeVmem());
		} finally {
	...
	public void testVMemForOlderProcesses() throws IOException {
		...
		try {
			...
			// verify cumulative memory
			assertEquals("Cumulative memory does not match", Long
					.parseLong("700000"), processTree.getCumulativeVmem());
			...
			newProcInfos[0] = new ProcessStatInfo(new String[] { "500",
					"proc5", "100", "100", "100", "500000" });
			writeStatFiles(procfsRootDir, newPids, newProcInfos);

			// check vmem includes the new process.
			processTree.getProcessTree();
			assertEquals("Cumulative memory does not include new process", Long
					.parseLong("1200000"), processTree.getCumulativeVmem());

			// however processes older than 1 iteration will retain the older
			// value
			assertEquals(
					"Cumulative memory shouldn't have included new process",
			...
			assertEquals(
					"Cumulative memory shouldn't have included new processes",
