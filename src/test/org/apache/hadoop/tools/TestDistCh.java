...
public class TestDistCh extends junit.framework.TestCase {
	...
	static {
		final long seed = RANDOM_NUMBER_GENERATOR_SEED == null ? RANDOM
				.nextLong() : RANDOM_NUMBER_GENERATOR_SEED;
	...
	static class ChPermissionStatus extends PermissionStatus {
		...
		ChPermissionStatus(FileStatus filestatus, String owner, String group,
				String permission) {
			super("".equals(owner) ? filestatus.getOwner() : owner, ""
					.equals(group) ? filestatus.getGroup() : group, ""
					.equals(permission) ? filestatus.getPermission()
					: new FsPermission(Short.parseShort(permission, 8)));
		}
	}
	...
	public void testDistCh() throws Exception {
		...
		try {
			...
			for (int i = 0; i < args.length; i++) {
				final int index = indices
						.remove(RANDOM.nextInt(indices.size()));
				final String sub = "sub" + index;
				final boolean changeOwner = RANDOM.nextBoolean();
				final boolean changeGroup = RANDOM.nextBoolean();
				final boolean changeMode = !changeOwner && !changeGroup ? true
						: RANDOM.nextBoolean();

				final String owner = changeOwner ? sub : "";
				final String group = changeGroup ? sub : "";
				final String permission = changeMode ? RANDOM.nextInt(8) + ""
						+ RANDOM.nextInt(8) + "" + RANDOM.nextInt(8) : "";

				args[i] = tree.root + "/" + sub + ":" + owner + ":" + group
						+ ":" + permission;
				newstatus[index] = new ChPermissionStatus(rootstatus, owner,
						group, permission);
			}
			for (int i = 0; i < NUN_SUBS; i++) {
				if (newstatus[i] == null) {
			...
			System.out
					.println("newstatus="
							+ Arrays.asList(newstatus).toString().replace(",",
									",\n  "));
