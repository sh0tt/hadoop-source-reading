...
public class DFSCIOTest extends TestCase {
	...
	private static String HDFS_LIB_VERSION = System.getProperty(
			"libhdfs.version", "1");
	private static String CHMOD = new String("chmod");
	private static Path HDFS_SHLIB = new Path(HDFS_TEST_DIR + "/libhdfs.so."
			+ HDFS_LIB_VERSION);
	...
	private static void createControlFile(FileSystem fs, int fileSize, // in MB
		...
		LOG.info("creating control file: " + fileSize + " mega bytes, "
				+ nrFiles + " files");
	...
	private abstract static class IOStatMapper extends IOMapperBase {
		...
		void collectStats(OutputCollector<UTF8, UTF8> output, String name,
				long execTime, Object objSize) throws IOException {
			...
			output.collect(new UTF8("l:size"), new UTF8(String
					.valueOf(totalSize)));
			output.collect(new UTF8("l:time"), new UTF8(String
					.valueOf(execTime)));
			output.collect(new UTF8("f:rate"), new UTF8(String
					.valueOf(ioRateMbSec * 1000)));
			output.collect(new UTF8("f:sqrate"), new UTF8(String
					.valueOf(ioRateMbSec * ioRateMbSec * 1000)));
		}
	}
	...
	public static class WriteMapper extends IOStatMapper {
		...
		public Object doIO(Reporter reporter, String name, long totalSize)
				throws IOException {
			// create file
			totalSize *= MEGA;
			...
			try {
				...
				synchronized (this) {
					...
					if (!(localFS.mkdirs(HDFS_TEST_DIR))) {
						throw new IOException("Failed to create "
								+ HDFS_TEST_DIR + " on local filesystem");
					}
				}

				synchronized (this) {
					if (!localFS.exists(HDFS_SHLIB)) {
						FileUtil.copy(fs, HDFS_SHLIB, localFS, HDFS_SHLIB,
								false, fsConfig);

						String chmodCmd = new String(CHMOD + " a+x "
								+ HDFS_SHLIB);
						Process process = runTime.exec(chmodCmd);
						int exitStatus = process.waitFor();
						if (exitStatus != 0) {
							throw new IOException(chmodCmd
									+ ": Failed with exitStatus: " + exitStatus);
						}
					}
				}
				...
				synchronized (this) {
					if (!localFS.exists(HDFS_WRITE)) {
						FileUtil.copy(fs, HDFS_WRITE, localFS, HDFS_WRITE,
								false, fsConfig);
				...
				String writeCmd = new String(HDFS_WRITE + " " + outFile + " "
						+ totalSize + " " + bufferSize);
				Process process = runTime.exec(writeCmd, null, new File(
						HDFS_TEST_DIR.toString()));
				int exitStatus = process.waitFor();
				if (exitStatus != 0) {
					throw new IOException(writeCmd
							+ ": Failed with exitStatus: " + exitStatus);
				}
	...
	public static class ReadMapper extends IOStatMapper {
		...
		public Object doIO(Reporter reporter, String name, long totalSize)
			...
			try {
				...
				synchronized (this) {
					if (!localFS.exists(HDFS_SHLIB)) {
						if (!FileUtil.copy(fs, HDFS_SHLIB, localFS, HDFS_SHLIB,
								false, fsConfig)) {
							throw new IOException("Failed to copy "
									+ HDFS_SHLIB + " to local filesystem");
						}
				...
				synchronized (this) {
					if (!localFS.exists(HDFS_READ)) {
						if (!FileUtil.copy(fs, HDFS_READ, localFS, HDFS_READ,
								false, fsConfig)) {
							throw new IOException("Failed to copy " + HDFS_READ
									+ " to local filesystem");
						}
				...
				String readCmd = new String(HDFS_READ + " " + inFile + " "
						+ totalSize + " " + bufferSize);
				Process process = runTime.exec(readCmd, null, new File(
						HDFS_TEST_DIR.toString()));
				int exitStatus = process.waitFor();

				if (exitStatus != 0) {
					throw new IOException(HDFS_READ
							+ ": Failed with exitStatus: " + exitStatus);
				}
			} catch (InterruptedException interruptedException) {
				reporter.setStatus(interruptedException.toString());
	...
	private static void sequentialTest(FileSystem fs, int testType,
			int fileSize, int nrFiles) throws Exception {
		...
		if (testType == TEST_TYPE_READ)
			...
			return;
		for (int i = 0; i < nrFiles; i++)
			ioer.doIO(Reporter.NULL, BASE_FILE_NAME + Integer.toString(i), MEGA
					* fileSize);
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (testType != TEST_TYPE_CLEANUP) {
				...
				fs.copyFromLocalFile(new Path(hadoopHome
						+ "/libhdfs/libhdfs.so." + HDFS_LIB_VERSION),
						HDFS_SHLIB);
				fs.copyFromLocalFile(
						new Path(hadoopHome + "/libhdfs/hdfs_read"), HDFS_READ);
				fs.copyFromLocalFile(new Path(hadoopHome
						+ "/libhdfs/hdfs_write"), HDFS_WRITE);
			}
			...
			if (isSequential) {
				...
				String resultLine = "Seq Test exec time sec: "
						+ (float) execTime / 1000;
				LOG.info(resultLine);
				return;
			}
	...
	private static void analyzeResult(FileSystem fs, int testType,
		...
		PrintStream res = new PrintStream(new FileOutputStream(new File(
				resFileName), true));
