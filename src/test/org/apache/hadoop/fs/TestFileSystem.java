...
public class TestFileSystem extends TestCase {
	...
	public static void testCommandFormat() throws Exception {
		...
		assertEquals(cf.parse(new String[] { "-get", "file", "-" }, 1).get(1),
				"-");
		assertEquals(cf.parse(
				new String[] { "-get", "file", "-ignoreCrc", "/foo" }, 1)
				.get(1), "/foo");
		cf = new CommandFormat("tail", 1, 1, "f");
		assertEquals(cf.parse(new String[] { "-tail", "fileName" }, 1).get(0),
				"fileName");
		assertEquals(cf.parse(new String[] { "-tail", "-f", "fileName" }, 1)
				.get(0), "fileName");
		cf = new CommandFormat("setrep", 2, 2, "R", "w");
		assertEquals(cf.parse(
				new String[] { "-setrep", "-R", "2", "/foo/bar" }, 1).get(1),
				"/foo/bar");
		cf = new CommandFormat("put", 2, 10000);
		assertEquals(cf.parse(new String[] { "-put", "-", "dest" }, 1).get(1),
				"dest");
	}

	public static void createControlFile(FileSystem fs, long megaBytes,
			int numFiles, long seed) throws Exception {

		LOG.info("creating control file: " + megaBytes + " bytes, " + numFiles
				+ " files");
		...
		SequenceFile.Writer writer = SequenceFile.createWriter(fs, conf,
				controlFile, UTF8.class, LongWritable.class,
	...
	public static class WriteMapper extends Configured implements
			Mapper<UTF8, LongWritable, UTF8, LongWritable> {
		...
		public void map(UTF8 key, LongWritable value,
				OutputCollector<UTF8, LongWritable> collector, Reporter reporter)
				throws IOException {
			...
			try {
				while (written < size) {
					if (fastCheck) {
						Arrays.fill(buffer, (byte) random
					...
					written += length;
					reporter.setStatus("writing " + name + "@" + written + "/"
							+ size);
				}
			} finally {
				out.close();
	...
	public static class ReadMapper extends Configured implements
			Mapper<UTF8, LongWritable, UTF8, LongWritable> {
		...
		public void map(UTF8 key, LongWritable value,
			...
			DataInputStream in = new DataInputStream(fs.open(new Path(DATA_DIR,
					name)));
			...
			try {
				while (read < size) {
					...
					int n = (remains <= buffer.length) ? (int) remains
							: buffer.length;
					in.readFully(buffer, 0, n);
					read += n;
					if (fastCheck) {
						Arrays.fill(check, (byte) random
								.nextInt(Byte.MAX_VALUE));
					} else {
					...
					reporter.setStatus("reading " + name + "@" + read + "/"
							+ size);
	...
	public static class SeekMapper<K> extends Configured implements
		...
		public void map(WritableComparable key, LongWritable value,
				OutputCollector<K, LongWritable> collector, Reporter reporter)
				throws IOException {
			String name = key.toString();
			long size = value.get();
			long seed = Long.parseLong(name);
	...
	static Configuration createConf4Testing(String username) throws Exception {
		...
		UnixUserGroupInformation
				.saveToConf(conf, UnixUserGroupInformation.UGI_PROPERTY_NAME,
	...
	static void runTestCache(int port) throws Exception {
		...
		try {
			...
			if (port == NameNode.DEFAULT_PORT) {
				...
				URI uri2 = new URI(uri.getScheme(), uri.getUserInfo(), uri
						.getHost(), NameNode.DEFAULT_PORT, uri.getPath(), uri
	...
	static void checkPath(MiniDFSCluster cluster, FileSystem fileSys)
			throws IOException {
	...
	public void testFsClose() throws Exception {
		...
		{
			Configuration conf = new Configuration();
			FileSystem fs = new Path("hftp://localhost:12345/")
					.getFileSystem(conf);
	...
	public void testCacheKeysAreCaseInsensitive() throws Exception {
		...
		FileSystem.Cache.Key lowercaseCachekey1 = new FileSystem.Cache.Key(
				new URI("hftp://localhost:12345/"), conf);
		FileSystem.Cache.Key lowercaseCachekey2 = new FileSystem.Cache.Key(
				new URI("hftp://localhost:12345/"), conf);
		assertEquals(lowercaseCachekey1, lowercaseCachekey2);

		// check insensitive equality
		FileSystem.Cache.Key uppercaseCachekey = new FileSystem.Cache.Key(
				new URI("HFTP://Localhost:12345/"), conf);
