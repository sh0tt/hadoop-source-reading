...
public class DistributedFSCheck extends TestCase {
	...
	private static Path TEST_ROOT_DIR = new Path(System.getProperty(
			"test.build.data", "/benchmarks/DistributedFSCheck"));
	...
	private void createInputFile(String rootName) throws IOException {
		...
		SequenceFile.Writer writer = SequenceFile
				.createWriter(fs, fsConfig, inputFile, UTF8.class,
	...
	private void listSubtree(Path rootFile, SequenceFile.Writer writer)
		...
		if (!fs.isDirectory(rootFile)) {
			...
			for (long offset = 0; offset < fileLength; offset += blockSize)
				writer.append(new UTF8(rootFile.toString()), new LongWritable(
						offset));
			return;
		}
	...
	public static class DistributedFSCheckMapper extends IOMapperBase {
		...
		public Object doIO(Reporter reporter, String name, long offset)
				throws IOException {
			// open file
			...
			try {
				...
				for (int curSize = bufferSize; curSize == bufferSize
						&& actualSize < blockSize; actualSize += curSize) {
					curSize = in.read(buffer, 0, bufferSize);
				}
			} catch (IOException e) {
				LOG.info("Corrupted block detected in \"" + name + "\" at "
						+ offset);
				return name + "@" + offset;
			} finally {
				in.close();
	...
	private void analyzeResult(long execTime, String resFileName,
			boolean viewStats) throws IOException {
		...
		resultLines.insertElementAt(
				" Number of corrupted files: " + nrBadFiles, nrBadFilesPos);

		if (viewStats) {
			resultLines.add("");
			resultLines.add("-----   Performance  ----- : ");
			resultLines.add("         Total MBytes read: " + size / MEGA);
			resultLines.add("         Throughput mb/sec: " + (float) size
					* 1000.0 / (time * MEGA));
			resultLines.add("    Average IO rate mb/sec: " + rate / 1000
					/ blocks);
			resultLines.add("        Test exec time sec: " + (float) execTime
					/ 1000);
		}
