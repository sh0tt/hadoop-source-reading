...
public class TestLocalDirAllocator extends TestCase {
	...
	final static private String BUFFER_DIR[] = new String[] {
			BUFFER_DIR_ROOT + "/tmp0", BUFFER_DIR_ROOT + "/tmp1",
			BUFFER_DIR_ROOT + "/tmp2", BUFFER_DIR_ROOT + "/tmp3",
			BUFFER_DIR_ROOT + "/tmp4", BUFFER_DIR_ROOT + "/tmp5",
			BUFFER_DIR_ROOT + "/tmp6" };
	final static private Path BUFFER_PATH[] = new Path[] {
			new Path(BUFFER_DIR[0]), new Path(BUFFER_DIR[1]),
			new Path(BUFFER_DIR[2]), new Path(BUFFER_DIR[3]),
			new Path(BUFFER_DIR[4]), new Path(BUFFER_DIR[5]),
	...
	final static private boolean isWindows = System.getProperty("os.name")
			.startsWith("Windows");
	...
	private void validateTempDirCreation(int i) throws IOException {
		...
		assertTrue("Checking for " + BUFFER_DIR[i] + " in " + result
				+ " - FAILED!", result.getPath().startsWith(
	...
	public void test2() throws Exception {
		...
		try {
			...
			int firstDirIdx = (dirAllocator.getCurrentDirectoryIndex() == 0) ? 2
					: 3;
	...
	public void test3() throws Exception {
		...
		try {
			...
			int nextDirIdx = (dirAllocator.getCurrentDirectoryIndex() == 0) ? 3
					: 4;
		...
		try {
			...
			for (int i = 0; i < TRIALS; ++i) {
				...
				if (result.getPath().startsWith(
						new File(BUFFER_DIR[5], FILENAME).getPath())) {
					inDir5++;
				} else if (result.getPath().startsWith(
						new File(BUFFER_DIR[6], FILENAME).getPath())) {
					inDir6++;
				}
				result.delete();
			}
