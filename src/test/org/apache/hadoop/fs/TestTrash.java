...
public class TestTrash extends TestCase {
	...
	private final static Path TEST_DIR = new Path(new File(System.getProperty(
	...
	protected static void checkTrash(FileSystem fs, Path trashRoot, Path path)
			throws IOException {
	...
	protected static void trashShell(final FileSystem fs, final Path base)
		...
		checkNotInTrash(fs, trashRoot, new Path(base, "test/mkdirs/myFile")
				.toString());
		...
		// Verify that skip trash option really skips the trash for files (rm)
		{
			...
			assertFalse(fs.exists(trashRoot)); // No new Current should be
			// created
