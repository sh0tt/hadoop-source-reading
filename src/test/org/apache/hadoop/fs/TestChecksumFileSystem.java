...
public class TestChecksumFileSystem extends TestCase {
	public void testgetChecksumLength() throws Exception {
		...
		assertEquals(4000000000008L, ChecksumFileSystem.getChecksumLength(
				10000000000000L, 10));
	}
	...
	public void testDeletionOfCheckSum() throws Exception {
		...
		InMemoryFileSystem inMemFs = (InMemoryFileSystem) FileSystem.get(uri,
				conf);
		...
		fout.close();
		assertTrue("checksum exists", inMemFs.exists(inMemFs
				.getChecksumFile(testPath)));
		inMemFs.delete(testPath, true);
		assertTrue("checksum deleted", !inMemFs.exists(inMemFs
				.getChecksumFile(testPath)));
		...
		fout.close();
		inMemFs.delete(testPath, true);
		assertTrue("nothing in the namespace", inMemFs
				.listStatus(new Path("/")).length == 0);
	}

	public void testVerifyChecksum() throws Exception {
		String TEST_ROOT_DIR = System.getProperty("test.build.data",
		...
		assertTrue("checksum deleted", !localFs.exists(localFs
				.getChecksumFile(testPath)));
		...
		assertTrue("checksum exists", localFs.exists(localFs
				.getChecksumFile(testPath)));
