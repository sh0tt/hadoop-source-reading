...
public class TestCopyFiles extends TestCase {
	...
	private static class MyFile {
		...
		private static String[] dirNames = { "zero", "one", "two", "three",
	...
	private static boolean checkFiles(FileSystem fs, String topdir,
			MyFile[] files) throws IOException {
	...
	private static boolean checkFiles(FileSystem fs, String topdir,
		...
		for (int idx = 0; idx < files.length; idx++) {
			...
			try {
				...
				assertEquals("Cannnot read file.", toRead.length, in
						.read(toRead));
				in.close();
	...
	private static void updateFiles(FileSystem fs, String topdir,
			MyFile[] files, int nupdate) throws IOException {
	...
	private static FileStatus[] getFileStatus(FileSystem fs, String topdir,
			MyFile[] files) throws IOException {
	...
	private static FileStatus[] getFileStatus(FileSystem fs, String topdir,
		...
		for (int idx = 0; idx < NFILES; ++idx) {
			try {
				statuses.add(fs.getFileStatus(new Path(root, files[idx]
						.getName())));
			} catch (FileNotFoundException fnfe) {
				if (!existingOnly) {
					throw fnfe;
	...
	private static boolean checkUpdate(FileSystem fs, FileStatus[] old,
		...
		for (int idx = 0; idx < nupdate; ++idx) {
			final FileStatus stat = fs.getFileStatus(new Path(root, upd[idx]
					.getName()));
	...
	public void testCopyFromDfsToDfs() throws Exception {
		...
		try {
			...
			if (namenode.startsWith("hdfs://")) {
				...
				ToolRunner.run(new DistCp(conf), new String[] { "-log",
						namenode + "/logs", namenode + "/srcdat",
						namenode + "/destdat" });
				assertTrue("Source and destination directories do not match.",
						checkFiles(hdfs, "/destdat", files));
				FileSystem fs = FileSystem.get(URI.create(namenode + "/logs"),
						conf);
				assertTrue("Log directory does not exist.", fs.exists(new Path(
						namenode + "/logs")));
				deldir(hdfs, "/destdat");
				deldir(hdfs, "/srcdat");
				deldir(hdfs, "/logs");
	...
	public void testCopyFromLocalToDfs() throws Exception {
		...
		try {
			...
			if (namenode.startsWith("hdfs://")) {
				...
				assertTrue("Log directory does not exist.", hdfs
						.exists(new Path(namenode + "/logs")));
				deldir(hdfs, "/destdat");
				deldir(hdfs, "/logs");
				deldir(FileSystem.get(LOCAL_FS, conf), TEST_ROOT_DIR
						+ "/srcdat");
			}
		} finally {
			if (cluster != null) {
	...
	public void testCopyFromDfsToLocal() throws Exception {
		...
		try {
			...
			if (namenode.startsWith("hdfs://")) {
				MyFile[] files = createFiles(URI.create(namenode), "/srcdat");
				ToolRunner.run(new DistCp(conf), new String[] { "-log",
						"/logs", namenode + "/srcdat",
						"file:///" + TEST_ROOT_DIR + "/destdat" });
				assertTrue("Source and destination directories do not match.",
						checkFiles(localfs, TEST_ROOT_DIR + "/destdat", files));
				assertTrue("Log directory does not exist.", hdfs
						.exists(new Path("/logs")));
				deldir(localfs, TEST_ROOT_DIR + "/destdat");
				deldir(hdfs, "/logs");
				deldir(hdfs, "/srcdat");
			}
	...
	public void testCopyDfsToDfsUpdateOverwrite() throws Exception {
		...
		try {
			...
			if (namenode.startsWith("hdfs://")) {
				MyFile[] files = createFiles(URI.create(namenode), "/srcdat");
				ToolRunner.run(new DistCp(conf), new String[] { "-p", "-log",
						namenode + "/logs", namenode + "/srcdat",
						namenode + "/destdat" });
				assertTrue("Source and destination directories do not match.",
						checkFiles(hdfs, "/destdat", files));
				...
				ToolRunner.run(new DistCp(conf), new String[] { "-p",
						"-update", "-log", namenode + "/logs",
						namenode + "/srcdat", namenode + "/destdat" });
				assertTrue("Source and destination directories do not match.",
						checkFiles(hdfs, "/destdat", files));
				assertTrue(
						"Update failed to replicate all changes in src",
						checkUpdate(hdfs, dchkpoint, "/destdat", files, nupdate));

				deldir(hdfs, "/logs");
				ToolRunner.run(new DistCp(conf), new String[] { "-p",
						"-overwrite", "-log", namenode + "/logs",
						namenode + "/srcdat", namenode + "/destdat" });
				assertTrue("Source and destination directories do not match.",
						checkFiles(hdfs, "/destdat", files));
				assertTrue("-overwrite didn't.", checkUpdate(hdfs, dchkpoint,
						"/destdat", files, NFILES));
	...
	public void testCopyDuplication() throws Exception {
		final FileSystem localfs = FileSystem
				.get(LOCAL_FS, new Configuration());
		try {
			...
			assertEquals(DistCp.DuplicationException.ERROR_CODE, ToolRunner
					.run(new DistCp(new Configuration()), new String[] {
							"file:///" + TEST_ROOT_DIR + "/srcdat",
							"file:///" + TEST_ROOT_DIR + "/src2/srcdat",
							"file:///" + TEST_ROOT_DIR + "/destdat", }));
		} finally {
	...
	public void testCopySingleFile() throws Exception {
		...
		try {
			...
			ToolRunner.run(new DistCp(new Configuration()), new String[] {
					"file:///" + TEST_ROOT_DIR + "/srcdat/" + fname,
			...
			ToolRunner.run(new DistCp(new Configuration()), new String[] {
					"-update", "file:///" + TEST_ROOT_DIR + "/srcdat/" + sname,
	...
	public void testPreserveOption() throws Exception {
		...
		try {
			...
			{// test preserving user
				...
				ToolRunner.run(new DistCp(conf), new String[] { "-pu",
			...
			{// test preserving group
				...
				ToolRunner.run(new DistCp(conf), new String[] { "-pg",
						nnUri + "/srcdat", nnUri + "/destdat" });
				assertTrue("Source and destination directories do not match.",
						checkFiles(fs, "/destdat", files));
			...
			{// test preserving mode
				...
				ToolRunner.run(new DistCp(conf), new String[] { "-pp",
						nnUri + "/srcdat", nnUri + "/destdat" });
				assertTrue("Source and destination directories do not match.",
						checkFiles(fs, "/destdat", files));
				...
				FileStatus[] dststat = getFileStatus(fs, "/destdat", files);
				for (int i = 0; i < dststat.length; i++) {
					assertEquals("i=" + i, permissions[i], dststat[i]
							.getPermission());
				}
				deldir(fs, "/destdat");
				deldir(fs, "/srcdat");
	...
	public void testMapCount() throws Exception {
		...
		try {
			...
			ToolRunner.run(new DistCp(job), new String[] { "-m", "100", "-log",
					namenode + "/logs", namenode + "/srcdat",
					namenode + "/destdat" });
			assertTrue("Source and destination directories do not match.",
					checkFiles(fs, "/destdat", files));
			...
			deldir(fs, "/destdat");
			deldir(fs, "/logs");
			ToolRunner.run(new DistCp(job), new String[] { "-m", "1", "-log",
					namenode + "/logs", namenode + "/srcdat",
					namenode + "/destdat" });
		...
		try {
			...
			{// test -filelimit
				...
				ToolRunner.run(distcp,
						new String[] { "-filelimit", "" + filelimit,
				...
				FileStatus[] dststat = getFileStatus(fs, dstrootdir, files,
						true);
			...
			{// test -sizelimit
				...
				ToolRunner.run(distcp,
						new String[] { "-sizelimit", "" + sizelimit,
								nnUri + srcrootdir, nnUri + dstrootdir });
			...
			{// test update
				...
				final long totalsize = fs.getContentSummary(srcrootpath)
						.getLength();
				...
				final String[] args = { "-filelimit", "" + filelimit,
						"-sizelimit", "" + sizelimit, "-update",
						nnUri + srcrootdir, nnUri + dstrootdir };
				...
				for (int i = 0; i <= parts; i++) {
					...
					FileStatus[] dststat = getFileStatus(fs, dstrootdir, srcs,
							true);
					System.out
							.println(i + ") dststat.length=" + dststat.length);
					assertTrue(dststat.length - dstfilecount <= filelimit);
					ContentSummary summary = fs.getContentSummary(dstrootpath);
					System.out.println(i + ") summary.getLength()="
							+ summary.getLength());
	...
	static UnixUserGroupInformation createUGI(String name, boolean issuper) {
		...
		return UnixUserGroupInformation.createImmutable(new String[] {
				username, group });
	}
	...
	public void testHftpAccessControl() throws Exception {
		...
		try {
			...
			final Path home = createHomeDirectory(FileSystem
					.get(nnURI, dfsConf), USER_UGI);
			...
			final String[] args = { "hftp://" + httpAdd + srcrootdir,
					nnUri + dstrootdir };
