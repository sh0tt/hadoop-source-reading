...
public class TestGetFileBlockLocations extends TestCase {
	...
	private void oneTest(int offBegin, int offEnd, FileStatus status)
		...
		BlockLocation[] locations = fs.getFileBlockLocations(status, offBegin,
				offEnd - offBegin);
	...
	public void testFailureNegativeParameters() throws IOException {
		...
		try {
			BlockLocation[] locations = fs.getFileBlockLocations(status, -1,
					100);
		...
		try {
			BlockLocation[] locations = fs.getFileBlockLocations(status, 100,
					-1);
			fail("Expecting exception being throw");
		} catch (IllegalArgumentException e) {
	...
	public void testGetFileBlockLocations1() throws IOException {
		...
		for (int i = 0; i < 10; ++i) {
			oneTest((int) status.getLen() * i / 10, (int) status.getLen()
					* (i + 1) / 10, status);
		}
	}
