...
public class BigMapOutput extends Configured implements Tool {
	public static final Log LOG = LogFactory.getLog(BigMapOutput.class
			.getName());
	...
	private static void createBigMapInputFile(Configuration conf,
			FileSystem fs, Path dir, long fileSizeInMB) throws IOException {
		// Check if the input path exists and is non-empty
		if (fs.exists(dir)) {
			FileStatus[] list = fs.listStatus(dir);
			if (list != null && list.length > 0) {
				throw new IOException("Input path: " + dir
						+ " already exists... ");
			}
		}
		...
		int valueSizeRange = conf.getInt("test.bmo.max_value", 20000)
				- minValueSize;
		...
		LOG.info("Writing " + numBytesToWrite + " bytes to " + file + " with "
				+ "minKeySize: " + minKeySize + " keySizeRange: "
				+ keySizeRange + " minValueSize: " + minValueSize
	...
	private static void usage() {
		System.err
				.println("BigMapOutput -input <input-dir> -output <output-dir> "
