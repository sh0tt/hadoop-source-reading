...
public class TestFileOutputFormat extends HadoopTestCase {
	...
	public void testCustomFile() throws Exception {
		...
		for (FileStatus status : statuses) {
			...
			reduce = reduce
					|| status.getPath().getName().equals("test-r-00000");
		}
	...
	public static class TestMap implements
			Mapper<LongWritable, Text, LongWritable, Text> {

		public void configure(JobConf conf) {
			try {
				FileSystem fs = FileSystem.get(conf);
				OutputStream os = fs.create(FileOutputFormat
						.getPathForCustomFile(conf, "test"));
				os.write(1);
				os.close();
	...
	public static class TestReduce implements
