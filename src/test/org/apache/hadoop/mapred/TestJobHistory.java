 ...
 * 
 * testJobHistoryFile Run a job that will be succeeded and validate its history
 * file format and content.
 * 
 * testJobHistoryUserLogLocation Run jobs with the given values of
 * hadoop.job.history.user.location as (1)null(default case), (2)"none", and
 * (3)some dir like "/tmp". Validate user history file location in each case.
 * 
 * testJobHistoryJobStatus Run jobs that will be (1) succeeded (2) failed (3)
 * killed. Validate job status read from history file in each case.
 * 
...
public class TestJobHistory extends TestCase {
	...
	private static final Pattern digitsPattern = Pattern
			.compile(JobHistory.DIGITS);

	// hostname like /default-rack/host1.foo.com OR host1.foo.com
	private static final Pattern hostNamePattern = Pattern
			.compile("(/(([\\w\\-\\.]+)/)+)?([\\w\\-\\.]+)");

	private static final String IP_ADDR = "\\d\\d?\\d?\\.\\d\\d?\\d?\\.\\d\\d?\\d?\\.\\d\\d?\\d?";

	// hostname like /default-rack/host1.foo.com OR host1.foo.com
	private static final Pattern trackerNamePattern = Pattern
			.compile("tracker_" + hostNamePattern + ":([\\w\\-\\.]+)/"
					+ IP_ADDR + ":" + JobHistory.DIGITS);

	private static final Pattern splitsPattern = Pattern
			.compile(hostNamePattern + "(," + hostNamePattern + ")*");
	...
	/**
	 * Listener for history log file, it populates JobHistory.JobInfo object
	 * with data from log file and validates the data.
	 */
	static class TestListener extends
			DefaultJobHistoryParser.JobTasksParseListener {
		...
		public void handle(RecordTypes recType, Map<Keys, String> values)
			...
			if (recType == JobHistory.RecordTypes.Meta) {
				...
				if (time != null) {
					if (isJobLaunched) {
						// We assume that if we see LAUNCH_TIME again, it is
						// because of JT restart
						isJTRestarted = true;
				...
				if (time != null) {
					assertTrue(
							"Job FINISH_TIME is seen in history file at line "
									+ lineNum + " before LAUNCH_TIME is seen",
							isJobLaunched);
				}
				...
				if (time != null) {
					...
					assertTrue("Duplicate START_TIME seen for task " + taskid
							+ " in history file at line " + lineNum,
							attemptIDs == null);
				...
				if (time != null) {
					...
					if (s != null) {
						List<String> attemptIDs = taskIDsToAttemptIDs
								.get(taskid);
						assertTrue(
								"Task FINISH_TIME is seen in history file at line "
										+ lineNum
										+ " before START_TIME is seen",
								attemptIDs != null);

						// Check if all the attemptIDs of this task are finished
						assertTrue("TaskId " + taskid + " is finished at line "
								+ lineNum
								+ " but its attemptID is not finished.",
								(attemptIDs.size() <= 1));

						// Check if at least 1 attempt of this task is seen
						assertTrue("TaskId " + taskid + " is finished at line "
								+ lineNum
								+ " but no attemptID is seen before this.",
								attemptIDs.size() == 1);

						if (s.equals("KILLED") || s.equals("FAILED")) {
							// Task End with KILLED/FAILED status in history
							// file is
							// considered as TaskEnd, TaskStart. This is useful
							// in checking
						...
						// This line of history file could be just an update to
						// finish time
					}
				}
				...
				String taskid = values.get(Keys.TASKID);
				assertTrue("record type " + recType
						+ " is seen without TASKID key"
						+ " in history file at line " + lineNum, taskid != null);
				...
				if (time != null) {
					List<String> attemptIDs = taskIDsToAttemptIDs.get(taskid);
					assertTrue("TaskAttempt is seen in history file at line "
							+ lineNum + " before Task is seen",
							attemptIDs != null);
					assertFalse(
							"Duplicate TaskAttempt START_TIME is seen in history "
									+ "file at line " + lineNum, attemptIDs
									.remove(attemptId));
					...
					if (tempIgnoreStartTimeOfTasks.contains(taskid)
							&& (id.getId() < 1000)) {
						// If Task line of this attempt is seen in history file
						// after
						// JT restart and if this attempt is < 1000(i.e. attempt
						// is noti
						// started after JT restart) - assuming single JT
						// restart happened
				...
				if (time != null) {
					List<String> attemptIDs = taskIDsToAttemptIDs.get(taskid);
					assertTrue(
							"TaskAttempt FINISH_TIME is seen in history file at line "
									+ lineNum + " before Task is seen",
							attemptIDs != null);
					...
					assertTrue(
							"TaskAttempt FINISH_TIME is seen in history file at line "
									+ lineNum
									+ " before TaskAttempt START_TIME is seen",
	...
	private static void validateJobLevelKeyValuesFormat(
			Map<Keys, String> values, String status) {
		...
		assertTrue("Unknown priority for the job in history file", (priority
				.equals("HIGH")
				|| priority.equals("LOW")
	...
	private static void validateTaskLevelKeyValuesFormat(JobInfo job,
		...
		for (JobHistory.Task task : tasks.values()) {
			...
			if (!ignoreStartTimeOfTasks.contains(tid)
					|| (Long.parseLong(time) != 0)) {
				assertTrue("Task START_TIME of " + tid
						+ " is in unexpected format:" + time
						+ " in history file", isTimeValid(time));
			}

			String time1 = task.get(Keys.FINISH_TIME);
			assertTrue("Task FINISH_TIME of " + tid
					+ " is in unexpected format:" + time1 + " in history file",
					isTimeValid(time1));
			...
			assertTrue("Unknown Task type \"" + type + "\" is seen in "
					+ "history file for task " + tid, (type.equals("MAP")
					|| type.equals("REDUCE") || type.equals("SETUP") || type
					.equals("CLEANUP")));
			...
			if (type.equals("MAP")) {
				...
				if (!splitsCanBeEmpty || splits.length() != 0) {
					...
					assertTrue(
							"Unexpected format of SPLITS \"" + splits
									+ "\" is seen"
									+ " in history file for task " + tid, m
									.matches());
				}
			}
			...
			assertTrue("Unexpected TASK_STATUS \"" + status + "\" is seen in"
					+ " history file for task " + tid, (status
					.equals("SUCCESS")
			...
			for (JobHistory.TaskAttempt attempt : task.getTaskAttempts()
				...
				assertTrue("Unknown Task type \"" + type + "\" is seen in "
						...
						(type.equals("MAP") || type.equals("REDUCE")
								|| type.equals("SETUP") || type
								.equals("CLEANUP")));

				// Validate task status
				String status = attempt.get(Keys.TASK_STATUS);
				assertTrue("Unexpected TASK_STATUS \"" + status
						+ "\" is seen in" + " history file for task attempt "
						+ id, (status.equals("SUCCESS")
						|| status.equals("FAILED") || status.equals("KILLED")));

				// Reduce Task Attempts should have valid SHUFFLE_FINISHED time
				// and
				// SORT_FINISHED time
				if (type.equals("REDUCE") && status.equals("SUCCESS")) {
					...
					assertTrue("SHUFFLE_FINISHED time of task attempt " + id
							...
							+ " in history file", isTimeValid(time1));
					assertTrue(
							"Reduce Task SHUFFLE_FINISHED time is < START_TIME "
									+ "in history file", areTimesInOrder(time,
									time1));
					...
					assertTrue("SORT_FINISHED of task attempt " + id
							...
							+ " in history file", isTimeValid(time));
					assertTrue(
							"Reduce Task SORT_FINISHED time is < SORT_FINISHED time"
									+ " in history file", areTimesInOrder(
									time1, time));
				}
				...
				if (!status.equals("KILLED")) {
					...
					assertTrue("Unexpected http port of task attempt " + id, m
							.matches());
				}
				...
				try {
					Counters readCounters = Counters
							.fromEscapedCompactString(counters);
					assertTrue("Counters of task attempt " + id
							+ " are not parsable", readCounters != null);
				} catch (ParseException pe) {
					LOG.warn("While trying to parse counters of task attempt "
							+ id + ", " + pe);
				}
			}
		}
	...
	/**
	 * Validates the format of contents of history file (1) history file exists
	 * and in correct location (2) Verify if the history file is parsable (3)
	 * Validate the contents of history file (a) Format of all TIMEs are checked
	 * against a regex (b) validate legality/format of job level key, values (c)
	 * validate legality/format of task level key, values (d) validate
	 * legality/format of attempt level key, values (e) check if all the
	 * TaskAttempts, Tasks started are finished. Check finish of each
	 * TaskAttemptID against its start to make sure that all TaskAttempts, Tasks
	 * started are indeed finished and the history log lines are in the proper
	 * order. We want to catch ordering of history lines like Task START Attempt
	 * START Task FINISH Attempt FINISH (speculative execution is turned off for
	 * this).
	 * 
	 * @param id
	 *            job id
	...
	static void validateJobHistoryFileFormat(JobID id, JobConf conf,
		...
		JobHistory.parseHistoryFromFS(logFile.toString().substring(5), l,
				fileSys);
		...
		if (status.equals("SUCCESS")) {
			...
			for (Iterator<String> it = taskIDsToAttemptIDs.keySet().iterator(); it
					.hasNext();) {
				String taskid = it.next();
				assertTrue(
						"There are some Tasks which are not finished in history "
								+ "file.", taskEnds.contains(taskid));
				List<String> attemptIDs = taskIDsToAttemptIDs.get(taskid);
				if (attemptIDs != null) {
					assertTrue("Unexpected. TaskID " + taskid
							+ " has task attempt(s)"
	...
	private static void validateJobLevelKeyValues(MiniMRCluster mr,
		...
		assertTrue("SUBMIT_TIME of job obtained from history file did not "
				+ "match the expected value", jip.getStartTime() == Long
				.parseLong(values.get(Keys.SUBMIT_TIME)));

		assertTrue("LAUNCH_TIME of job obtained from history file did not "
				+ "match the expected value", jip.getLaunchTime() == Long
				.parseLong(values.get(Keys.LAUNCH_TIME)));

		assertTrue("FINISH_TIME of job obtained from history file did not "
				+ "match the expected value", jip.getFinishTime() == Long
				.parseLong(values.get(Keys.FINISH_TIME)));

		assertTrue("Job Status of job obtained from history file did not "
				+ "match the expected value", values.get(Keys.JOB_STATUS)
				.equals("SUCCESS"));

		assertTrue("Job Priority of job obtained from history file did not "
				+ "match the expected value", jip.getPriority().toString()
				.equals(values.get(Keys.JOB_PRIORITY)));
		...
		assertTrue("Counters of job obtained from history file did not "
				+ "match the expected value", c.makeEscapedCompactString()
				.equals(values.get(Keys.COUNTERS)));
		...
		assertTrue("Unexpected number of total maps in history file", Integer
				.parseInt(totalMaps) == jip.desiredMaps());
		...
		assertTrue("Unexpected number of failed maps in history file", Integer
				.parseInt(failedMaps) == jip.failedMapTasks);
	...
	private static void validateTaskLevelKeyValues(MiniMRCluster mr,
		...
		for (JobHistory.Task task : tasks.values()) {
			...
			if (tid.equals(mapTaskId.toString())
				...
				assertTrue("START_TIME of Task " + tid
						+ " obtained from history "
						+ "file did not match the expected value", tip
						.getExecStartTime() == Long.parseLong(task
						.get(Keys.START_TIME)));

				assertTrue("FINISH_TIME of Task " + tid
						+ " obtained from history "
						+ "file did not match the expected value", tip
						.getExecFinishTime() == Long.parseLong(task
						.get(Keys.FINISH_TIME)));

				if (taskId == mapTaskId) {// check splits only for map task
					assertTrue("Splits of Task " + tid
							+ " obtained from history file "
							+ " did not match the expected value", tip
							.getSplitNodes().equals(task.get(Keys.SPLITS)));
				}
				...
				assertTrue("Counters of Task " + tid
						+ " obtained from history file "
						+ " did not match the expected value", c
						.makeEscapedCompactString().equals(
								task.get(Keys.COUNTERS)));
			}
		}
	}
	...
	private static void validateTaskAttemptLevelKeyValues(MiniMRCluster mr,
		...
		for (JobHistory.Task task : tasks.values()) {
			...
			for (JobHistory.TaskAttempt attempt : task.getTaskAttempts()
				...
				// Validate task id
				assertTrue("Task id of Task Attempt " + idStr
						+ " obtained from "
						+ "history file did not match the expected value", tid
						.toString().equals(attempt.get(Keys.TASKID)));

				TaskInProgress tip = jip.getTaskInProgress(tid);
				TaskStatus ts = tip.getTaskStatus(attemptId);

				// Validate task attempt start time
				assertTrue("START_TIME of Task attempt " + idStr
						+ " obtained from "
						+ "history file did not match the expected value", ts
						.getStartTime() == Long.parseLong(attempt
						.get(Keys.START_TIME)));

				// Validate task attempt finish time
				assertTrue("FINISH_TIME of Task attempt " + idStr
						+ " obtained from "
						+ "history file did not match the expected value", ts
						.getFinishTime() == Long.parseLong(attempt
						.get(Keys.FINISH_TIME)));

				TaskTrackerStatus ttStatus = jt.getTaskTracker(ts
						.getTaskTracker());

				if (ttStatus != null) {
					assertTrue("http port of task attempt " + idStr
							+ " obtained from "
							+ "history file did not match the expected value",
							ttStatus.getHttpPort() == Integer.parseInt(attempt
									.get(Keys.HTTP_PORT)));

					if (attempt.get(Keys.TASK_STATUS).equals("SUCCESS")) {
						String ttHostname = jt.getNode(ttStatus.getHost())
								.toString();

						// check if hostname is valid
						assertTrue(
								"Host name of task attempt "
										+ idStr
										+ " obtained from"
										+ " history file did not match the expected value",
				...
				if (attempt.get(Keys.TASK_STATUS).equals("SUCCESS")) {
					...
					if (attempt.get(Keys.TASK_TYPE).equals("REDUCE")) {
						assertTrue(
								"SHUFFLE_FINISHED time of task attempt "
										+ idStr
										+ " obtained from history file did not match the expected"
										+ " value",
								ts.getShuffleFinishTime() == Long
										.parseLong(attempt
												.get(Keys.SHUFFLE_FINISHED)));
						assertTrue(
								"SORT_FINISHED time of task attempt "
										+ idStr
										+ " obtained from history file did not match the expected"
										+ " value",
								ts.getSortFinishTime() == Long
										.parseLong(attempt
												.get(Keys.SORT_FINISHED)));
					}

					// Validate task counters
					Counters c = ts.getCounters();
					assertTrue("Counters of Task Attempt " + idStr
							+ " obtained from "
							+ "history file did not match the expected value",
							c.makeEscapedCompactString().equals(
									attempt.get(Keys.COUNTERS)));
				}

				// check if tracker name is valid
				assertTrue("Tracker name of task attempt " + idStr
						+ " obtained from "
						+ "history file did not match the expected value", ts
						.getTaskTracker()
						.equals(attempt.get(Keys.TRACKER_NAME)));
			}
		}
	}
	 ...
	 * actual values obtained from JT. Job Level, Task Level and Task Attempt
	 * Level Keys, Values are validated.
	 * 
	 * @param job
	 *            RunningJob object of the job whose history is to be validated
	 * @param conf
	 *            job conf
	 */
	static void validateJobHistoryFileContent(MiniMRCluster mr, RunningJob job,
		...
		// Now the history file contents are available in jobInfo. Let us
		// compare
	...
	public void testJobHistoryFile() throws IOException {
		...
		try {
			...
			// Make sure that the job is not removed from memory until we do
			// finish
	...
	private static Path getLogLocationInOutputPath(String logFileName,
		...
		FileOutputFormat.setOutputPath(jobConf, FileOutputFormat
				.getOutputPath(conf));
		return JobHistory.JobInfo.getJobHistoryLogLocationForUser(logFileName,
				jobConf);
	}
	...
	private static void validateJobHistoryUserLogLocation(JobID id, JobConf conf)
		...
		if (conf.get("hadoop.job.history.user.location") == null) {
			assertTrue("User log file " + logFile + " does not exist", fileSys
					.exists(logFile));
			...
			if (logFile != logFile1) {
				...
				assertFalse(
						"Unexpected. User log file exists in output dir when "
	...
	private static void validateJobHistoryJobStatus(JobID id, JobConf conf,
		...
		assertTrue("History file permissions does not match", fileSys
				.getFileStatus(logFile).getPermission().equals(
		...
		assertTrue("Job Status read from job history file is not the expected"
				+ " status", status.equals(jobInfo.getValues().get(
				Keys.JOB_STATUS)));
	}
	...
	public void testJobHistoryJobStatus() throws IOException {
		...
		try {
			...
			assertTrue(historyCleanerRanAt == JobHistory.HistoryCleaner
					.getLastRan());
