...
/**
 * Tests various failures in setup/cleanup of job, like throwing exception,
 * command line kill and lost tracker
 */
public class TestSetupAndCleanupFailure extends TestCase {
	...
	static class CommitterWithLongSetupAndCleanup extends FileOutputCommitter {
		...
		@Override
		public void setupJob(JobContext context) throws IOException {
			waitForSignalFile(FileSystem.get(context.getJobConf()),
					setupSignalFile);
	...
	/**
	 * Tests setup and cleanup attempts getting killed from command-line and
	 * lost tracker
	 * 
	 * @param mr
	 * @param dfs
	 * @param commandLineKill
	 *            if true, test with command-line kill else, test with lost
	 *            tracker
	 * @throws IOException
	 */
	private void testSetupAndCleanupKill(MiniMRCluster mr, MiniDFSCluster dfs,
			boolean commandLineKill) throws IOException {
		// launch job with waiting setup/cleanup
		...
		UtilsForTests.writeFile(dfs.getNameNode(), dfs.getFileSystem()
				.getConf(), setupSignalFile, (short) 3);
		...
		UtilsForTests.writeFile(dfs.getNameNode(), dfs.getFileSystem()
				.getConf(), cleanupSignalFile, (short) 3);
		// wait for the job to finish.
		job.waitForCompletion();
		assertEquals(JobStatus.SUCCEEDED, job.getJobState());
		assertEquals(TaskStatus.State.KILLED, jt.getTaskStatus(setupID)
				.getRunState());
		assertEquals(TaskStatus.State.KILLED, jt.getTaskStatus(cleanupID)
				.getRunState());
	}

	// kill the task from command-line
	// wait till it kill is reported back
	private void killTaskFromCommandLine(RunningJob job, TaskAttemptID taskid,
	...
	public void testWithDFS() throws IOException {
		...
		try {
			...
			mr = new MiniMRCluster(taskTrackers, fileSys.getUri().toString(),
					1, null, null, jtConf);
			// test setup/cleanup throwing exceptions
			testFailCommitter(CommitterWithFailSetup.class, mr.createJobConf());
			testFailCommitter(CommitterWithFailCleanup.class, mr
					.createJobConf());
