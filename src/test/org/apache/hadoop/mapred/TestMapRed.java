...
public class TestMapRed extends TestCase {
	 ...
	 * instructed.
	 */
	static class RandomGenMapper implements
		...
		public void map(IntWritable key, IntWritable val,
			...
			for (int i = 0; i < randomCount; i++) {
				out.collect(new IntWritable(Math.abs(r.nextInt())),
	...
	/**
   */
	static class RandomGenReducer implements
	...
	static class RandomCheckMapper implements
		...
		public void map(WritableComparable key, Text val,
				...
				throws IOException {
			out.collect(
					new IntWritable(Integer.parseInt(val.toString().trim())),
					new IntWritable(1));
		}
   ...
   */
	 ...
	 */
	static class MergeMapper implements
			Mapper<IntWritable, IntWritable, IntWritable, IntWritable> {
	...
	static class MergeReducer implements
			Reducer<IntWritable, IntWritable, IntWritable, IntWritable> {
		public void configure(JobConf job) {
		}
	...
	private static class MyMap implements
	...
	private static class BadPartitioner implements
	...
	public static class NullMapper implements
	...
	public void testNullKeys() throws Exception {
		...
		SequenceFile.Reader r = new SequenceFile.Reader(fs, new Path(testdir,
				"nullout/part-00000"), conf);
		String m = "AAAAAAAAAAAAAA";
		for (int i = 1; r.next(NullWritable.get(), t); ++i) {
			assertTrue(t.toString() + " doesn't match " + m, m.equals(t
					.toString()));
	...
	private void checkCompression(boolean compressMapOutputs,
			CompressionType redCompression, boolean includeCombine)
			throws Exception {
		JobConf conf = new JobConf(TestMapRed.class);
		...
		try {
			...
			assertEquals("is reduce output compressed " + output,
					redCompression != CompressionType.NONE, rdr.isCompressed());
			rdr.close();
		} finally {
			fs.delete(testdir, true);
		}
	...
	public void testCompression() throws Exception {
		EnumSet<SequenceFile.CompressionType> seq = EnumSet
				.allOf(SequenceFile.CompressionType.class);
   ...
   */
	public static void launch() throws Exception {
		...
		SequenceFile.Writer out = SequenceFile.createWriter(fs, conf,
				answerkey, IntWritable.class, IntWritable.class,
				SequenceFile.CompressionType.NONE);
		try {
			for (int i = 0; i < range; i++) {
		...
		SequenceFile.Reader in = new SequenceFile.Reader(fs, recomputedkey,
				conf);
		int totalseen = 0;
		try {
			...
			for (int i = 0; i < range; i++) {
				...
				if (!in.next(key, val)) {
					...
					if (!((key.get() == i) && (val.get() == dist[i]))) {
						System.err.println("Mismatch!  Pos=" + key.get()
								+ ", i=" + i + ", val=" + val.get()
								+ ", dist[i]=" + dist[i]);
						success = false;
					}
	...
	private static boolean isSequenceFile(FileSystem fs, Path f)
			throws IOException {
	...
	public void runJob(int items) {
		try {
			...
			SequenceFile.Writer writer = SequenceFile.createWriter(fs, conf,
					inFile, Text.class, Text.class);

			StringBuffer content = new StringBuffer();

			for (int i = 0; i < 1000; i++) {
				content.append(i)
						.append(": This is one more line of content\n");
			}
