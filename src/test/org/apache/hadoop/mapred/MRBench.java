...
public class MRBench {
	...
	private static Path BASE_DIR = new Path(System.getProperty(
			"test.build.data", "/benchmarks/MRBench"));
	...
	public static class Map extends MapReduceBase implements
			Mapper<WritableComparable, Text, UTF8, UTF8> {
	...
	public static void generateTextFile(FileSystem fs, Path inputFile,
		...
		LOG.info("creating control file: " + numLines + " numLines, "
				+ sortOrder + " sortOrder");
	 ...
	 * the same file.
	 */
	private static ArrayList<Long> runJobInSequence(JobConf masterJobConf,
		...
		for (int i = 0; i < numRuns; i++) {
			...
			FileOutputFormat.setOutputPath(jobConf, new Path(OUTPUT_DIR,
					"output_" + rand.nextInt()));

			LOG.info("Running job " + i + ":" + " input="
					+ FileInputFormat.getInputPaths(jobConf)[0] + " output="
	...
	public static void main(String[] args) throws IOException {
		...
		if (numRuns < 1
				|| // verify args
				numMaps < 1 || numReduces < 1 || inputLines < 0
				|| inputSortOrder == null) {
			System.err.println(usage);
			System.exit(-1);
		}
		...
		Path inputFile = new Path(INPUT_DIR, "input_"
		...
		System.out.println(inputLines + "\t\t" + numMaps + "\t" + numReduces
				+ "\t" + avgTime);
	}
