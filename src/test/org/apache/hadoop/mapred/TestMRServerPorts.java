...
public class TestMRServerPorts extends TestCase {
	...
	private JobTracker startJobTracker(JobConf conf, JTRunner runner)
		...
		try {
			...
			conf.set("mapred.job.tracker.http.address", "0.0.0.0:"
					+ jt.getInfoPort());
	...
	private void setDataNodePorts(Configuration conf) {
		conf.set("dfs.datanode.address", TestHDFSServerPorts.NAME_NODE_HOST
				+ "0");
		conf.set("dfs.datanode.http.address",
				TestHDFSServerPorts.NAME_NODE_HTTP_HOST + "0");
		conf.set("dfs.datanode.ipc.address", TestHDFSServerPorts.NAME_NODE_HOST
				+ "0");
	}
	 ...
	 * Check whether the JobTracker can be started.
	 */
	private boolean canStartJobTracker(JobConf conf) throws IOException,
	 ...
	 */
	private boolean canStartTaskTracker(JobConf conf) throws IOException,
			InterruptedException {
		TaskTracker tt = null;
		try {
	...
	public void testJobTrackerPorts() throws Exception {
		...
		try {
			...
			conf2.set("mapred.job.tracker", FileSystem.getDefaultUri(
					hdfs.getConfig()).toString());
			...
			conf2.set("mapred.job.tracker.http.address", hdfs.getConfig().get(
					"dfs.http.address"));
	...
	public void testTaskTrackerPorts() throws Exception {
		...
		try {
			...
			// start job tracker on the same port as name-node
			conf2.set("mapred.task.tracker.report.address", FileSystem
					.getDefaultUri(hdfs.getConfig()).toString());
			...
			conf2.set("mapred.task.tracker.report.address",
					TestHDFSServerPorts.NAME_NODE_HOST + 0);
			conf2.set("mapred.task.tracker.http.address", hdfs.getConfig().get(
					"dfs.http.address"));
