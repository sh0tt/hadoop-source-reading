...
public class TestTTMemoryReporting extends TestCase {
	...
	public static class FakeTaskScheduler extends JobQueueTaskScheduler {
		...
		public List<Task> assignTasks(TaskTrackerStatus status)
			...
			long totalVirtualMemoryOnTT = getConf().getLong("totalVmemOnTT",
					JobConf.DISABLED_MEMORY_LIMIT);
			long totalPhysicalMemoryOnTT = getConf().getLong("totalPmemOnTT",
					JobConf.DISABLED_MEMORY_LIMIT);
			long mapSlotMemorySize = getConf().getLong("mapSlotMemorySize",
					JobConf.DISABLED_MEMORY_LIMIT);
			long reduceSlotMemorySize = getConf().getLong(
					"reduceSlotMemorySize", JobConf.DISABLED_MEMORY_LIMIT);

			long reportedTotalVirtualMemoryOnTT = status.getResourceStatus()
					.getTotalVirtualMemory();
			long reportedTotalPhysicalMemoryOnTT = status.getResourceStatus()
					.getTotalPhysicalMemory();
			long reportedMapSlotMemorySize = status.getResourceStatus()
					.getMapSlotMemorySizeOnTT();
			long reportedReduceSlotMemorySize = status.getResourceStatus()
					.getReduceSlotMemorySizeOnTT();

			message = "expected memory values : (totalVirtualMemoryOnTT, totalPhysicalMemoryOnTT, "
		...
		try {
			...
			conf
					.setClass(
							...
							DummyMemoryCalculatorPlugin.class,
	...
	public void testConfiguredMemoryValues() throws Exception {
		...
		conf
				.setLong(JobTracker.MAPRED_CLUSTER_REDUCE_MEMORY_MB_PROPERTY,
						1024L);
	...
	private void runSleepJob(JobConf conf) throws Exception {
		String[] args = { "-m", "1", "-r", "1", "-mt", "10", "-rt", "10" };
		ToolRunner.run(conf, new SleepJob(), args);
	}

	private void verifyTestResults() {
		FakeTaskScheduler scheduler = (FakeTaskScheduler) miniMRCluster
				.getJobTrackerRunner().getJobTracker().getTaskScheduler();
