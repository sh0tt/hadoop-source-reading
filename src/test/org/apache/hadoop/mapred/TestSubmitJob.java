...
public class TestSubmitJob extends TestCase {
	...
	/**
	 * Test to verify that jobs with invalid memory requirements are killed at
	 * the JT.
	 * 
	 * @throws Exception
	 */
	public void testJobWithInvalidMemoryReqs() throws Exception {
		...
		jtConf.setLong(JobTracker.MAPRED_CLUSTER_MAP_MEMORY_MB_PROPERTY,
				1 * 1024L);
		...
		runJobAndVerifyFailure(jobConf, JobConf.DISABLED_MEMORY_LIMIT,
				1 * 1024L, "Invalid job requirements.");

		// No reduce-memory configuration
		jobConf = new JobConf(clusterConf);
		jobConf.setMemoryForMapTask(1 * 1024L);
		runJobAndVerifyFailure(jobConf, 1 * 1024L,
	...
	private void runJobAndVerifyFailure(JobConf jobConf, long memForMapTasks,
			long memForReduceTasks, String expectedMsg) throws Exception,
			IOException {
		...
		String overallExpectedMsg = "(" + memForMapTasks + " memForMapTasks "
				+ memForReduceTasks + " memForReduceTasks): " + expectedMsg;
		assertTrue(
				"Observed message - " + msg
						+ " - doesn't contain expected message - "
						+ overallExpectedMsg, msg.contains(overallExpectedMsg));
	}
}
