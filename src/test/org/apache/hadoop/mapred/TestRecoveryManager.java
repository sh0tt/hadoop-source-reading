...
public class TestRecoveryManager extends TestCase {
	...
	private static final Path TEST_DIR = new Path(System.getProperty(
			"test.build.data", "/tmp"), "test-recovery-manager");

	/**
	 * Tests the {@link JobTracker} against the exceptions thrown in
	 * {@link JobTracker.RecoveryManager}. It does the following : - submits 2
	 * jobs - kills the jobtracker - Garble job.xml for one job causing it to
	 * fail in constructor and job.split for another causing it to fail in init.
	 * - restarts the jobtracker - checks if the jobtraker starts normally
	 */
	public void testJobTracker() throws Exception {
		...
		UtilsForTests.configureWaitingJobConf(job1,
				new Path(TEST_DIR, "input"), new Path(TEST_DIR, "output1"), 2,
				0, "test-recovery-manager", signalFile, signalFile);
		...
		UtilsForTests.configureWaitingJobConf(job2,
				new Path(TEST_DIR, "input"), new Path(TEST_DIR, "output2"), 30,
				0, "test-recovery-manager", signalFile, signalFile);
		...
		Path jobFile = new Path(sysDir, rJob1.getID().toString()
		...
		Path jobSplitFile = new Path(sysDir, rJob2.getID().toString()
				+ Path.SEPARATOR + "job.split");
		...
		assertEquals("JobTracker crashed!", JobTracker.State.RUNNING, status
				.getJobTrackerState());
	 ...
	 * thrown during recovery. It does the following : - submits a job with HIGH
	 * priority and x tasks - allows it to complete 50% - submits another job
	 * with normal priority and y tasks - kills the jobtracker - restarts the
	 * jobtracker with max-tasks-per-job such that y < max-tasks-per-job < x -
	...
	public void testRecoveryManager() throws Exception {
		...
		UtilsForTests.configureWaitingJobConf(job1,
				new Path(TEST_DIR, "input"), new Path(TEST_DIR, "output3"), 30,
				0, "test-recovery-manager", signalFile, signalFile);
		...
		UtilsForTests.configureWaitingJobConf(job2,
				new Path(TEST_DIR, "input"), new Path(TEST_DIR, "output4"), 20,
				0, "test-recovery-manager", signalFile1, signalFile1);
		...
		UtilsForTests.configureWaitingJobConf(job3,
				new Path(TEST_DIR, "input"), new Path(TEST_DIR, "output5"), 1,
				0, "test-recovery-manager", signalFile, signalFile);
		...
		assertEquals("Recovery manager failed to tolerate job failures", 2,
				jobtracker.getAllJobs().length);

		// check if the job#1 has failed
		JobStatus status = jobtracker.getJobStatus(rJob1.getID());
		assertEquals("Faulty job not failed", JobStatus.FAILED, status
				.getRunState());
	...
	/**
	 * Test if restart count of the jobtracker is correctly managed. Steps are
	 * as follows : - start the jobtracker and check if the info file gets
	 * created. - stops the jobtracker, deletes the jobtracker.info file and
	 * checks if upon restart the recovery is 'off' - submit a job to the
	 * jobtracker. - restart the jobtracker k times and check if the restart
	 * count on ith iteration is i. - submit a new job and check if its restart
	 * count is 0. - garble the jobtracker.info file and restart he jobtracker,
	 * the jobtracker should crash.
	 */
	public void testRestartCount() throws Exception {
		...
		UtilsForTests.configureWaitingJobConf(job, new Path(TEST_DIR, "input"),
				new Path(TEST_DIR, "output6"), 2, 0, "test-recovery-manager",
		...
		for (int i = 1; i <= 5; ++i) {
			...
			// check if the system dir is sane
			assertTrue("Recovery file is missing upon restart", fs
					.exists(rFile));
			assertFalse("Temp recovery file exists upon restart", fs
					.exists(tFile));
			...
			assertEquals("Recovery manager failed to recover restart count", i,
					jip.getNumRestarts());
		}
		...
		UtilsForTests.configureWaitingJobConf(job1,
				new Path(TEST_DIR, "input"), new Path(TEST_DIR, "output7"), 50,
				0, "test-recovery-manager", signalFile, signalFile);
	...
	public void testJobTrackerInfoCreation() throws Exception {
		...
		Path tmpRestartFile = jobtracker.recoveryManager
				.getTempRestartCountFile();
		FileSystem fs = dfs.getFileSystem();
		assertFalse("Info file exists after update failure", fs
				.exists(restartFile));
		assertFalse("Temporary restart-file exists after update failure", fs
				.exists(restartFile));
