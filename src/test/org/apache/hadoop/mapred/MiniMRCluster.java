...
public class MiniMRCluster {
	...
	public void waitUntilIdle() {
		...
		try {
			...
			while (client.getClusterStatus().getTaskTrackers() < taskTrackerList
					.size()) {
	...
	public MiniMRCluster(int jobTrackerPort, int taskTrackerPort,
			int numTaskTrackers, String namenode, int numDir, String[] racks,
			String[] hosts, UnixUserGroupInformation ugi) throws IOException {
		this(jobTrackerPort, taskTrackerPort, numTaskTrackers, namenode,
	...
	public MiniMRCluster(int jobTrackerPort, int taskTrackerPort,
			int numTaskTrackers, String namenode, int numDir, String[] racks,
			String[] hosts, UnixUserGroupInformation ugi, JobConf conf)
			throws IOException {
		if (racks != null && racks.length < numTaskTrackers) {
			LOG
					.error("Invalid number of racks specified. It should be at least "
	...
	public MapTaskCompletionEventsUpdate getMapTaskCompletionEventsUpdates(
		...
		return taskTrackerList.get(index).getTaskTracker()
				.getMapCompletionEvents(jobId, 0, max, dummy);
	}
