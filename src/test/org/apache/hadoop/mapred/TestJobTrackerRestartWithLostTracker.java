...
public class TestJobTrackerRestartWithLostTracker extends TestCase {
	...
	private JobConf configureJob(JobConf conf, int maps, int reduces,
		...
		UtilsForTests.configureWaitingJobConf(conf, inDir, outputDir, maps,
				reduces, "test-jobtracker-restart-with-lost-tt", mapSignal,
				redSignal);
		return conf;
	}

	public void testRecoveryWithLostTracker(MiniDFSCluster dfs, MiniMRCluster mr)
			throws IOException {
		...
		assertEquals(
				"Tracker killed while the jobtracker was down did not get lost "
						+ "upon restart", jobClient.getClusterStatus()
						.getTaskTrackers(), 1);
