...
public class TestMapredSystemDir extends TestCase {
	...
	// dfs ugi
	private static final UnixUserGroupInformation DFS_UGI = TestMiniMRWithDFSWithDistinctUsers
			.createUGI("dfs", true);
	// mapred ugi
	private static final UnixUserGroupInformation MR_UGI = TestMiniMRWithDFSWithDistinctUsers
			.createUGI("mr", false);
	private static final FsPermission SYSTEM_DIR_PERMISSION = FsPermission
	...
	public void testGarbledMapredSystemDir() throws Exception {
		...
		try {
			...
			fs.setPermission(mapredSysDir, new FsPermission(
			...
			mr = new MiniMRCluster(0, 0, 0, dfs.getFileSystem().getUri()
					.toString(), 1, null, null, MR_UGI, new JobConf(mrConf));
			...
			assertFalse("JobTracker did not bail out (waited for 10 secs)", mr
					.getJobTrackerRunner().isActive());
		} finally {
			if (dfs != null) {
				dfs.shutdown();
			}
