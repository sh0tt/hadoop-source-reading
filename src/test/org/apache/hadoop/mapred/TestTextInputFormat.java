...
public class TestTextInputFormat extends TestCase {
	private static final Log LOG = LogFactory.getLog(TestTextInputFormat.class
			.getName());
	...
	public void testFormat() throws Exception {
		...
		for (int length = 0; length < MAX_LENGTH; length += random
				.nextInt(MAX_LENGTH / 10) + 1) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (length == 0) {
					assertEquals(
							...
							1, splits.length);
					assertEquals("Empty file length == 0", 0, splits[0]
							.getLength());
				}
				...
				for (int j = 0; j < splits.length; j++) {
					...
					RecordReader<LongWritable, Text> reader = format
							.getRecordReader(splits[j], job, reporter);
					try {
						int count = 0;
						while (reader.next(key, value)) {
							...
							if (bits.get(v)) {
								LOG
										.warn("conflict with " + v
												+ " in split " + j
												+ " at position "
												+ reader.getPos());
							}
							assertFalse("Key in multiple partitions.", bits
									.get(v));
							bits.set(v);
							count++;
						}
						LOG.debug("splits[" + j + "]=" + splits[j] + " count="
								+ count);
					} finally {
						reader.close();
					}
				}
				assertEquals("Some keys in no partition.", length, bits
						.cardinality());
			}
	...
	/**
	 * Test readLine for various kinds of line termination sequneces. Varies
	 * buffer size to stress test. Also check that returned value matches the
	 * string length.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Test readLine for correct interpretation of maxLineLength (returned
	 * string should be clipped at maxLineLength, and the remaining bytes on the
	 * same line should be thrown out). Also check that returned value matches
	 * the string length. Varies buffer size to stress test.
	 * 
	 * @throws Exception
	 */
	...
	private static void writeFile(FileSystem fs, Path name,
			CompressionCodec codec, String contents) throws IOException {
		OutputStream stm;
		if (codec == null) {
			stm = fs.create(name);
		} else {
	...
	private static List<Text> readSplit(TextInputFormat format,
			InputSplit split, JobConf job) throws IOException {
		List<Text> result = new ArrayList<Text>();
		RecordReader<LongWritable, Text> reader = format.getRecordReader(split,
				job, voidReporter);
	...
	public static void testGzip() throws IOException {
		...
		assertEquals("splits[1][0]", "this is a test", results.get(0)
				.toString());
