...
public class TestJobTrackerSafeMode extends TestCase {
	final Path testDir = new Path(
			System.getProperty("test.build.data", "/tmp"), "jt-safemode");
	...
	private static final Log LOG = LogFactory
			.getLog(TestJobTrackerSafeMode.class);
	...
	private JobConf configureJob(JobConf conf, int maps, int reduces,
		...
		UtilsForTests.configureWaitingJobConf(conf, inDir, outputDir, maps,
				reduces, "test-jobtracker-safemode", mapSignal, redSignal);
		return conf;
	}

	/**
	 * Tests the jobtracker's safemode. The test is as follows : - starts a
	 * cluster with 2 trackers - submits a job with large (40) maps to make sure
	 * that all the trackers are logged to the job history - wait for the job to
	 * be 50% done - stop the jobtracker - wait for the trackers to be done with
	 * all the tasks - kill a task tracker - start the jobtracker - start 2 more
	 * trackers - now check that while all the tracker are detected (or lost)
	 * the scheduling window is closed - check that after all the trackers are
	 * recovered, scheduling is opened
	 */
	private void testSafeMode(MiniDFSCluster dfs, MiniMRCluster mr)
		...
		for (int i = 0; i < numTracker; ++i) {
			...
			while (runningCount != 0) {
				LOG
						.info("Waiting for tracker " + t.getName()
								+ " to stabilize");
				UtilsForTests.waitFor(100);
				runningCount = 0;
				for (TaskStatus status : t.getRunningTaskStatuses()) {
					if (status.getIsMap()
							&& (status.getRunState() == TaskStatus.State.UNASSIGNED || status
		...
		assertTrue("JobTracker hasnt opened up scheduling even all the"
				+ " trackers were recovered", jobtracker.recoveryManager
				.shouldSchedule());
	...
	private boolean checkTrackers(JobTracker jobtracker, Set<String> present,
			Set<String> absent) {
		long jobtrackerRecoveryFinishTime = jobtracker.getStartTime()
		...
		for (String trackerName : present) {
			...
			// check if the status is present and also the tracker has contacted
			// back
			// after restart
			if (status == null
	...
	public void testJobTrackerSafeMode() throws IOException {
		...
		try {
			...
			mr = new MiniMRCluster(numTrackers, namenode, numDir, null, null,
					jtConf);
