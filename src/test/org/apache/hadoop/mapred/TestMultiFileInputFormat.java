...
public class TestMultiFileInputFormat extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog(TestMultiFileInputFormat.class);
	...
	private class DummyMultiFileInputFormat extends
			MultiFileInputFormat<Text, Text> {
		@Override
	...
	public void testFormat() throws IOException {
		...
		for (int numFiles = 1; numFiles < MAX_NUM_FILES; numFiles += (NUM_FILES_INCR / 2)
			...
			for (int i = 1; i < MAX_SPLIT_COUNT; i += rand
					.nextInt(SPLIT_COUNT_INCR) + 1) {
				LOG.info("Running for Num Files=" + numFiles + ", split count="
						+ i);

				MultiFileSplit[] splits = (MultiFileSplit[]) format.getSplits(
						job, i);
				bits.clear();
				...
				for (MultiFileSplit split : splits) {
					...
					for (Path p : split.getPaths()) {
						...
						assertEquals(length, lengths.get(p.getName())
								.longValue());
						splitLength += length;
						String name = p.getName();
						int index = Integer.parseInt(name.substring(name
								.lastIndexOf("file_") + 5));
