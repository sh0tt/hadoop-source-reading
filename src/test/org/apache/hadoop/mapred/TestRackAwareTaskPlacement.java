...
public class TestRackAwareTaskPlacement extends TestCase {
	...
	private static final String hosts2[] = new String[] { "host1.rack2.com",
			"host2.rack2.com" };
	...
	static void launchJobAndTestCounters(String jobName, MiniMRCluster mr,
			FileSystem fileSys, Path in, Path out, int numMaps,
			int otherLocalMaps, int dataLocalMaps, int rackLocalMaps)
			throws IOException {
		...
		assertEquals("Number of local maps", counters
				.getCounter(JobInProgress.Counter.OTHER_LOCAL_MAPS),
				otherLocalMaps);
		assertEquals("Number of Data-local maps", counters
				.getCounter(JobInProgress.Counter.DATA_LOCAL_MAPS),
				dataLocalMaps);
		assertEquals("Number of Rack-local maps", counters
				.getCounter(JobInProgress.Counter.RACK_LOCAL_MAPS),
				rackLocalMaps);
		mr.waitUntilIdle();
		mr.shutdown();
	}
	...
	public void testTaskPlacement() throws IOException {
		...
		try {
			 ...
			 */
			launchJobAndTestCounters(testName, mr, fileSys, inDir, outputPath,
					3, 0, 2, 0);
			mr.shutdown();
