...
public class TestLineInputFormat extends TestCase {
	...
	private static Path workDir = new Path(new Path(System.getProperty(
			"test.build.data", "."), "data"), "TestLineInputFormat");
	...
	void checkFormat(JobConf job, int expectedN) throws IOException {
		...
		for (int j = 0; j < splits.length - 1; j++) {
			assertEquals("There are no split locations", 0, splits[j]
					.getLocations().length);
			RecordReader<LongWritable, Text> reader = format.getRecordReader(
					splits[j], job, voidReporter);
			Class readerClass = reader.getClass();
			assertEquals("reader class is LineRecordReader.",
					LineRecordReader.class, readerClass);
			LongWritable key = reader.createKey();
			Class keyClass = key.getClass();
			assertEquals("Key class is LongWritable.", LongWritable.class,
					keyClass);
			...
			assertEquals("number of lines in split is " + expectedN, expectedN,
					count);
		}
	}
