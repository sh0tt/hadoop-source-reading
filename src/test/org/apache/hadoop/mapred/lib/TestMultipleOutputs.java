...
public class TestMultipleOutputs extends HadoopTestCase {
	...
	protected void _testMultipleOutputs(boolean withCounters) throws Exception {
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(fs
				.open(new Path(FileOutputFormat.getOutputPath(conf),
						"text-r-00000"))));
		int count = 0;
		...
		SequenceFile.Reader seqReader = new SequenceFile.Reader(fs, new Path(
				FileOutputFormat.getOutputPath(conf), "sequence_B-r-00000"),
				conf);
		...
		Counters.Group counters = job.getCounters().getGroup(
				MultipleOutputs.class.getName());
		if (!withCounters) {
	...
	public static class MOMap implements
	...
	@SuppressWarnings( { "unchecked" })
	public static class MOReduce implements
