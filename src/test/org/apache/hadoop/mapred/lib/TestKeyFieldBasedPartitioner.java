...
public class TestKeyFieldBasedPartitioner extends TestCase {
	...
	public void testEmptyKey() throws Exception {
		...
		kfbp.configure(conf);
		assertEquals("Empty key should map to 0th partition", 0, kfbp
		...
		kfbp.configure(conf);
		assertEquals("Partitioner doesnt work as expected", 0, kfbp
				.getPartition(new Text(input), new Text(), numReducers));
		...
		kfbp = new KeyFieldBasedPartitioner<Text, Text>();
		conf = new JobConf();
		conf.set("mapred.text.key.partitioner.options",
				"-k2,2 -k30,21 -k4,4 -k5");
		kfbp.configure(conf);
		expectedOutput = "def";
		eBytes = expectedOutput.getBytes();
