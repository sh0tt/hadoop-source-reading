...
public class TestCombineFileInputFormat extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog(TestCombineFileInputFormat.class);
	...
	public void testSplitPlacement() throws IOException {
		...
		try {
			/*
			 * Start 3 datanodes, one each in rack r1, r2, r3. Create three
			 * files 1) file1, just after starting the datanode on r1, with a
			 * repl factor of 1, and, 2) file2, just after starting the datanode
			 * on r2, with a repl factor of 2, and, 3) file3 after starting the
			 * all three datanodes, with a repl factor of 3. At the end, file1
			 * will be present on only datanode1, file2 will be present on
			 * datanode 1 and datanode2 and file3 will be present on all
			 * datanodes.
			 */
			...
			assertEquals(fileSplit.getLocations()[0], hosts2[0]); // should be
			// on r2
			...
			assertEquals(fileSplit.getOffset(0), 0);
			assertEquals(fileSplit.getLength(0), BLOCKSIZE);
			assertEquals(fileSplit.getLocations()[0], hosts1[0]); // should be
			// on r1
			...
			assertEquals(fileSplit.getLocations()[0], hosts3[0]); // should be
			// on r3
			fileSplit = (CombineFileSplit) splits[1];
			assertEquals(fileSplit.getNumPaths(), 2);
			assertEquals(fileSplit.getLocations().length, 1);
			...
			inFormat.setMinSplitSizeRack(1); // everything is at least rack
			// local
	...
	static void writeFile(Configuration conf, Path name, short replication,
			int numBlocks) throws IOException {
		FileSystem fileSys = FileSystem.get(conf);

		FSDataOutputStream stm = fileSys.create(name, true, conf.getInt(
