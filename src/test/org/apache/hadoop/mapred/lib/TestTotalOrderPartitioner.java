...
public class TestTotalOrderPartitioner extends TestCase {
	...
	public void testTotalOrderMemCmp() throws Exception {
		...
		try {
			...
			for (Check<Text> chk : testStrings) {
				assertEquals(chk.data.toString(), chk.part, partitioner
						.getPartition(chk.data, nw, splitStrings.length + 1));
			}
		} finally {
	...
	public static class ReverseStringComparator implements RawComparator<Text> {
		...
		public int compare(byte[] b1, int s1, int l1, byte[] b2, int s2, int l2) {
			...
			return -1
					* WritableComparator.compareBytes(b1, s1 + n1, l1 - n1, b2,
							s2 + n2, l2 - n2);
		}
	}
	...
	public void testTotalOrderCustomComparator() throws Exception {
		TotalOrderPartitioner<Text, NullWritable> partitioner = new TotalOrderPartitioner<Text, NullWritable>();
		JobConf job = new JobConf();
		Text[] revSplitStrings = Arrays.copyOf(splitStrings,
				splitStrings.length);
