...
public class TestChainMapReduce extends HadoopTestCase {
	...
	private static void writeFlag(JobConf conf, String flag) throws IOException {
		...
		DataOutputStream file = fs.create(new Path(getFlagDir(conf.getBoolean(
				"localFS", true)), flag));
		file.close();
	}

	private static boolean getFlag(JobConf conf, String flag)
			throws IOException {
		FileSystem fs = FileSystem.get(conf);
		return fs.exists(new Path(getFlagDir(conf.getBoolean("localFS", true)),
				flag));
	}
	...
	public void testChain() throws Exception {
		...
		reduceConf.set("a", "C");
		ChainReducer.setReducer(conf, CReduce.class, LongWritable.class,
				Text.class, LongWritable.class, Text.class, true, reduceConf);
		...
		mapEConf.set("a", "E");
		ChainReducer.addMapper(conf, EMap.class, LongWritable.class,
				Text.class, LongWritable.class, Text.class, true, mapEConf);
	...
	public static class IDMap implements
	...
	public static class IDReduce implements
