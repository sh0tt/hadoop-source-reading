...
public class TestDelegatingInputFormat extends TestCase {
	...
	public void testSplitting() throws Exception {
		...
		try {
			dfs = new MiniDFSCluster(conf, 4, true, new String[] { "/rack0",
					"/rack0", "/rack1", "/rack1" }, new String[] { "host0",
					"host1", "host2", "host3" });
			...
			MultipleInputs.addInputPath(conf, path2, TextInputFormat.class,
					MapClass2.class);
			MultipleInputs.addInputPath(conf, path3,
					KeyValueTextInputFormat.class, MapClass.class);
			...
			for (InputSplit split : splits) {
				...
				if (tis.getInputFormatClass().equals(
	...
	static Path getPath(final String location, final FileSystem fs)
		...
		DataOutputStream out = fs
				.create(path, true, 4096, (short) 2, 512, null);
