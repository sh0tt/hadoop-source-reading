...
public class TestJobControl extends junit.framework.TestCase {
	...
	public static void doJobControlTest() throws Exception {
		...
		inPaths_1.add(indir);
		JobConf jobConf_1 = JobControlTestUtils.createCopyJob(inPaths_1,
				outdir_1);
		Job job_1 = new Job(jobConf_1, dependingJobs);
		ArrayList<Path> inPaths_2 = new ArrayList<Path>();
		inPaths_2.add(indir);
		JobConf jobConf_2 = JobControlTestUtils.createCopyJob(inPaths_2,
				outdir_2);
		...
		JobConf jobConf_3 = JobControlTestUtils.createCopyJob(inPaths_3,
				outdir_3);
		...
		JobConf jobConf_4 = JobControlTestUtils.createCopyJob(inPaths_4,
				outdir_4);
		dependingJobs = new ArrayList<Job>();
		...
		if (job_1.getState() != Job.FAILED
			...
			throw new Exception(
					"The state of job_1 is not in a complete state\n" + states);
		}
		...
		if (job_2.getState() != Job.FAILED
			...
			throw new Exception(
					"The state of job_2 is not in a complete state\n" + states);
		}
		...
		if (job_3.getState() != Job.FAILED
			...
			throw new Exception(
					"The state of job_3 is not in a complete state\n" + states);
		}
		if (job_4.getState() != Job.FAILED
			...
			throw new Exception(
					"The state of job_4 is not in a complete state\n" + states);
		}
		...
		if (job_1.getState() == Job.FAILED || job_2.getState() == Job.FAILED
			...
			if (job_3.getState() != Job.DEPENDENT_FAILED) {
				...
				throw new Exception(
						"The states of jobs 1, 2, 3, 4 are not consistent\n"
								+ states);
			}
		}
		if (job_3.getState() == Job.FAILED
