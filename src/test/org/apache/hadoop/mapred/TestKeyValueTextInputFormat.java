...
public class TestKeyValueTextInputFormat extends TestCase {
	private static final Log LOG = LogFactory
			.getLog(TestKeyValueTextInputFormat.class.getName());
	...
	private static Path workDir = new Path(new Path(System.getProperty(
			"test.build.data", "."), "data"), "TestKeyValueTextInputFormat");
		...
		for (int length = 0; length < MAX_LENGTH; length += random
			...
			for (int i = 0; i < 3; i++) {
				...
				for (int j = 0; j < splits.length; j++) {
					...
					RecordReader<Text, Text> reader = format.getRecordReader(
							splits[j], job, reporter);
					Class readerClass = reader.getClass();
					assertEquals("reader class is KeyValueLineRecordReader.",
							KeyValueLineRecordReader.class, readerClass);
