...
public class TestMiniMRDFSSort extends TestCase {
	...
	public static Test suite() {
		TestSetup setup = new TestSetup(new TestSuite(TestMiniMRDFSSort.class)) {
			protected void setUp() throws Exception {
				...
				dfsCluster = new MiniDFSCluster(conf, NUM_HADOOP_SLAVES, true,
						null);
				dfs = dfsCluster.getFileSystem();
				mrCluster = new MiniMRCluster(NUM_HADOOP_SLAVES, dfs.getUri()
						.toString(), 1);
			}
	...
	private static void runSort(JobConf job, Path sortInput, Path sortOutput)
		...
		assertTrue("map input = " + mapInput + ", hdfs read = " + hdfsRead,
				(hdfsRead < (mapInput * 1.1)) && (hdfsRead > mapInput));
	}

	private static void runSortValidator(JobConf job, Path sortInput,
			Path sortOutput) throws Exception {
		String[] svArgs = { "-sortInput", sortInput.toString(), "-sortOutput",
				sortOutput.toString() };
	...
	private static class ReuseDetector extends MapReduceBase implements
	...
	private static void runJvmReuseTest(JobConf job, boolean reuse)
			throws IOException {
