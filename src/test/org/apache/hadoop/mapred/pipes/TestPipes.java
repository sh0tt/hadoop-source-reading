...
public class TestPipes extends TestCase {
	...
	public void testPipes() throws IOException {
		...
		try {
			...
			runNonPipedProgram(mr, dfs, new Path(cppExamples,
					"bin/wordcount-nopipe"));
			mr.waitUntilIdle();
		} finally {
			mr.shutdown();
			dfs.shutdown();
	...
	private void writeInputFile(FileSystem fs, Path dir) throws IOException {
		...
		out
				.writeBytes("Alice was beginning to get very tired of sitting by her\n");
		out
				.writeBytes("sister on the bank, and of having nothing to do: once\n");
		out
				.writeBytes("or twice she had peeped into the book her sister was\n");
	...
	private void runProgram(MiniMRCluster mr, MiniDFSCluster dfs, Path program,
			Path inputPath, Path outputPath, int numMaps, int numReduces,
		...
		for (Path p : FileUtil.stat2Paths(dfs.getFileSystem().listStatus(
				outputPath, new OutputLogFilter()))) {
			results.add(TestMiniMRWithDFS.readOutput(p, job));
		}
		assertEquals("number of reduces is wrong", expectedResults.length,
				results.size());
	...
	private void runNonPipedProgram(MiniMRCluster mr, MiniDFSCluster dfs,
		...
		Path testDir = new Path(
				"file:" + System.getProperty("test.build.data"), "pipes");
		...
		System.err.println("About to run: Submitter -conf " + jobXml
				+ " -input " + inDir + " -output " + outDir + " -program "
				+ dfs.getFileSystem().makeQualified(wordExec));
		try {
			int ret = ToolRunner.run(new Submitter(), new String[] { "-conf",
					jobXml.toString(), "-input", inDir.toString(), "-output",
					outDir.toString(), "-program",
					dfs.getFileSystem().makeQualified(wordExec).toString(),
					"-reduces", "2" });
			assertEquals(0, ret);
		} catch (Exception e) {
			assertTrue("got exception: " + StringUtils.stringifyException(e),
					false);
		}
	}
}
