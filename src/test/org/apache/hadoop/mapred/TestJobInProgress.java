...
public class TestJobInProgress extends TestCase {
	...
	JobTracker jt;
	private static Path TEST_DIR = new Path(System.getProperty(
			"test.build.data", "/tmp"), "jip-testing");
	...
	private void testRunningTaskCount(boolean speculation, boolean locality)
		...
		JobConf job = configure(UtilsForTests.WaitingMapper.class,
				IdentityReducer.class, 1, 1, locality);
		job.set(UtilsForTests.getTaskSignalParameter(true), mapSignalFile
				.toString());
		job.set(UtilsForTests.getTaskSignalParameter(false), redSignalFile
				.toString());
		...
		for (Map.Entry<Node, Set<TaskInProgress>> s : jip.getRunningMapCache()
				.entrySet()) {
		...
		assertEquals(
				"Running map count doesnt match for jobs with speculation "
						+ speculation + ", and locality " + locality, jip
						.runningMaps(), uniqueTasks.size());

		assertEquals(
				"Running reducer count doesnt match for jobs with speculation "
						+ speculation + ", and locality " + locality, jip
						.runningReduces(), jip.getRunningReduces().size());

		// signal the tasks
		LOG.info("Signaling the tasks");
		UtilsForTests.signalTasks(dfsCluster, dfsCluster.getFileSystem(),
				mapSignalFile.toString(), redSignalFile.toString(), numSlaves);
	...
	@SuppressWarnings("unchecked")
	JobConf configure(Class MapClass, Class ReduceClass, int maps,
			int reducers, boolean locality) throws Exception {
	...
	void checkTaskCounts() {
		...
		for (JobStatus js : status) {
			...
			long totalTaskCount = counter
					...
					+ counter
							.getCounter(JobInProgress.Counter.TOTAL_LAUNCHED_REDUCES);
