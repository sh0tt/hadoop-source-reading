...
public class TestSequenceFileInputFilter extends TestCase {
	...
	private static final Path inDir = new Path(System.getProperty(
			"test.build.data", ".")
			+ "/mapred");
	...
	private static void createSequenceFile(int numRecords) throws Exception {
		...
		SequenceFile.Writer writer = SequenceFile.createWriter(fs, conf,
				inFile, Text.class, BytesWritable.class);
		try {
	...
	private int countRecords(int numSplits) throws IOException {
		...
		if (numSplits == 0) {
			numSplits = random.nextInt(MAX_LENGTH
		...
		for (int j = 0; j < splits.length; j++) {
			RecordReader<Text, BytesWritable> reader = format.getRecordReader(
	...
	public void testRegexFilter() throws Exception {
		...
		for (int length = 1; length < MAX_LENGTH; length += random
