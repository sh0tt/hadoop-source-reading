...
public class TestMiniMRWithDFS extends TestCase {
	private static final Log LOG = LogFactory.getLog(TestMiniMRWithDFS.class
			.getName());
	...
	public static TestResult launchWordCount(JobConf conf, Path inDir,
			Path outDir, String input, int numMaps, int numReduces)
			throws IOException {
	...
	public static String readOutput(Path outDir, JobConf conf)
			throws IOException {
	 ...
	 */
	static void checkTaskDirectories(MiniMRCluster mr, String[] jobIds,
			String[] taskDirs) {
		mr.waitUntilIdle();
		...
		for (int i = 0; i < trackers; ++i) {
			...
			for (int j = 0; j < trackerContents.length; ++j) {
				System.out.println("Local jobcache " + trackerDir + ": "
						+ trackerContents[j]);
			}
	...
	public static void runPI(MiniMRCluster mr, JobConf jobconf)
		...
		assertTrue("Error in PI estimation " + error + " exceeds 0.01",
				(error < 0.01));
	...
	public static void runWordCount(MiniMRCluster mr, JobConf jobConf)
		...
		String pattern = TaskAttemptID.getTaskAttemptIDsPattern(null, null,
				true, 1, null);
		...
		long hdfsWrite = counters.findCounter(Task.FILESYSTEM_COUNTER_GROUP,
		...
		// default fs is hdfs.
		{
			...
			String TEST_ROOT_DIR = new File(System.getProperty(
					"test.build.data", "/tmp")).toString().replace(' ', '+');
			Path localIn = localfs.makeQualified(new Path(TEST_ROOT_DIR
					+ "/local/in"));
			Path localOut = localfs.makeQualified(new Path(TEST_ROOT_DIR
					+ "/local/out"));
	...
	public void testWithDFSWithDefaultPort() throws IOException {
		...
		try {
			...
			dfs = new MiniDFSCluster(NameNode.DEFAULT_PORT, conf, 4, true,
					true, null, null);
			fileSys = dfs.getFileSystem();
			mr = new MiniMRCluster(taskTrackers, fileSys.getUri().toString(), 1);
			...
			LOG.info(
					"Skip the test this time because can not start namenode on port "
