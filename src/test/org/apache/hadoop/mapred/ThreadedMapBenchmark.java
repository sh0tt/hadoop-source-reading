...
public class ThreadedMapBenchmark extends Configured implements Tool {
	...
	private static final Log LOG = LogFactory
			.getLog(ThreadedMapBenchmark.class);
	private static Path BASE_DIR = new Path(System.getProperty(
			"test.build.data", File.separator + "benchmarks" + File.separator
	...
	public static class Map extends MapReduceBase implements
		...
		public void configure(JobConf job) {
			...
			valueSizeRange = job.getInt("test.tmb.max_value", 10)
					- minValueSize;
		}
	}
	...
	public static void generateInputData(int dataSizePerMap,
			int numSpillsPerMap, int numMapsPerHost, JobConf masterConf)
			throws Exception {
		...
		job.set("test.tmb.bytes_per_map", String
				.valueOf(dataSizePerMap * 1024 * 1024));
	...
	public int run(String[] args) throws Exception {
		...
		try {
			...
			LOG.info("Total time taken : "
					+ String.valueOf(endTime - startTime) + " millisec");
			...
			ioSortMb = (int) Math.ceil(FACTOR
					* Math.ceil((double) dataSizePerMap / numSpillsPerMap));
			...
			LOG
					.info("Running sort with " + numSpillsPerMap
							+ " spills per map");
