...
public class TestSpecialCharactersInOutputPath extends TestCase {
	private static final Log LOG = LogFactory
			.getLog(TestSpecialCharactersInOutputPath.class.getName());

	private static final String OUTPUT_FILENAME = "result[0]";

	public static boolean launchJob(String fileSys, String jobTracker,
			JobConf conf, int numMaps, int numReduces) throws IOException {
		...
		try {
			...
			assertTrue("Output folder not found!", fs.exists(new Path(
					"/testing/output/" + OUTPUT_FILENAME)));
	...
	static class SpecialTextOutputFormat<K, V> extends TextOutputFormat<K, V> {
		...
		public RecordWriter<K, V> getRecordWriter(FileSystem ignored,
				...
				throws IOException {
			return super.getRecordWriter(ignored, job, OUTPUT_FILENAME,
					progress);
		}
	}
}
