...
public class TestEmptyJob extends TestCase {
	private static final Log LOG = LogFactory.getLog(TestEmptyJob.class
			.getName());
	...
	private boolean launchEmptyJob(URI fileSys, int numMaps, int numReduces)
		...
		JobInProgress job = mr.getJobTrackerRunner().getJobTracker().getJob(
				runningJob.getID());
		...
		// submit another job so that the map load increases and scheduling
		// happens
		...
		assertTrue("Number of job-setup tips is not 2!",
				jobSetupTasks.length == 2);
		...
		assertTrue("Number of maps is not zero!", jc
				.getMapTaskReports(runningJob.getID()).length == 0);
		assertTrue("Map progress is " + runningJob.mapProgress()
				+ " and not 1.0!", runningJob.mapProgress() == 1.0);
