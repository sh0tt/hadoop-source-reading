...
public class TestMiniMRMapRedDebugScript extends TestCase {
	private static final Log LOG = LogFactory
			.getLog(TestMiniMRMapRedDebugScript.class.getName());
	 ...
	 * 
	 * @param filter
	 *            Task log filter; can be STDOUT, STDERR, SYSLOG, DEBUGOUT,
	 *            DEBUGERR
	 * @param taskId
	 ...
	 * @throws IOException
	 */
	public String launchFailMapAndDebug(JobConf conf, Path inDir, Path outDir,
			Path debugDir, String debugScript, String input) throws IOException {
		...
		URI uri = debugFs.getUri().resolve(
				cachePath + "/testscript.txt#testscript");
	...
	public void testMapDebugScript() throws Exception {
		try {
			...
			String result = launchFailMapAndDebug(conf, inDir, outDir,
					debugDir, debugScript, input);
