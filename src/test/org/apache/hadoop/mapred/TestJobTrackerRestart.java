...
public class TestJobTrackerRestart extends TestCase {
	static final Path testDir = new Path(System.getProperty("test.build.data",
			"/tmp"), "jt-restart-testing");
	...
	private static JobConf[] getJobs(JobConf conf, JobPriority[] priorities,
			int[] numMaps, int[] numReds, Path outputDir, Path inDir,
			String mapSignalFile, String reduceSignalFile) throws IOException {
		JobConf[] jobs = new JobConf[priorities.length];
		for (int i = 0; i < jobs.length; ++i) {
			jobs[i] = new JobConf(conf);
			Path newOutputDir = outputDir.suffix(String
					.valueOf(numJobsSubmitted++));
	 ...
	 */
	private static void cleanUp(FileSystem fileSys, Path dir)
			throws IOException {
	 ...
	 */
	public void testRestartWithoutRecovery(MiniDFSCluster dfs, MiniMRCluster mr)
			throws IOException {
		...
		JobConf newConf = getJobs(mr.createJobConf(),
				new JobPriority[] { JobPriority.NORMAL }, new int[] { 2 },
				new int[] { 0 }, outputDir, inDir, getMapSignalFile(shareDir),
		...
		// Signal the tasks
		UtilsForTests.signalTasks(dfs, fileSys, true,
				getMapSignalFile(shareDir), getReduceSignalFile(shareDir));
	...
	public void testTaskEventsAndReportsWithRecovery(MiniDFSCluster dfs,
			MiniMRCluster mr) throws IOException {
		...
		JobConf newConf = getJobs(mr.createJobConf(),
				...
				new int[] { numMaps }, new int[] { numReducers }, outputDir,
				inDir, getMapSignalFile(shareDir),
				getReduceSignalFile(shareDir))[0];
		...
		while (true) {
			...
			// 50%
			TaskCompletionEvent[] trackerEvents = mr
					.getMapTaskCompletionEventsUpdates(0, id, numMaps)
		...
		TaskCompletionEvent[] prevEvents = mr.getTaskCompletionEvents(id, 0,
				numMaps);
		...
		TaskCompletionEvent[] jtEvents = mr.getTaskCompletionEvents(id, 0,
				2 * numMaps);
		...
		assertEquals("Job priority change is not reflected", JobPriority.HIGH,
				mr.getJobPriority(id));

		List<TaskCompletionEvent> jtMapEvents = new ArrayList<TaskCompletionEvent>();
		...
		while (true) {
			...
			trackerEvents = mr.getMapTaskCompletionEventsUpdates(0, id,
					jtMapEvents.size()).getMapTaskCompletionEvents();
		...
		UtilsForTests.signalTasks(dfs, fileSys, false,
				getMapSignalFile(shareDir), getReduceSignalFile(shareDir));

		UtilsForTests.waitTillDone(jobClient);

		testTaskCompletionEvents(jtMapEvents
				.toArray(new TaskCompletionEvent[0]), trackerEvents, true, -1);

		// validate the history file
		TestJobHistory.validateJobHistoryFileFormat(id, newConf, "SUCCESS",
				true);
		TestJobHistory.validateJobHistoryFileContent(mr, job, newConf);

		// check if the cluster status is insane
		ClusterStatus status = jobClient.getClusterStatus();
		assertTrue("Cluster status is insane", checkClusterStatusOnCompletion(
				status, prevStatus));
	}
	...
	private void testJobHistoryFiles(JobID id, JobConf conf) throws IOException {
		...
		String tempLogFileName = JobHistory.JobInfo
				.getSecondaryJobHistoryFile(logFileName);
		...
		Path tempLogFile = JobHistory.JobInfo.getJobHistoryLogLocationForUser(
				tempLogFileName, conf);
		assertFalse("User temporary log file exists", fileSys
				.exists(tempLogFile));
		...
		// Check if the temporary file is deleted
		tempLogFile = JobHistory.JobInfo
				.getJobHistoryLogLocation(tempLogFileName);
	...
	private void testTaskReports(TaskReport[] source, TaskReport[] target,
		...
		for (int i = 0; i < numToMatch; ++i) {
			...
			assertTrue("Task reports for same attempt has changed", source[i]
					.equals(target[i]));
		}
	}
	...
	private void testTaskCompletionEvents(TaskCompletionEvent[] source,
			TaskCompletionEvent[] target, boolean fullMatch, int numToMatch) {
		// Check if the event list size matches
		// The lengths should match only incase of full match
		if (fullMatch) {
			assertEquals("Map task completion events mismatch", source.length,
					target.length);
			numToMatch = source.length;
		}
		// Check if the events match
		for (int i = 0; i < numToMatch; ++i) {
			if (source[i].getTaskAttemptId().equals(
					target[i].getTaskAttemptId())) {
	...
	private boolean checkClusterStatusOnCompletion(ClusterStatus status,
		...
		return status.getJobTrackerState() == prevStatus.getJobTrackerState()
				&& status.getMapTasks() == 0 && status.getReduceTasks() == 0;
	}
	 ...
	 * jobhistory file. Preparation : - Configure a job with - num-maps : 0
	 * (long waiting setup) - num-reducers : 0
	 * 
	...
	public void testJobRecoveryWithEmptyHistory(MiniDFSCluster dfs,
		...
		RunningJob job1 = UtilsForTests.runJob(conf, inDir, outputDir, 30, 0);
		...
		JobConf newConf = getJobs(mr.createJobConf(),
				new JobPriority[] { JobPriority.NORMAL }, new int[] { 10 },
				new int[] { 0 }, outDir2, inDir2, getMapSignalFile(shareDir),
				getReduceSignalFile(shareDir))[0];
		...
		/*
		 * RunningJob job2 = UtilsForTests.runJob(mr.createJobConf(), inDir2,
		 * outDir2, 0);
		 * 
		 * JobID id = job2.getID();
		 */
		...
		String history = JobHistory.JobInfo.getJobHistoryFileName(jip
				.getJobConf(), id);
	...
	public void testJobTrackerRestart() throws IOException {
		...
		try {
			...
			UtilsForTests.writeFile(dfs.getNameNode(), conf, new Path(inDir
					+ "/file"), (short) 1);
			...
			jtConf
					.set("mapred.queue.default.acl-submit-job", ugi
							.getUserName());
