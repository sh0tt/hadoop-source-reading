...
public class TestCounters extends TestCase {
	...
	private void testCounter(Counters counter) throws ParseException {
		...
		Counters recoveredCounter = Counters
				.fromEscapedCompactString(compactEscapedString);
		// Check for recovery from string
		assertEquals("Recovered counter does not match on content", counter,
				recoveredCounter);
		assertEquals("recovered counter has wrong hash code", counter
				.hashCode(), recoveredCounter.hashCode());
	}
