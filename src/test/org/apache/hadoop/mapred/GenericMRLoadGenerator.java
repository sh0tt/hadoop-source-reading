...
public class GenericMRLoadGenerator extends Configured implements Tool {
	...
	protected static int printUsage() {
		System.err
				.println("Usage: [-m <maps>] [-r <reduces>]\n"
	 ...
	 */
	public static boolean parseArgs(String[] argv, JobConf job)
			throws IOException {
		if (argv.length < 1) {
			return 0 == printUsage();
		}
		for (int i = 0; i < argv.length; ++i) {
			...
			try {
				if ("-m".equals(argv[i])) {
					...
					job.setInputFormat(Class.forName(argv[++i]).asSubclass(
							InputFormat.class));
				} else if ("-outFormat".equals(argv[i])) {
					job.setOutputFormat(Class.forName(argv[++i]).asSubclass(
							OutputFormat.class));
				} else if ("-outKey".equals(argv[i])) {
					job.setOutputKeyClass(Class.forName(argv[++i]).asSubclass(
							WritableComparable.class));
				} else if ("-outValue".equals(argv[i])) {
					job.setOutputValueClass(Class.forName(argv[++i])
					...
					job.setClass("mapred.indirect.input.format", Class.forName(
							argv[++i]).asSubclass(InputFormat.class),
							InputFormat.class);
	...
	public int run(String[] argv) throws Exception {
		...
		if (0 == FileInputFormat.getInputPaths(job).length) {
			...
			SequenceFile.Writer writer = SequenceFile.createWriter(sysdir
					.getFileSystem(job), job, indirInputFile,
	...
	public static void main(String[] argv) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new GenericMRLoadGenerator(), argv);
		System.exit(res);
	}
	...
	static class RandomInputFormat implements InputFormat {
		...
		public InputSplit[] getSplits(JobConf conf, int numSplits) {
			...
			for (int i = 0; i < numSplits; ++i) {
				splits[i] = new IndirectInputFormat.IndirectSplit(new Path(
						"ignore" + i), 1);
			}
			return splits;
		}
	...
	static class RandomMapOutput extends MapReduceBase implements
	...
	protected static void confRandom(JobConf job) throws IOException {
		...
		if (numMaps == 0 && totalBytesToWrite > 0) {
			numMaps = 1;
			job
					.setLong("test.randomtextwrite.bytes_per_map",
							totalBytesToWrite);
		}
	...
	static abstract class SampleMapReduceBase<K extends WritableComparable, V extends Writable>
	...
	public static class SampleReducer<K extends WritableComparable, V extends Writable>
		...
		public void configure(JobConf job) {
			setKeep(job.getFloat("hadoop.sort.reduce.keep.percent",
					(float) 100.0)
					/ (float) 100.0);
		}
	...
	static class IndirectInputFormat implements InputFormat {
		...
		public InputSplit[] getSplits(JobConf job, int numSplits)
			...
			ArrayList<IndirectSplit> splits = new ArrayList<IndirectSplit>(
					numSplits);
			LongWritable key = new LongWritable();
			Text value = new Text();
			for (SequenceFile.Reader sl = new SequenceFile.Reader(fs, src, job); sl
					.next(key, value);) {
				splits.add(new IndirectSplit(new Path(value.toString()), key
						.get()));
			}
		...
		public RecordReader getRecordReader(InputSplit split, JobConf job,
				Reporter reporter) throws IOException {
			InputFormat indirIF = (InputFormat) ReflectionUtils.newInstance(job
					.getClass("mapred.indirect.input.format",
							SequenceFileInputFormat.class), job);
			IndirectSplit is = ((IndirectSplit) split);
			return indirIF.getRecordReader(new FileSplit(is.getPath(), 0, is
					.getLength(), (String[]) null), job, reporter);
		}
	}
	 ...
	 */
	private static final String[] words = { "diurnalness", "Homoiousian",
