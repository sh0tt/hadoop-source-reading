...
public class TestJobHistoryParsing extends TestCase {
	...
	static class TestListener implements Listener {
		...
		public void handle(RecordTypes recType,
				Map<JobHistory.Keys, String> values) throws IOException {
	...
	public void testHistoryParsing() throws IOException {
		...
		JobHistory.log(historyWriter, RecordTypes.Job, new JobHistory.Keys[] {
				Keys.JOBTRACKERID, Keys.TRACKER_NAME, Keys.JOBNAME,
				Keys.JOBCONF, Keys.USER }, new String[] { value1, value2,
		...
		JobHistory.parseHistoryFromFS(historyLog.toString(), new TestListener(
				job), fs);
