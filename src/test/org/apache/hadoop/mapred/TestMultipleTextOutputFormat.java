...
public class TestMultipleTextOutputFormat extends TestCase {
	...
	private static Path workDir = new Path(new Path(new Path(System
			.getProperty("test.build.data", "."), "data"),
	...
	static class KeyBasedMultipleTextOutputFormat extends
		...
		protected String generateFileNameForKeyValue(Text key, Text v,
				String name) {
	...
	private static void test1(JobConf job) throws IOException {
		...
		RecordWriter<Text, Text> rw = theOutputFormat.getRecordWriter(fs, job,
				name, null);
		writeData(rw);
		rw.close(null);
	}
