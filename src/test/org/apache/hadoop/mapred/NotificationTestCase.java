...
public abstract class NotificationTestCase extends HadoopTestCase {
		...
		protected void doGet(HttpServletRequest req, HttpServletResponse res)
			...
			if (counter % 2 == 0) {
				stdPrintln((new Date()).toString()
						+ "Receiving First notification for ["
						+ req.getQueryString() + "], returning error");
				res.sendError(HttpServletResponse.SC_BAD_REQUEST,
						"forcing error");
			} else {
				stdPrintln((new Date()).toString()
						+ "Receiving Second notification for ["
						+ req.getQueryString() + "], returning OK");
	...
	public void testMR() throws Exception {
		...
		System.out.println(UtilsForTests.runJobKill(this.createJobConf(),
				inDir, outDir).getID());
		synchronized (Thread.currentThread()) {
			stdPrintln("Sleeping for 2 seconds to give time for retry");
			Thread.currentThread().sleep(2000);
		...
		System.out.println(UtilsForTests.runJobFail(this.createJobConf(),
				inDir, outDir).getID());
		synchronized (Thread.currentThread()) {
			stdPrintln("Sleeping for 2 seconds to give time for retry");
			Thread.currentThread().sleep(2000);
	...
	private String launchWordCount(JobConf conf, String input, int numMaps,
