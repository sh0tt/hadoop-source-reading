...
public class TestJobQueueTaskScheduler extends TestCase {
		...
		public Task obtainNewMapTask(final TaskTrackerStatus tts,
			...
			Task task = new MapTask("", attemptId, 0, "", new BytesWritable()) {
				...
				public String toString() {
					return String.format("%s on %s", getTaskID(), tts
							.getTrackerName());
				}
			};
	...
	static class FakeTaskTrackerManager implements TaskTrackerManager {
		...
		public ClusterStatus getClusterStatus() {
			...
			return new ClusterStatus(numTrackers, 0,
					JobTracker.TASKTRACKER_EXPIRY_INTERVAL, maps, reduces,
					numTrackers * maxMapTasksPerTracker, numTrackers
							* maxReduceTasksPerTracker,
	...
	static void submitJobs(FakeTaskTrackerManager taskTrackerManager,
			JobConf jobConf, int numJobs, int state) throws IOException {
		for (int i = 0; i < numJobs; i++) {
			JobInProgress job = new FakeJobInProgress(jobConf,
					taskTrackerManager);
	...
	public void testTaskNotAssignedWhenNoJobsArePresent() throws IOException {
		assertEquals(0, scheduler.assignTasks(
				tracker(taskTrackerManager, "tt1")).size());
	}
