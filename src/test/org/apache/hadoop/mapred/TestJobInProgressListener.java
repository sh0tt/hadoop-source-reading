...
public class TestJobInProgressListener extends TestCase {
	private static final Log LOG = LogFactory
			.getLog(TestJobInProgressListener.class);
	private final Path testDir = new Path("test-jip-listener-update");

	private static String TEST_ROOT_DIR = new File(System.getProperty(
			"test.build.data", "/tmp")).toURI().toString().replace(' ', '+');

	private JobConf configureJob(JobConf conf, int m, int r, Path inDir,
			Path outputDir, String mapSignalFile, String redSignalFile)
			throws IOException {
	...
	public void testJobQueueChanges() throws IOException {
		...
		// add the listener
		mr.getJobTrackerRunner().getJobTracker().addJobInProgressListener(
				myListener);

		// big blocking job
		Path outputDir = new Path(testDir, "output");
		Path newOutputDir = outputDir.suffix("0");
		JobConf job1 = configureJob(mr.createJobConf(), 10, 0, inDir,
				newOutputDir, mapSignalFile, redSignalFile);

		// short blocked job
		newOutputDir = outputDir.suffix("1");
		JobConf job2 = configureJob(mr.createJobConf(), 1, 0, inDir,
				newOutputDir, mapSignalFile, redSignalFile);
		...
		assertTrue("Priority change garbles the queue", myListener
				.getJobQueue().size() == 2);

		JobInProgress[] queue = myListener.getJobQueue().toArray(
				new JobInProgress[0]);
		...
		JobInProgress jip2 = mr.getJobTrackerRunner().getJobTracker().getJob(
				rJob2.getID());
		JobInProgress jip1 = mr.getJobTrackerRunner().getJobTracker().getJob(
				rJob1.getID());
		...
		JobStatusChangeEvent event = new JobStatusChangeEvent(jip2,
				EventType.START_TIME_CHANGED, prevStatus, newStatus);
		myListener.jobUpdated(event);

		// check if the queue is sane
		assertTrue("Start time change garbles the queue", myListener
				.getJobQueue().size() == 2);
		...
		assertTrue("Job completion garbles the queue", myListener.getJobQueue()
				.size() == 0);
	}
	...
	public static class MyListener extends JobInProgressListener {
		...
		public void jobUpdated(JobChangeEvent event) {
			LOG.info("Job " + event.getJobInProgress().getJobID().toString()
					+ " updated");
			...
			if (event instanceof JobStatusChangeEvent) {
				...
				if (statusEvent.getEventType() == EventType.RUN_STATE_CHANGED) {
					...
					if (jip.isComplete()) {
						LOG.info("Job " + jobId
								+ " deleted from the running queue");
						...
						// PREP->RUNNING
						LOG.info("Job " + jobId
								+ " deleted from the waiting queue");
						wjobs.remove(jip);
						jobs.add(jip);
					}
				}
	...
	public void testJobSuccess() throws Exception {
		...
		assertFalse("Missing event notification for a running job", myListener
				.contains(rJob.getID(), true));
	...
	/**
	 * This scheduler never schedules any task as it doesnt init any task. So
	 * all the jobs are queued forever.
	 */
	...
	public void testQueuedJobKill() throws Exception {
		...
		assertTrue("Missing event notification on submiting a job", myListener
				.contains(id, true));
