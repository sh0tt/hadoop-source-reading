...
public class TestJobHistoryVersion extends TestCase {
	...
	private static final String FILENAME = HOSTNAME + "_" + TIME + "_" + JOB
			+ "_" + USER + "_" + JOBNAME;
	...
	private static final Path TEST_DIR = new Path(System.getProperty(
			"test.build.data", "/tmp"), "test-history-version");
	private static final String DELIM = ".";

	/**
	 * Creates a job history file of a given specific version. This method
	 * should change if format/content of future versions of job history file
	 * changes.
	 */
	private void writeHistoryFile(FSDataOutputStream out, long version)
		...
		out.writeBytes("MapAttempt TASK_TYPE=\"MAP\" TASKID=\"" + TASK_ID
				...
				+ " START_TIME=\"" + TIME + "\"" + " HOSTNAME=\"" + HOSTNAME
				+ "\" " + delim);

		// Write the attempt finish line
		out.writeBytes("MapAttempt TASK_TYPE=\"MAP\" TASKID=\"" + TASK_ID
				+ "\"" + " TASK_ATTEMPT_ID=\"" + TASK_ATTEMPT_ID + "\""
				+ " FINISH_TIME=\"" + TIME + "\"" + " TASK_STATUS=\"SUCCESS\""
				+ " HOSTNAME=\"" + HOSTNAME + "\" " + delim);

		// Write the task finish line
		out.writeBytes("Task TASKID=\"" + TASK_ID + "\" TASK_TYPE=\"MAP\""
				+ " TASK_STATUS=\"SUCCESS\"" + " FINISH_TIME=\"" + TIME + "\""
		...
		out.writeBytes("Job JOBID=\"" + JOB + "\" FINISH_TIME=\"" + TIME + "\""
				...
				+ " FAILED_REDUCES=\"0\"" + " COUNTERS=\"" + counters + "\" "
				+ delim);
	...
	public void testJobHistoryVersion() throws IOException {
		...
		for (long version = 0; version <= JobHistory.VERSION; version++) {
			...
			DefaultJobHistoryParser.parseJobTasks(historyPath.toString(), job,
					fs);

			assertTrue(
					"Failed to parse jobhistory files of version " + version,
