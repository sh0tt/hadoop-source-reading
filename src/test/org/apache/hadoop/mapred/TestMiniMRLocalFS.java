...
public class TestMiniMRLocalFS extends TestCase {
	...
	public void testWithLocal() throws IOException, InterruptedException,
		...
		try {
			...
			TestResult ret = MRCaching.launchMRCache(TEST_ROOT_DIR
					+ "/wc/input", TEST_ROOT_DIR + "/wc/output", TEST_ROOT_DIR
					+ "/cachedir", job, "The quick brown fox\n"
					+ "has many silly\n" + "red fox sox\n");
			...
			assertEquals("number of map inputs", 3, counters
					.getCounter(Task.Counter.MAP_INPUT_RECORDS));
			assertEquals("number of reduce outputs", 9, counters
	...
	private static class MyInputFormat implements
			InputFormat<IntWritable, Text> {

		static final String[] data = new String[] { "crocodile pants",
				"aunt annie", "eggs", "finagle the agent", "bumble boat",
				"duck-dog", };
	...
	static class MyOutputFormat implements OutputFormat {
		static class MyRecordWriter implements RecordWriter<Object, Object> {
			...
			public MyRecordWriter(Path outputFile, JobConf job)
					throws IOException {
	...
	private void runSecondarySort(Configuration conf) throws IOException,
		...
		TestMapReduceLocal.writeFile("in/part1",
				"-1 -4\n-3 23\n5 10\n-1 -2\n-1 300\n-1 10\n4 1\n"
						+ "4 2\n4 10\n4 -1\n4 -10\n10 20\n10 30\n10 25\n");
