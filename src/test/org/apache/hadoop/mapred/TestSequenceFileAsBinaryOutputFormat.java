...
public class TestSequenceFileAsBinaryOutputFormat extends TestCase {
	private static final Log LOG = LogFactory
			.getLog(TestSequenceFileAsBinaryOutputFormat.class.getName());
	...
	public void testBinary() throws IOException {
		...
		Path dir = new Path(new Path(new Path(System.getProperty(
				"test.build.data", ".")), FileOutputCommitter.TEMP_DIR_NAME),
				"_" + attempt);
		...
		for (InputSplit split : iformat.getSplits(job, NUM_SPLITS)) {
			RecordReader<IntWritable, DoubleWritable> reader = iformat
			...
			try {
				...
				while (reader.next(iwritable, dwritable)) {
					...
					assertEquals("Keys don't match: " + "*" + iwritable.get()
							+ ":" + sourceInt + "*", sourceInt, iwritable.get());
					assertTrue("Vals don't match: " + "*" + dwritable.get()
							+ ":" + sourceDouble + "*", Double.compare(
							dwritable.get(), sourceDouble) == 0);
					++count;
				}
			} finally {
				reader.close();
	...
	public void testSequenceOutputClassDefaultsToMapRedOutputClass()
		...
		assertEquals("SequenceFileOutputKeyClass not updated",
				IntWritable.class, SequenceFileAsBinaryOutputFormat
						.getSequenceFileOutputKeyClass(job));
		assertEquals("SequenceFileOutputValueClass not updated",
				DoubleWritable.class, SequenceFileAsBinaryOutputFormat
						.getSequenceFileOutputValueClass(job));
	}
