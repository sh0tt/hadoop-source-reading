...
/**
 * This class tests reliability of the framework in the face of failures of both
 * tasks and tasktrackers. Steps: 1) Get the cluster status 2) Get the number of
 * slots in the cluster 3) Spawn a sleepjob that occupies the entire cluster
 * (with two waves of maps) 4) Get the list of running attempts for the job 5)
 * Fail a few of them 6) Now fail a few trackers (ssh) 7) Job should run to
 * completion 8) The above is repeated for the Sort suite of job (randomwriter,
 * sort, validator). All jobs must complete, and finally, the sort validation
 * should succeed. To run the test: ./bin/hadoop --config <config> jar
...
public class ReliabilityTest extends Configured implements Tool {
	...
	private void displayUsage() {
		LOG
				.info("This must be run in only the distributed mode "
	...
	public int run(String[] args) throws Exception {
		...
		// to protect against the case of jobs failing even when multiple
		// attempts
	...
	private void runSleepJobTest(final JobClient jc, final Configuration conf)
		...
		String[] sleepJobArgs = new String[] { "-m", Integer.toString(maxMaps),
				"-r", Integer.toString(maxReduces), "-mt",
				Integer.toString(mapSleepTime), "-rt",
				Integer.toString(reduceSleepTime) };
	...
	private void runRandomWriterTest(final JobClient jc,
			final Configuration conf, final String inputPath) throws Exception {
		runTest(jc, conf, "org.apache.hadoop.examples.RandomWriter",
				new String[] { inputPath }, null, new KillTrackerThread(jc, 0,
						0.4f, false, 1));
		LOG.info("RandomWriter job done");
	}

	private void runSortTest(final JobClient jc, final Configuration conf,
			final String inputPath, final String outputPath) throws Exception {
		runTest(jc, conf, "org.apache.hadoop.examples.Sort", new String[] {
				inputPath, outputPath }, new KillTaskThread(jc, 2, 0.2f, false,
				2), new KillTrackerThread(jc, 2, 0.8f, false, 1));
	...
	private void runSortValidatorTest(final JobClient jc,
		...
		runTest(jc, conf, "org.apache.hadoop.mapred.SortValidator",
				new String[] { "-sortInput", inputPath, "-sortOutput",
						outputPath },
	...
	private void runTest(final JobClient jc, final Configuration conf,
			final String jobClass, final String[] args,
			KillTaskThread killTaskThread, KillTrackerThread killTrackerThread)
			throws Exception {
		...
		Thread t = new Thread("Job Test") {
			public void run() {
				try {
					...
					int status = ToolRunner.run(conf, (Tool) (jobClassObj
							.newInstance()), args);
	...
	private class KillTrackerThread extends Thread {
		...
		final private String slavesFile = dir
				+ "/_reliability_test_slaves_file_";
			...
			while (!killed) {
				try {
					...
					if (progress >= thresholdVal) {
						...
						Thread.sleep((int) Math.ceil(1.5 * c
								.getTTExpiryInterval()));
						startTaskTrackers();
		...
		private void stopTaskTrackers(ClusterStatus c) throws Exception {
			...
			ArrayList<String> trackerNamesList = new ArrayList<String>(
					trackerNames);
					...
					if (progress >= thresholdVal) {
						...
						if (runningTasks.size() > c.getTaskTrackers() / 2) {
							...
							for (TaskAttemptID t : runningTasks) {
								...
								if (count++ > runningTasks.size() / 2) { // kill
									// 50%
									break;
								}
							}
	...
	public static void main(String args[]) throws Exception {
		int res = ToolRunner.run(new Configuration(), new ReliabilityTest(),
				args);
		System.exit(res);
	}
}