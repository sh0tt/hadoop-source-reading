...
public class TestDatamerge extends TestCase {
	...
	private static class InnerJoinChecker extends
			SimpleCheckerBase<TupleWritable> {
	...
	private static class OuterJoinChecker extends
			SimpleCheckerBase<TupleWritable> {
		public void map(IntWritable key, TupleWritable val,
				OutputCollector<IntWritable, IntWritable> out, Reporter reporter)
				throws IOException {
	...
	private static void joinAs(String jointype,
		...
		Path base = cluster.getFileSystem().makeQualified(
				new Path("/" + jointype));
	...
	public void testNestedJoin() throws Exception {
		...
		for (int i = 0; i < SOURCES; ++i) {
			sb.append(CompositeInputFormat.compose(
					SequenceFileInputFormat.class, src[i].toString()));
			if (i + 1 != SOURCES)
				sb.append(",");
		}
		sb.append("),outer(");
		...
		SequenceFile.Reader r = new SequenceFile.Reader(
				cluster.getFileSystem(), outlist[0].getPath(), job);
		...
		while (r.next(k, v)) {
			...
			for (int i = 2; i < SOURCES + 2; ++i) {
				if ((ki % i) == 0 && ki <= i * ITEMS) {
					assertEquals(i - 2, ((IntWritable) ((TupleWritable) v
							.get(1)).get((i - 1))).get());
				} else
					chk = false;
			}
	...
	public static class Fake_IF<K, V> implements InputFormat<K, V>,
			JobConfigurable {
		...
		public void configure(JobConf job) {
			keyclass = (Class<? extends K>) job.getClass(
					"test.fakeif.keyclass", IncomparableKey.class,
					WritableComparable.class);
			valclass = (Class<? extends V>) job.getClass(
					"test.fakeif.valclass", NullWritable.class,
					WritableComparable.class);
		}
		...
		public RecordReader<K, V> getRecordReader(InputSplit ignored,
