...
public class TestTupleWritable extends TestCase {
	...
	private TupleWritable makeTuple(Writable[] writs) {
		...
		Writable[] vals = { writs[0], new TupleWritable(sub1),
				new TupleWritable(sub2), writs[7], writs[8], writs[9] };
	...
	public void testIterable() throws Exception {
		Random r = new Random();
		Writable[] writs = { new BooleanWritable(r.nextBoolean()),
				new FloatWritable(r.nextFloat()),
				new FloatWritable(r.nextFloat()), new IntWritable(r.nextInt()),
				new LongWritable(r.nextLong()),
				new BytesWritable("dingo".getBytes()),
				new LongWritable(r.nextLong()), new IntWritable(r.nextInt()),
	...
	public void testWideTuple() throws Exception {
		...
		for (int pos = 0; pos < tuple.size(); pos++) {
			...
			if (pos == 42) {
				...
				assertFalse("Tuple position is incorrectly labelled as set: "
						+ pos, has);
			}
		}
	}
