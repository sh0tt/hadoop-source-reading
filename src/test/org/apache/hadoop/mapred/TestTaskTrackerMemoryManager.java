...
public class TestTaskTrackerMemoryManager extends TestCase {
	...
	private static final Log LOG = LogFactory
			.getLog(TestTaskTrackerMemoryManager.class);
	private static String TEST_ROOT_DIR = new Path(System.getProperty(
			"test.build.data", "/tmp")).toString().replace(' ', '+');

	private MiniMRCluster miniMRCluster;

	private String taskOverLimitPatternString = "TaskTree \\[pid=[0-9]*,tipID=.*\\] is running beyond memory-limits. "
	...
	private void runAndCheckSuccessfulJob(JobConf conf) throws IOException {
		Pattern taskOverLimitPattern = Pattern.compile(String.format(
				taskOverLimitPatternString, "[0-9]*"));
		Matcher mat = null;
		...
		for (TaskCompletionEvent tce : taskComplEvents) {
			String[] diagnostics = rj
					.getTaskDiagnostics(tce.getTaskAttemptId());
	...
	private boolean isProcfsBasedTreeAvailable() {
		try {
			if (!ProcfsBasedProcessTree.isAvailable()) {
				LOG
						.info("Currently ProcessTree has only one implementation "
	...
	/**
	 * Test for verifying that tasks within limits, with the cumulative usage
	 * also under TT's limits succeed.
	 * 
	 * @throws Exception
	 */
	public void testTasksWithinLimits() throws Exception {
		...
		fConf.setLong(JobTracker.MAPRED_CLUSTER_MAP_MEMORY_MB_PROPERTY,
				2 * 1024L);
		fConf.setLong(JobTracker.MAPRED_CLUSTER_REDUCE_MEMORY_MB_PROPERTY,
				2 * 1024L);
		startCluster(new JobConf());
	...
	private void runJobExceedingMemoryLimit() throws IOException {
		...
		Pattern taskOverLimitPattern = Pattern.compile(String.format(
				taskOverLimitPatternString, String
	...
	public void testTasksCumulativelyExceedingTTLimits() throws Exception {
		...
		// Start cluster with proper configuration.
		JobConf fConf = new JobConf();
		fConf.setLong(JobTracker.MAPRED_CLUSTER_MAP_MEMORY_MB_PROPERTY, 1L);
		fConf.setLong(JobTracker.MAPRED_CLUSTER_REDUCE_MEMORY_MB_PROPERTY, 1L);
		...
		Pattern taskOverLimitPattern = Pattern.compile(String.format(
				taskOverLimitPatternString, String.valueOf(PER_TASK_LIMIT)));

		Pattern trackerOverLimitPattern = Pattern
		...
		// Start the job
		RunningJob job = jClient.submitJob(sleepJob.setupJobConf(1, 1, 5000, 1,
				1000, 1));
		...
		while (true) {
			...
			allTaskReports.addAll(Arrays.asList(jClient
					.getSetupTaskReports((org.apache.hadoop.mapred.JobID) job
							.getID())));
			allTaskReports.addAll(Arrays.asList(jClient
					.getMapTaskReports((org.apache.hadoop.mapred.JobID) job
							.getID())));
		...
		// If it comes here without a test-timeout, it means there was a task
		// that
	...
	public void testProcessTreeLimits() throws IOException {
		...
		try {
			...
			procs[6] = new TestProcfsBasedProcessTree.ProcessStatInfo(
					new String[] { "700", "proc7", "600", "600", "600",
							"100000" });
			// write stat files.
			TestProcfsBasedProcessTree.writeStatFiles(procfsRootDir, pids,
					procs);
			...
			TaskMemoryManagerThread test = new TaskMemoryManagerThread(
					1000000L, 5000L, 5000L);
			// create process trees
			// tree rooted at 100 is over limit immediately, as it is
			// twice over the mem limit.
			ProcfsBasedProcessTree pTree = new ProcfsBasedProcessTree("100",
					procfsRootDir.getAbsolutePath());
			pTree.getProcessTree();
			assertTrue("tree rooted at 100 should be over limit "
					+ "after first iteration.", test.isProcessTreeOverLimit(
					pTree, "dummyId", limit));

			// the tree rooted at 200 is initially below limit.
			pTree = new ProcfsBasedProcessTree("200", procfsRootDir
					.getAbsolutePath());
			pTree.getProcessTree();
			assertFalse("tree rooted at 200 shouldn't be over limit "
					+ "after one iteration.", test.isProcessTreeOverLimit(
					pTree, "dummyId", limit));
			...
			pTree = new ProcfsBasedProcessTree("600", procfsRootDir
					.getAbsolutePath());
			pTree.getProcessTree();
			assertFalse("tree rooted at 600 should never be over limit.", test
					.isProcessTreeOverLimit(pTree, "dummyId", limit));
