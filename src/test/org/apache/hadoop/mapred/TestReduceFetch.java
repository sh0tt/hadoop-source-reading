...
public class TestReduceFetch extends TestCase {
	...
	public static Test suite() {
		TestSetup setup = new TestSetup(new TestSuite(TestReduceFetch.class)) {
			protected void setUp() throws Exception {
				...
				mrCluster = new MiniMRCluster(2, dfsCluster.getFileSystem()
						.getUri().toString(), 1);
			}
	...
	public void testReduceFromDisk() throws Exception {
		...
		final long spill = c.findCounter(Task.Counter.SPILLED_RECORDS)
				.getCounter();
		final long out = c.findCounter(Task.Counter.MAP_OUTPUT_RECORDS)
				.getCounter();
		assertTrue(
				"Expected all records spilled during reduce (" + spill + ")",
				spill >= 2 * out); // all records spill at map, reduce
		assertTrue("Expected intermediate merges (" + spill + ")", spill >= 2
				* out + (out / MAP_TASKS)); // some records hit twice
	}
	...
	public void testReduceFromPartialMem() throws Exception {
		...
		assertTrue("Expected some records not spilled during reduce" + spill
				+ ")", spill < 2 * out); // spilled map records, some records at
		// the reduce
	}
	...
	public void testReduceFromMem() throws Exception {
		...
		final long out = c.findCounter(Task.Counter.MAP_OUTPUT_RECORDS)
				.getCounter();
		assertEquals("Spilled records: " + spill, out, spill); // no reduce
		// spill
	}
