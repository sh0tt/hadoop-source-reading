...
public class TestSequenceFileAsBinaryInputFormat extends TestCase {
	...
	public void testBinary() throws IOException {
		...
		SequenceFile.Writer writer = new SequenceFile.Writer(fs, job, file,
		...
		for (InputSplit split : bformat.getSplits(job, NUM_SPLITS)) {
			RecordReader<BytesWritable, BytesWritable> reader = bformat
			...
			try {
				while (reader.next(bkey, bval)) {
					...
					assertTrue("Keys don't match: " + "*" + cmpkey.toString()
							+ ":" + tkey.toString() + "*", cmpkey.toString()
							.equals(tkey.toString()));
					assertTrue("Vals don't match: " + "*" + cmpval.toString()
							+ ":" + tval.toString() + "*", cmpval.toString()
							.equals(tval.toString()));
					++count;
				}
			} finally {
				reader.close();
