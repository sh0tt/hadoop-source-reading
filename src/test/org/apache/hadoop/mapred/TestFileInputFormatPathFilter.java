...
public class TestFileInputFormatPathFilter extends TestCase {
	...
	private static Path workDir = new Path(new Path(System.getProperty(
			"test.build.data", "."), "data"), "TestFileInputFormatPathFilter");
	...
	public static class TestPathFilter implements PathFilter {
		...
		public boolean accept(Path path) {
			...
			return name.equals("TestFileInputFormatPathFilter")
					|| name.length() == 1;
		}
	}

	private void _testInputFiles(boolean withFilter, boolean withGlob)
			throws Exception {
		...
		DummyFileInputFormat inputFormat = (DummyFileInputFormat) conf
				.getInputFormat();
