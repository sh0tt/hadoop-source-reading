...
public class TestQueueManager extends TestCase {
	...
	public void testSubmissionToInvalidQueue() throws IOException {
		...
		String queueName = "q1";
		try {
			RunningJob rjob = submitSleepJob(1, 1, 100, 100, true, null,
					queueName);
	...
	public void testUserEnabledACLForJobSubmission() throws IOException,
			LoginException {
		// login as self...
		UserGroupInformation ugi = UnixUserGroupInformation.login();
		String userName = ugi.getUserName();
	...
	public void testGroupsEnabledACLForJobSubmission() throws IOException,
			LoginException {
		...
		JobConf conf = setupConf("mapred.queue.default.acl-submit-job",
				"3698-junk-user1,3698-junk-user2 " + groups[groups.length - 1]
						+ ",3698-junk-group");
		verifyJobSubmission(conf, true);
	}
	...
	private void verifyJobSubmission(JobConf conf, boolean shouldSucceed,
		...
		try {
			...
			if (shouldSucceed) {
				...
				assertTrue(ioe.getMessage().contains(
						"cannot perform operation " + "SUBMIT_JOB on queue "
								+ queue));
				// check if the system directory gets cleaned up or not
				JobTracker jobtracker = miniMRCluster.getJobTrackerRunner()
						.getJobTracker();
				...
				while (size > 1) { // ignore the jobtracker.info file
					System.out
							.println("Waiting for the job files in sys directory to be cleaned up");
	...
	private void verifyJobKillAsOtherUser(JobConf conf, boolean shouldSucceed,
			String otherUserInfo) throws IOException {
		setUpCluster(conf);
		try {
	...
	private void verifyJobPriorityChangeAsOtherUser(JobConf conf,
			boolean shouldSucceed, String otherUserInfo) throws IOException {
		setUpCluster(conf);
		try {
	...
	private RunningJob submitSleepJob(int numMappers, int numReducers,
			long mapSleepTime, long reduceSleepTime, boolean shouldComplete)
			throws IOException {
	...
	private RunningJob submitSleepJob(int numMappers, int numReducers,
			long mapSleepTime, long reduceSleepTime, boolean shouldComplete,
			String userInfo, String queueName) throws IOException {
		JobConf clientConf = new JobConf();
		clientConf.set("mapred.job.tracker", "localhost:"
				+ miniMRCluster.getJobTrackerPort());
		SleepJob job = new SleepJob();
		job.setConf(clientConf);
		clientConf = job.setupJobConf(numMappers, numReducers, mapSleepTime,
				(int) mapSleepTime / 100, reduceSleepTime,
