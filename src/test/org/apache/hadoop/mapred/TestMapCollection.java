...
public class TestMapCollection extends TestCase {
	...
	private static final Log LOG = LogFactory.getLog(TestMapCollection.class
			.getName());
	...
	public static class KeyWritable implements WritableComparable<KeyWritable>,
		...
		public static class Comparator extends WritableComparator {
			...
			public int compare(byte[] b1, int s1, int l1, byte[] b2, int s2,
				...
				if (pedantic) {
					for (int i = s1; i < l1; ++i) {
						assertEquals("Invalid key at " + s1, b1[i],
								(byte) ('K' & 0xFF));
					}
					for (int i = s2; i < l2; ++i) {
						assertEquals("Invalid key at " + s2, b2[i],
								(byte) ('K' & 0xFF));
					}
				}
				return l1 - l2;
	...
	public static class SpillMapper implements
			Mapper<NullWritable, NullWritable, KeyWritable, ValWritable> {
	...
	public static class SpillReducer implements
			Reducer<KeyWritable, ValWritable, NullWritable, NullWritable> {
		...
		public void reduce(KeyWritable k, Iterator<ValWritable> values,
			...
			assertEquals("Unexpected record count (" + i + "/" + numrecs + ")",
					numrecs, i);
		}
	...
	public static class FakeIF implements
	...
	private static void runTest(String name, int keylen, int vallen,
			int records, boolean pedantic) throws Exception {
