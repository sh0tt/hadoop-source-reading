...
public class RecordBench {
	...
	private static Record[] makeArray(String type, int numRecords, Times times) {
		...
		try {
			init = RecordBench.class.getDeclaredMethod("init"
					+ toCamelCase(type) + "s", new Class[] { Record[].class });
	...
	private static void printTimes(String type, String format, int numRecords,
			Times times) {
		...
		if (times.write != 0) {
			System.out.println("Write Time (Per Record) : " + times.write
					/ numRecords + " Nanoseconds");
		}
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			bench = RecordBench.class.getDeclaredMethod("run"
					+ toCamelCase(format) + "Bench", new Class[] {
