...
public class TestPermission extends TestCase {
	...
	{
		((Log4JLogger) UserGroupInformation.LOG).getLogger()
				.setLevel(Level.ALL);
	}
	...
	public void testCreate() throws Exception {
		...
		try {
			...
			FSDataOutputStream out = fs.create(new Path("/b1/b2/b3.txt"),
					filePerm, true, conf.getInt("io.file.buffer.size", 4096),
			...
			FsPermission permission = FsPermission
					.createImmutable((short) 0666);
			FileSystem
					.mkdirs(fs, new Path("/c1"), new FsPermission(permission));
			FileSystem.create(fs, new Path("/c1/c2.txt"), new FsPermission(
					permission));
