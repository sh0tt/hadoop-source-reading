...
public class TestConfiguredPolicy extends TestCase {
	...
	private static class TestPolicyProvider extends PolicyProvider {
		...
		public Service[] getServices() {
			return new Service[] { new Service(KEY_1, Protocol1.class),
	...
	public void testConfiguredPolicy() throws Exception {
		...
		ConfiguredPolicy policy = new ConfiguredPolicy(conf,
				new TestPolicyProvider());
		SecurityUtil.setPolicy(policy);

		Subject user1 = SecurityUtil.getSubject(new UnixUserGroupInformation(
				USER1, GROUPS1));
		...
		// Should fail
		Subject user2 = SecurityUtil.getSubject(new UnixUserGroupInformation(
				USER2, GROUPS2));
