...
public class TestCLI extends TestCase {
	...
	public static final String TEST_CACHE_DATA_DIR = System.getProperty(
			"test.cache.data", "build/test/cache");
	...
	private void readTestConfigFile() {
		...
		if (testsFromConfigFile == null) {
			...
			testConfigFile = TEST_CACHE_DATA_DIR + File.separator
					+ testConfigFile;
			try {
	...
	public void setUp() throws Exception {
		...
		clitestDataDir = new File(TEST_CACHE_DATA_DIR).toURI().toString()
				.replace(' ', '+');
		...
		mrCluster = new MiniMRCluster(1, dfsCluster.getFileSystem().getUri()
				.toString(), 1, null, null, mrConf);
		jobtracker = mrCluster.createJobConf().get("mapred.job.tracker",
				"local");
	...
	private void displayResults() {
		...
		for (int i = 0; i < testsFromConfigFile.size(); i++) {
			CLITestData td = testsFromConfigFile.get(i);
			totalComparators += testsFromConfigFile.get(i).getComparatorData()
					.size();
		...
		if (testMode.equals(TESTMODE_TEST)) {
			try {
				...
				comparatorClass = Class.forName("org.apache.hadoop.cli.util."
						+ comparatorType);
				ComparatorBase comp = (ComparatorBase) comparatorClass
						.newInstance();
				compareOutput = comp.compare(CommandExecutor
						.getLastCommandOutput(), compdata.getExpectedOutput());
			} catch (Exception e) {
		...
		public void endElement(String uri, String localName, String qName)
			...
			if (qName.equals("description")) {
				...
				if (testCommands != null) {
					...
					cleanupCommands
							.add(new TestCmd(charString, CommandType.FS));
				}
			} else if (qName.equals("dfs-admin-command")) {
				if (testCommands != null) {
					testCommands.add(new TestCmd(charString,
							CommandType.DFSADMIN));
				} else if (cleanupCommands != null) {
					cleanupCommands.add(new TestCmd(charString,
							CommandType.DFSADMIN));
				}
