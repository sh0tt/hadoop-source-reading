...
public class CommandExecutor {
	...
	private static String[] getCommandAsArgs(final String cmd,
			final String masterKey, final String master) {
		...
		while (tokenizer.hasMoreTokens()) {
			...
			args[i] = args[i].replaceAll("CLITEST_DATA", new File(
					TestCLI.TEST_CACHE_DATA_DIR).toURI().toString().replace(
					' ', '+'));
			args[i] = args[i].replaceAll("USERNAME", System
	...
	public static int executeCommand(final TestCmd cmd, final String namenode,
			final String jobtracker) throws Exception {
		switch (cmd.getType()) {
		case DFSADMIN:
			return CommandExecutor.executeDFSAdminCommand(cmd.getCmd(),
					namenode);
		case MRADMIN:
			return CommandExecutor.executeMRAdminCommand(cmd.getCmd(),
					jobtracker);
		case FS:
	...
	public static int executeDFSAdminCommand(final String cmd,
