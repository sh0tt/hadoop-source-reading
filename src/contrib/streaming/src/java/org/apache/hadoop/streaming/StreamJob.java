...
public class StreamJob implements Tool {
	...
	/**
	 * @deprecated use StreamJob() with ToolRunner or set the Configuration
	 *             using {@link #setConf(Configuration)} and run with
	 *             {@link #run(String[])}.
	 */
	@Deprecated
	...
	private void setupOptions() {
		...
		allOptions = new Options().addOption(input).addOption(output)
				...
				.addOption(file).addOption(dfs).addOption(jt).addOption(
						additionalconfspec).addOption(inputformat).addOption(
						outputformat).addOption(partitioner).addOption(
						numReduceTasks).addOption(inputreader).addOption(
						mapDebug).addOption(reduceDebug).addOption(jobconf)
				.addOption(cmdenv).addOption(cacheFile).addOption(cacheArchive)
				.addOption(verbose).addOption(info).addOption(debug).addOption(
						inputtagged).addOption(help);
	}
	...
	protected void setJobConf() throws IOException {
		if (additionalConfSpec_ != null) {
			LOG
					.warn("-additionalconfspec option is deprecated, please use -conf instead.");
		...
		if (mapCmd_ != null) {
			...
			if (c != null) {
				...
				jobConf_.set("stream.map.streamprocessor", URLEncoder.encode(
						mapCmd_, "UTF-8"));
			}
		}
		...
		if (redCmd_ != null) {
			...
			if (redCmd_.compareToIgnoreCase("aggregate") == 0) {
				...
				if (c != null) {
					...
					jobConf_.set("stream.reduce.streamprocessor", URLEncoder
							.encode(redCmd_, "UTF-8"));
				}
			}
		}
