...
public abstract class PipeMapRed {
	...
	protected static final Log LOG = LogFactory.getLog(PipeMapRed.class
			.getName());
	...
	public void configure(JobConf job) {
		try {
			...
			clientOut_ = new DataOutputStream(new BufferedOutputStream(sim
					.getOutputStream(), BUFFER_SIZE));
			clientIn_ = new DataInputStream(new BufferedInputStream(sim
					.getInputStream(), BUFFER_SIZE));
			clientErr_ = new DataInputStream(new BufferedInputStream(sim
					.getErrorStream()));
		...
		try {
			...
			if (exitVal != 0) {
				if (nonZeroExitIsFailure_) {
					...
					logprintln("PipeMapRed.waitOutputThreads(): subprocess exited with code "
							+ exitVal + " in " + PipeMapRed.class.getName());
				}
			}
			if (outThread_ != null) {
	...
	class MRErrorThread extends Thread {
		...
		private void incrCounter(String line) {
			...
			if (columns.length == 3) {
				try {
					reporter.incrCounter(columns[0], columns[1], Long
							.parseLong(columns[2]));
