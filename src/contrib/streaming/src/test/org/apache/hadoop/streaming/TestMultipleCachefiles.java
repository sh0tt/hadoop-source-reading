...
public class TestMultipleCachefiles extends TestCase {
	...
	public void testMultipleCachefiles() throws Exception {
		...
		try {
			...
			String strJobtracker = "mapred.job.tracker=" + "localhost:"
			...
			String argv[] = new String[] {
					...
					"-cacheFile",
					"hdfs://" + fileSys.getName() + CACHE_FILE + "#"
							+ mapString,
					"-cacheFile",
					"hdfs://" + fileSys.getName() + CACHE_FILE_2 + "#"
							+ mapString2 };
			...
			String line = null;
			String line2 = null;
			Path[] fileList = FileUtil.stat2Paths(fileSys.listStatus(new Path(
					OUTPUT_DIR), new OutputLogFilter()));
			for (int i = 0; i < fileList.length; i++) {
				System.out.println(fileList[i].toString());
				BufferedReader bread = new BufferedReader(
						new InputStreamReader(fileSys.open(fileList[i])));
				line = bread.readLine();
