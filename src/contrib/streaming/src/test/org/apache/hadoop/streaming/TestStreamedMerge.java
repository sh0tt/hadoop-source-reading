...
public class TestStreamedMerge extends TestCase {
	...
	void printSampleInput() throws IOException {
		...
		String content = StreamUtil
				.slurpHadoop(new Path("/input/part-00"), fs_);
		System.out.println(content);
		System.out.println("cat done.");
	}

	void callStreaming(String argSideOutput, boolean inputTagged)
			throws IOException {
		String[] testargs = new String[] {
				...
				"stream.testmerge=1",
				"-input",
				"+/input/part-00 | /input/part-01 | /input/part-02",
				"-mapper",
				StreamUtil.localizeBin("/bin/cat"),
				"-reducer",
				"NONE",
				"-output",
				"/my.output",
				...
				argSideOutput,
				"-dfs",
				conf_.get("fs.default.name"),
				"-jt",
				"local",
				"-jobconf",
				"stream.sideoutput.localfs=true",
				"-jobconf",
				"stream.tmpdir="
	...
	public void testMain() throws Exception {
		...
		String overrideFS = StreamUtil.getBoundAntProperty("fs.default.name",
				null);
	...
	void goSocketTagged(boolean socket, boolean inputTagged) throws Exception {
		System.out.println("***** goSocketTagged: " + socket + ", "
				+ inputTagged);
