...
public class TestStreamingSeparator extends TestCase {
	...
	protected String map = StreamUtil.makeJavaCommand(TrApp.class,
			new String[] { "2", "3" });
	...
	protected String reduce = StreamUtil.makeJavaCommand(TrAppReduce.class,
			new String[] { "3", "4" });
	...
	protected String[] genArgs() {
		return new String[] {
				...
				"-inputformat", "KeyValueTextInputFormat", "-jobconf",
				"key.value.separator.in.input.line=1", "-jobconf",
				"stream.map.input.field.separator=2", "-jobconf",
				"stream.map.output.field.separator=3", "-jobconf",
				"stream.reduce.input.field.separator=3", "-jobconf",
				"stream.reduce.output.field.separator=4", "-jobconf",
				"mapred.textoutputformat.separator=5", };
	}
	...
	public void testCommandLine() throws Exception {
		try {
			...
			File outFileCRC = new File(OUTPUT_DIR, ".part-00000.crc")
					.getAbsoluteFile();
			INPUT_FILE.delete();
			outFileCRC.delete();
			OUTPUT_DIR.getAbsoluteFile().delete();
