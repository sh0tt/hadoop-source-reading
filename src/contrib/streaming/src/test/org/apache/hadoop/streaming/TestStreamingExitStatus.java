...
public class TestStreamingExitStatus extends TestCase {
	protected File TEST_DIR = new File("TestStreamingExitStatus")
			.getAbsoluteFile();

	protected File INPUT_FILE = new File(TEST_DIR, "input.txt");
	protected File OUTPUT_DIR = new File(TEST_DIR, "out");

	protected String failingTask = StreamUtil.makeJavaCommand(FailApp.class,
			new String[] { "true" });
	protected String echoTask = StreamUtil.makeJavaCommand(FailApp.class,
			new String[] { "false" });
	...
	protected String[] genArgs(boolean exitStatusIsFailure, boolean failMap) {
		return new String[] {
				...
				"-mapper",
				(failMap ? failingTask : echoTask),
				"-reducer",
				(failMap ? echoTask : failingTask),
				"-jobconf",
				"keep.failed.task.files=true",
				"-jobconf",
				"stream.non.zero.exit.is.failure=" + exitStatusIsFailure,
				"-jobconf",
				"stream.tmpdir="
	...
	public void runStreamJob(boolean exitStatusIsFailure, boolean failMap)
		...
		StreamJob job = new StreamJob(genArgs(exitStatusIsFailure, failMap),
				mayExit);
		returnStatus = job.go();
