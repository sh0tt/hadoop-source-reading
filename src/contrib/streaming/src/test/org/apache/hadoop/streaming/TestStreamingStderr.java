...
public class TestStreamingStderr extends TestCase {
	...
	protected String[] genArgs(File input, File output, int preLines,
			int duringLines, int postLines) {
		return new String[] {
				"-input",
				...
				"-mapper",
				StreamUtil.makeJavaCommand(StderrApp.class, new String[] {
						Integer.toString(preLines),
						Integer.toString(duringLines),
						Integer.toString(postLines) }),
				"-reducer",
				StreamJob.REDUCE_NONE,
				"-jobconf",
				"keep.failed.task.files=true",
				"-jobconf",
				"mapred.task.timeout=5000",
				"-jobconf",
				"stream.tmpdir="
	...
	public void runStreamJob(String baseName, boolean hasInput, int preLines,
			int duringLines, int postLines) throws Exception {
