...
public class TestStreamingBadRecords extends ClusterMapReduceTestCase {
	...
	private static final Log LOG = LogFactory
			.getLog(TestStreamingBadRecords.class);

	private static final List<String> MAPPER_BAD_RECORDS = Arrays.asList(
			"hey022", "hey023", "hey099");

	private static final List<String> REDUCER_BAD_RECORDS = Arrays.asList(
			"hey001", "hey018");

	private static final String badMapper = StreamUtil.makeJavaCommand(
			BadApp.class, new String[] {});
	private static final String badReducer = StreamUtil.makeJavaCommand(
			BadApp.class, new String[] { "true" });
	...
	private void createInput() throws Exception {
		OutputStream os = getFileSystem().create(
				new Path(getInputDir(), "text.txt"));
	...
	private void validateOutput(RunningJob runningJob, boolean validateCount)
		...
		if (validateCount) {
			...
			assertEquals(counters
					.findCounter(counterGrp, "MAP_SKIPPED_RECORDS")
			...
			assertEquals(counters.findCounter(counterGrp,
					"REDUCE_SKIPPED_RECORDS").getCounter(), REDUCER_BAD_RECORDS
					.size());
			assertEquals(counters.findCounter(counterGrp,
					"REDUCE_SKIPPED_GROUPS").getCounter(), REDUCER_BAD_RECORDS
					.size());
			assertEquals(counters
					.findCounter(counterGrp, "REDUCE_INPUT_GROUPS")
					.getCounter(), redRecs);
			assertEquals(counters.findCounter(counterGrp,
					"REDUCE_INPUT_RECORDS").getCounter(), redRecs);
			assertEquals(counters.findCounter(counterGrp,
					"REDUCE_OUTPUT_RECORDS").getCounter(), redRecs);
		}
		...
		Path[] outputFiles = FileUtil.stat2Paths(getFileSystem().listStatus(
				getOutputDir(), new OutputLogFilter()));
	...
	public void testSkip() throws Exception {
		...
		String[] args = new String[] {
				...
				"-jobconf",
				"mapred.skip.attempts.to.start.skipping=" + attSkip,
				"-jobconf",
				"mapred.skip.out.dir=none",
				"-jobconf",
				"mapred.map.max.attempts=" + mapperAttempts,
				"-jobconf",
				"mapred.reduce.max.attempts=" + reducerAttempts,
				"-jobconf",
				"mapred.skip.map.max.skip.records=" + Long.MAX_VALUE,
				"-jobconf",
				"mapred.skip.reduce.max.skip.groups=" + Long.MAX_VALUE,
				"-jobconf",
				"mapred.map.tasks=1",
				"-jobconf",
				"mapred.reduce.tasks=1",
				"-jobconf",
				"fs.default.name=" + clusterConf.get("fs.default.name"),
				"-jobconf",
				"mapred.job.tracker=" + clusterConf.get("mapred.job.tracker"),
				"-jobconf",
	...
	public void testNarrowDown() throws Exception {
		...
		String[] args = new String[] {
				...
				"-jobconf",
				"mapred.reduce.max.attempts=15",
				"-jobconf",
				"mapred.skip.map.max.skip.records=1",
				"-jobconf",
				"mapred.skip.reduce.max.skip.groups=1",
				"-jobconf",
				"mapred.map.tasks=1",
				"-jobconf",
				"mapred.reduce.tasks=1",
	...
	static class App {
		...
		public App(String[] args) throws Exception {
			...
			while ((line = in.readLine()) != null) {
				...
				count++;
				if (count >= 10) {
					System.err.println("reporter:counter:"
							+ SkipBadRecords.COUNTER_GROUP + "," + counter
							+ "," + count);
					count = 0;
				}
			}
	...
	static class BadApp extends App {
		...
		protected void processLine(String line) throws Exception {
			...
			if (badRecords.size() > 0 && line.contains(badRecords.get(0))) {
				...
				System.exit(-1);
			} else if (badRecords.size() > 1
					&& line.contains(badRecords.get(1))) {
				LOG.warn("Encountered BAD record");
				throw new Exception("Got bad record..crashing");
			} else if (badRecords.size() > 2
					&& line.contains(badRecords.get(2))) {
				LOG.warn("Encountered BAD record");
				System.exit(-1);
			}
