...
public abstract class DataJoinReducerBase extends JobBase {
	...
	public void configure(JobConf job) {
		super.configure(job);
		this.job = job;
		this.maxNumOfValuesPerGroup = job.getLong(
	...
	protected void collect(Object key, TaggedMapOutput aRecord,
		...
		if (aRecord != null) {
			...
			reporter.setStatus("key: " + key.toString() + " collected: "
					+ collected);
	 ...
	 */
	private void joinAndCollect(Object[] tags, ResetableIterator[] values,
			int pos, Object[] partialList, Object key, OutputCollector output,
			Reporter reporter) throws IOException {
		...
		while (nextValues.hasNext()) {
			...
			partialList[pos] = v;
			joinAndCollect(tags, values, pos + 1, partialList, key, output,
					reporter);
		}
	}
