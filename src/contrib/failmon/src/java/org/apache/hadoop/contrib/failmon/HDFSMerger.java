...
public class HDFSMerger {
	...
	public HDFSMerger() throws IOException {
		...
		try {
			if (!hdfs.getFileStatus(hdfsDirPath).isDir()) {
				Environment.logInfo("HDFSMerger: Not an HDFS directory: "
						+ hdfsDirPath.toString());
				System.exit(0);
			}
		} catch (FileNotFoundException e) {
			Environment.logInfo("HDFSMerger: Directory not found: "
					+ hdfsDirPath.toString());
		}
	...
	private void appendFile(Path inputPath) throws IOException {
		...
		try {
			...
			Environment.logInfo("Error while copying file:"
					+ inputPath.toString());
		} finally {
			inputFile.close();
		}
