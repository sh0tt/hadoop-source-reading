...
public class PersistentState {
	...
	public static ParseState getState(String fname) {
		String[] fields = persData.getProperty(fname, "null" + SEPARATOR + "0")
				.split(SEPARATOR, 2);
		String firstLine;
		long offset;
	...
	public static void setState(ParseState state) {
		...
		persData.setProperty(state.filename, state.firstLine + SEPARATOR
				+ state.offset);
	}
	 ...
	 */
	public static void updateState(String filename, String firstLine,
			long offset) {
	...
	public static void writeState(String fname) {
		try {
			persData.store(new FileOutputStream(fname), Calendar.getInstance()
					.getTime().toString());
		} catch (FileNotFoundException e1) {
			e1.printStackTrace();
		} catch (IOException e) {
