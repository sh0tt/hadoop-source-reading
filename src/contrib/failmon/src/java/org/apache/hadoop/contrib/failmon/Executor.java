...
public class Executor implements Runnable {
	...
	public Executor(Configuration conf) {
		...
		if (conf == null) {
			...
			String hadoopLogPath = System.getProperty("hadoop.log.dir") + "/"
		...
		if (Environment.getProperty("local.upload.interval") != null)
			upload_interval = Integer.parseInt(Environment
					.getProperty("local.upload.interval"));
			...
			try {
				...
				for (int i = 0; i < monitors.size(); i++) {
					...
					if (monitors.get(i).counter <= 0) {
						...
						Environment.logInfo("Calling "
								+ monitors.get(i).job.getInfo() + "...\t");
