...
public class LocalStore {
	...
	public LocalStore() {
		...
		if (Environment.getProperty("local.tmp.dir") == null)
			Environment.setProperty("local.tmp.dir", System
	...
	public void upload() {
		try {
			...
			if ("true".equalsIgnoreCase(Environment
				...
				remoteName += Anonymizer.getMD5Hash(InetAddress.getLocalHost()
			...
			if (compress)
				copyToHDFS(filename + COMPRESSION_SUFFIX, hdfsDir + "/"
						+ remoteName + COMPRESSION_SUFFIX);
			else
	 ...
	 */
	public static void copyToHDFS(String localFile, String hdfsFile)
			throws IOException {
		...
		hadoopConf
				.addResource(new Path(hadoopConfPath + "/hadoop-default.xml"));
