...
public class NICParser extends ShellParser {
	...
	public EventRecord query(String device) throws UnknownHostException {
		...
		EventRecord retval = new EventRecord(InetAddress.getLocalHost()
				...
				.getLocalHost().getHostName()), Calendar.getInstance(), "NIC",
				"Unknown", device, "-");
		...
		String tmp = findPattern("inet\\s+addr:\\s*([\\w.?]*)", sb.toString(),
				1);
		...
		retval.set("rxOverruns", findPattern("RX.+overruns\\s*:\\s*(\\d+)", sb
				.toString(), 1));
		retval.set("rxFrame", findPattern("RX.+frame\\s*:\\s*(\\d+)", sb
				.toString(), 1));
		...
		retval.set("collisions", findPattern("\\s+collisions\\s*:\\s*(\\d+)",
				sb.toString(), 1));
