...
public class TestHdfsProxy extends TestCase {
	...
	private static MyFile createFile(Path root, FileSystem fs)
			throws IOException {
	...
	public void testHdfsProxyInterface() throws Exception {
		...
		try {
			...
			proxyConf.set("hdfsproxy.dfs.namenode.address", hdfs.getUri()
					.getHost()
					+ ":" + hdfs.getUri().getPort());
			...
			if (namenode.startsWith("hdfs://")) {
				MyFile[] files = createFiles(LOCAL_FS, TEST_ROOT_DIR
						+ "/srcdat");
				ToolRunner.run(distcp, new String[] { "-log",
						namenode + "/logs",
				...
				proxy.start();
				InetSocketAddress proxyAddr = NetUtils
						.createSocketAddr("localhost:0");
