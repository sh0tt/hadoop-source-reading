...
public class ProxyHttpServer extends HttpServer {
	...
	protected Connector createBaseListener(Configuration conf)
		...
		if (null == (sAddr = conf.get("proxy.http.test.listener.addr"))) {
			...
			sslListener.setPassword(conf
					.get("ssl.server.keystore.password", ""));
			sslListener.setKeyPassword(conf.get(
					"ssl.server.keystore.keypassword", ""));
			sslListener.setKeystoreType(conf.get("ssl.server.keystore.type",
					"jks"));
			sslListener.setNeedClientAuth(true);
			System.setProperty("javax.net.ssl.trustStore", conf.get(
