...
public class HftpFileSystem extends FileSystem {
	...
	protected String pickOneAddress(String hostname)
	...
	protected HttpURLConnection openConnection(String path, String query)
			throws IOException {
		try {
			final URL url = new URI("http", null, pickOneAddress(nnAddr
					.getHostName()), nnAddr.getPort(), path, query, null)
					.toURL();
	...
	class LsParser extends DefaultHandler {
		...
		public void startElement(String ns, String localname, String qname,
			...
			FileStatus fs = "file".equals(qname) ? new FileStatus(Long.valueOf(
					attrs.getValue("size")).longValue(), false, Short.valueOf(
					attrs.getValue("replication")).shortValue(), Long.valueOf(
					attrs.getValue("blocksize")).longValue(), modif, atime,
					FsPermission.valueOf(attrs.getValue("permission")), attrs
							.getValue("owner"), attrs.getValue("group"),
					...
					: new FileStatus(0L, true, 0, 0L, modif, atime,
							...
							new Path(getUri().toString(), attrs
									.getValue("path"))
									.makeQualified(HftpFileSystem.this));
			fslist.add(fs);
		}
		...
		private void fetchList(String path, boolean recur) throws IOException {
			try {
				...
				HttpURLConnection connection = openConnection("/listPaths"
						+ path, "ugi=" + ugi + (recur ? "&recursive=yes" : ""));
				connection.setRequestMethod("GET");
				connection.connect();
		...
		public FileStatus[] listStatus(Path f, boolean recur)
				throws IOException {
			fetchList(f.toUri().getPath(), recur);
			if (fslist.size() > 0
					&& (fslist.size() != 1 || fslist.get(0).isDir())) {
				fslist.remove(0);
			}
	...
	private class ChecksumParser extends DefaultHandler {
		...
		private FileChecksum getFileChecksum(String f) throws IOException {
			final HttpURLConnection connection = openConnection("/fileChecksum"
					+ f, "ugi=" + ugi);
			try {
