...
public class UpgradeObjectCollection {
	...
	static class UOSignature implements Comparable<UOSignature> {
		...
		Upgradeable instantiate() throws IOException {
			try {
				return (Upgradeable) Class.forName(getClassName())
						.newInstance();
		...
		public int compareTo(UOSignature o) {
			...
			int res = this.getType().toString().compareTo(
					o.getType().toString());
			if (res != 0)
				return res;
	...
	public static SortedSet<Upgradeable> getDistributedUpgrades(
			int versionFrom, HdfsConstants.NodeType type) throws IOException {
		assert FSConstants.LAYOUT_VERSION <= versionFrom : "Incorrect version "
				+ versionFrom + ". Expected to be <= "
