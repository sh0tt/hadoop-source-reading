...
public class FileDataServlet extends DfsServlet {
	...
	protected URI createUri(FileStatus i, UnixUserGroupInformation ugi,
		...
		return new URI(scheme, null, hostname,
				"https".equals(scheme) ? (Integer) getServletContext()
						.getAttribute("datanode.https.port") : host
						.getInfoPort(), "/streamFile", "filename="
						+ i.getPath() + "&ugi=" + ugi, null);
	}
	...
	private static DatanodeID pickSrcDatanode(FileStatus i,
		...
		// A proper fix should make JspHelper a singleton. Since it doesn't
		// affect
		// correctness, we leave it as is for now.
		if (jspHelper == null)
			jspHelper = new JspHelper();
		final LocatedBlocks blks = nnproxy.getBlockLocations(i.getPath()
				.toUri().getPath(), 0, 1);
	 ...
	 * http://<nn>:<port>/data[/<path>] HTTP/1.1 * }
	 */
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if ((info != null) && !info.isDir()) {
				response.sendRedirect(createUri(info, ugi, nnproxy, request)
						.toURL().toString());
			} else if (info == null) {
