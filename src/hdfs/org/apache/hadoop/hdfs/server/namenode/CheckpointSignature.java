 ...
 */
public class CheckpointSignature extends StorageInfo implements
		WritableComparable<CheckpointSignature> {
	...
	void validateStorageInfo(StorageInfo si) throws IOException {
		if (layoutVersion != si.layoutVersion || namespaceID != si.namespaceID
				|| cTime != si.cTime) {
			// checkpointTime can change when the image is saved - do not
			// compare
			throw new IOException("Inconsistent checkpoint fileds. " + "LV = "
					+ layoutVersion + " namespaceID = " + namespaceID
					+ " cTime = " + cTime + ". Expecting respectively: "
					+ si.layoutVersion + "; " + si.namespaceID + "; "
					+ si.cTime);
		}
	}
