...
public class JspHelper {
	...
	public void streamBlockInAscii(InetSocketAddress addr, long blockId,
			...
			long chunkSizeToView, JspWriter out) throws IOException {
		...
		DFSClient.BlockReader blockReader = DFSClient.BlockReader
				.newBlockReader(s, addr.toString() + ":" + blockId, blockId,
						genStamp, offsetIntoBlock, amtToRead, conf.getInt(
	...
	public void addTableRow(JspWriter out, String[] columns, int row)
		...
		for (int i = 0; i < columns.length; i++) {
			if (row / 2 * 2 == row) {// even
				out
						.print("<td style=\"vertical-align: top;background-color:LightGrey;\"><B>"
								+ columns[i] + "</B><br></td>");
			} else {
				out
						.print("<td style=\"vertical-align: top;background-color:LightBlue;\"><B>"
								+ columns[i] + "</B><br></td>");
	...
	public static void printGotoForm(JspWriter out, int namenodeInfoPort,
		...
		out
				.print("<form action=\"browseDirectory.jsp\" method=\"get\" name=\"goto\">");
		out.print("Goto : ");
		out
				.print("<input name=\"dir\" type=\"text\" width=\"50\" id\"dir\" value=\""
						+ file + "\">");
