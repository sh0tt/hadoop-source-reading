...
public class NameNode implements ClientProtocol, DatanodeProtocol,
	...
	private void initialize(Configuration conf) throws IOException {
		...
		// create rpc server
		this.server = RPC.getServer(this, socAddr.getHostName(), socAddr
	...
	private void startHttpServer(Configuration conf) throws IOException {
		...
		if (conf.getBoolean("dfs.https.enable", false)) {
			...
			InetSocketAddress secInfoSocAddr = NetUtils.createSocketAddr(conf
					.get("dfs.https.address", infoHost + ":" + 0));
			Configuration sslConf = new Configuration(false);
			sslConf.addResource(conf.get("dfs.https.server.keystore.resource",
					"ssl-server.xml"));
			this.httpServer.addSslListener(secInfoSocAddr, sslConf,
					needClientAuth);
		...
		this.httpServer.addInternalServlet("getimage", "/getimage",
	...
	public void create(String src, FsPermission masked, String clientName,
		...
		namesystem.startFile(src, new PermissionStatus(UserGroupInformation
				.getCurrentUGI().getUserName(), null, masked), clientName,
   ...
   */
	...
	public boolean mkdirs(String src, FsPermission masked) throws IOException {
		...
		return namesystem.mkdirs(src, new PermissionStatus(UserGroupInformation
				.getCurrentUGI().getUserName(), null, masked));
	}
