...
public class NamenodeFsck {
	 ...
	 * @throws IOException
	 */
	public NamenodeFsck(Configuration conf, NameNode nn,
			Map<String, String[]> pmap, HttpServletResponse response)
			throws IOException {
		this.conf = conf;
		this.nn = nn;
	...
	private void check(FileStatus file, FsckResult res) throws IOException {
		...
		LocatedBlocks blocks = nn.namesystem
				.getBlockLocations(path, 0, fileLen);
		...
		for (LocatedBlock lBlk : blocks.getLocatedBlocks()) {
			...
			if (locs.length < targetFileReplication && locs.length > 0) {
				...
				out.println(" Under replicated " + block
						+ ". Target Replicas is " + targetFileReplication
	...
	private void lostFoundMove(FileStatus file, LocatedBlocks blocks)
		...
		try {
			...
			String errmsg = "Failed to move " + file.getPath()
					+ " to /lost+found";
			try {
				PermissionStatus ps = new PermissionStatus(file.getOwner(),
						file.getGroup(), file.getPermission());
				if (!nn.namesystem.dir.mkdirs(target, ps, false, FSNamesystem
						.now())) {
					LOG.warn(errmsg);
					return;
				}
				...
				for (LocatedBlock lBlk : blocks.getLocatedBlocks()) {
					...
					if (fos == null) {
						...
						if (fos != null)
							...
							LOG.warn(errmsg + ": could not store chain "
									+ chain);
					...
					try {
						...
						LOG.warn(" - could not copy block " + lblock.getBlock()
								+ " to " + target);
						fos.flush();
						fos.close();
						fos = null;
				...
				if (fos != null)
					fos.close();
				LOG.warn("\n - moved corrupted file " + file.getPath()
						+ " to /lost+found");
	...
	private void copyBlock(DFSClient dfs, LocatedBlock lblock, OutputStream fos)
			...
			try {
				...
				blockReader = DFSClient.BlockReader.newBlockReader(s,
						targetAddr.toString() + ":" + block.getBlockId(), block
								.getBlockId(), block.getGenerationStamp(), 0,
						-1, conf.getInt("io.file.buffer.size", 4096));
		...
		if (blockReader == null) {
			throw new Exception("Could not open data stream for "
					+ lblock.getBlock());
		}
		...
		try {
			...
			if (bytesRead != block.getNumBytes()) {
				throw new IOException("Recorded block size is "
						+ block.getNumBytes() + ", but datanode returned "
						+ bytesRead + " bytes");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static class FsckResult {
		...
		public String toString() {
			...
			if (totalOpenFilesSize != 0)
				res.append(" (Total open files size: " + totalOpenFilesSize
						+ " B)");
			...
			if (totalBlocks > 0)
				res.append(" (avg. block size " + (totalSize / totalBlocks)
						+ " B)");
			...
			if (totalReplicas > 0)
				res
						.append(" ("
								+ ((float) (missingReplicas * 100) / (float) totalReplicas)
								+ " %)");
