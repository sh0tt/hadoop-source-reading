 ...
 * publishing them through the metrics interfaces. The SNamesystem creates and
...
public class FSNamesystemMetrics implements Updater {
	...
	final MetricsLongValue blocksTotal = new MetricsLongValue("BlocksTotal",
			registry);
	final MetricsIntValue capacityTotalGB = new MetricsIntValue(
			"CapacityTotalGB", registry);
	final MetricsIntValue capacityUsedGB = new MetricsIntValue(
			"CapacityUsedGB", registry);
	final MetricsIntValue capacityRemainingGB = new MetricsIntValue(
			"CapacityRemainingGB", registry);
	final MetricsIntValue totalLoad = new MetricsIntValue("TotalLoad", registry);
	final MetricsIntValue pendingDeletionBlocks = new MetricsIntValue(
			"PendingDeletionBlocks", registry);
	...
	final MetricsIntValue excessBlocks = new MetricsIntValue("ExcessBlocks",
			registry);
	final MetricsIntValue pendingReplicationBlocks = new MetricsIntValue(
			"PendingReplicationBlocks", registry);
	final MetricsIntValue underReplicatedBlocks = new MetricsIntValue(
			"UnderReplicatedBlocks", registry);
	final MetricsIntValue scheduledReplicationBlocks = new MetricsIntValue(
			"ScheduledReplicationBlocks", registry);
	...
	public FSNamesystemMetrics(Configuration conf) {
		...
		metricsRecord = MetricsUtil
				.createRecord(metricsContext, "FSNamesystem");
	...
	public void doUpdates(MetricsContext unused) {
		/**
		 * ToFix If the metrics counter were instead stored in the metrics
		 * objects themselves we could avoid copying the values on each update.
		 */
		synchronized (this) {
			FSNamesystem fsNameSystem = FSNamesystem.getFSNamesystem();
			filesTotal.set((int) fsNameSystem.getFilesTotal());
			blocksTotal.set((int) fsNameSystem.getBlocksTotal());
			capacityTotalGB.set(roundBytesToGBytes(fsNameSystem
					.getCapacityTotal()));
			capacityUsedGB.set(roundBytesToGBytes(fsNameSystem
					.getCapacityUsed()));
			...
			pendingDeletionBlocks.set((int) fsNameSystem
					.getPendingDeletionBlocks());
			pendingReplicationBlocks.set((int) fsNameSystem
					.getPendingReplicationBlocks());
			underReplicatedBlocks.set((int) fsNameSystem
