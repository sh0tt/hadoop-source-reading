...
public class FSEditLog {
		...
		private void preallocate() throws IOException {
			...
			if (position + 4096 >= fc.size()) {
				...
				FSNamesystem.LOG.debug("Edit log size is now " + fc.size()
						+ " written " + written + " bytes " + " at offset "
						+ newsize);
			}
		}
	...
	private int getNumStorageDirs() {
		...
		for (Iterator<StorageDirectory> it = fsimage
				.dirIterator(NameNodeDirType.EDITS); it.hasNext(); it.next())
			numStorageDirs++;
	...
	synchronized void processIOError(int index) {
		...
		File parentStorageDir = ((EditLogFileOutputStream) editStreams
				.get(index)).getFile().getParentFile().getParentFile();
	...
	synchronized void processIOError(StorageDirectory sd) {
		...
		for (int idx = 0; idx < editStreams.size(); idx++) {
			File parentStorageDir = ((EditLogFileOutputStream) editStreams
					.get(idx)).getFile().getParentFile().getParentFile();
	...
	static int loadFSEdits(EditLogInputStream edits) throws IOException {
			...
			while (true) {
				...
				try {
					...
					if (opcode == OP_INVALID) {
						FSNamesystem.LOG
								.info("Invalid opcode, reached end of edit log "
				...
				case OP_CLOSE: {
					...
					if (-7 == logVersion && length != 3 || -17 < logVersion
					...
					INodeFile node = (INodeFile) fsDir.unprotectedAddFile(path,
							permissions, blocks, replication, mtime, atime,
							blockSize);
					if (opcode == OP_ADD) {
						numOpAdd++;
						//
						...
						INodeFileUnderConstruction cons = new INodeFileUnderConstruction(
								...
								node.getReplication(), node
										.getModificationTime(), node
										.getPreferredBlockSize(), node
										.getBlocks(), node
										.getPermissionStatus(), clientName,
								clientMachine, null);
				...
				case OP_MKDIR: {
					...
					PermissionStatus permissions = fsNamesys
							.getUpgradePermission();
					int length = in.readInt();
					if (-17 < logVersion && length != 2 || logVersion <= -17
							&& length != 3) {
						throw new IOException("Incorrect data format. "
								+ "Mkdir operation.");
					}
				...
				case OP_SET_OWNER: {
					...
					if (logVersion > -11)
						throw new IOException("Unexpected opcode " + opcode
								+ " for version " + logVersion);
					fsDir.unprotectedSetOwner(FSImage.readString(in), FSImage
							.readString_EmptyAsNull(in), FSImage
				...
				case OP_SET_NS_QUOTA: {
					...
					fsDir.unprotectedSetQuota(FSImage.readString(in),
							readLongWritable(in), FSConstants.QUOTA_DONT_SET);
					break;
				}
				case OP_CLEAR_NS_QUOTA: {
					...
					fsDir
							.unprotectedSetQuota(FSImage.readString(in),
				...
				case OP_SET_QUOTA:
					fsDir.unprotectedSetQuota(FSImage.readString(in),
							readLongWritable(in), readLongWritable(in));
		...
		if (FSImage.LOG.isDebugEnabled()) {
			FSImage.LOG.debug("numOpAdd = " + numOpAdd + " numOpClose = "
					...
					+ " numOpTimes = " + numOpTimes + " numOpOther = "
					+ numOpOther);
		}
	...
	public void logSync() throws IOException {
		...
		synchronized (this) {
			...
			if (mytxid <= synctxid) {
				...
				if (metrics != null) // Metrics is non-null only when used
					// inside name node
	...
	public void logOpenFile(String path, INodeFileUnderConstruction newNode)
		...
		logEdit(OP_ADD, new ArrayWritable(UTF8.class, nameReplicationPair),
				new ArrayWritable(Block.class, newNode.getBlocks()), newNode
						.getPermissionStatus(), new UTF8(newNode
						.getClientName()), new UTF8(newNode.getClientMachine()));
	}
	...
	void logSetQuota(String src, long nsQuota, long dsQuota) {
		logEdit(OP_SET_QUOTA, new UTF8(src), new LongWritable(nsQuota),
				new LongWritable(dsQuota));
	}
	...
	void logTimes(String src, long mtime, long atime) {
		UTF8 info[] = new UTF8[] { new UTF8(src), FSEditLog.toLogLong(mtime),
	...
	synchronized void rollEditLog() throws IOException {
		...
		for (Iterator<StorageDirectory> it = fsimage
			...
			StorageDirectory sd = it.next();
			try {
				EditLogFileOutputStream eStream = new EditLogFileOutputStream(
						getEditNewFile(sd));
				eStream.create();
				editStreams.add(eStream);
			} catch (IOException e) {
	...
	synchronized long getFsEditTime() {
		Iterator<StorageDirectory> it = fsimage
