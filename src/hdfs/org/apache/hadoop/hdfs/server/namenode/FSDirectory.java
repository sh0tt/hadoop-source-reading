...
class FSDirectory implements FSConstants, Closeable {
   ...
   */
	...
	Block addBlock(String path, INode[] inodes, Block block) throws IOException {
		...
		synchronized (rootDir) {
			...
			updateCount(inodes, inodes.length - 1, 0, fileNode
	...
	private void replaceNode(String path, INodeFile oldnode, INodeFile newnode,
		...
		synchronized (rootDir) {
			...
			long dsNew = 0;
			if (updateDiskspace
					&& (dsNew = newnode.diskspaceConsumed()) != dsOld) {
				try {
   ...
   */
	...
	private static FileStatus createFileStatus(String path, INode node) {
		...
		return new FileStatus(node.isDirectory() ? 0 : node
				.computeContentSummary().getLength(), node.isDirectory(), node
				.isDirectory() ? 0 : ((INodeFile) node).getReplication(),
				node.isDirectory() ? 0 : ((INodeFile) node)
						.getPreferredBlockSize(), node.getModificationTime(),
				node.getAccessTime(), node.getFsPermission(), node
						.getUserName(), node.getGroupName(), new Path(path));
	}
}
