...
public class FSNamesystem implements FSConstants, FSNamesystemMBean {
	...
	synchronized void metaSave(String filename) throws IOException {
		...
		synchronized (neededReplications) {
			...
			for (Block block : neededReplications) {
				...
				out.print(block + " (replicas:" + " l: "
						+ numReplicas.liveReplicas() + " d: "
	...
	public LocatedBlocks getBlockLocations(String src, long offset,
			long length, boolean doAccessTime) throws IOException {
		if (offset < 0) {
		...
		final LocatedBlocks ret = getBlockLocationsInternal(src, offset,
				length, Integer.MAX_VALUE, doAccessTime);
		if (auditLog.isInfoEnabled()) {
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
	...
	private synchronized LocatedBlocks getBlockLocationsInternal(String src,
			long offset, long length, int nrBlocksToReturn, boolean doAccessTime)
			throws IOException {
	...
	public synchronized void setTimes(String src, long mtime, long atime)
		...
		if (inode != null) {
			...
			if (auditLog.isInfoEnabled()) {
				final FileStatus stat = dir.getFileInfo(src);
				logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
						.getRemoteIp(), "setTimes", src, null, stat);
			}
		} else {
	...
	private synchronized void startFileInternal(String src,
			...
			boolean overwrite, boolean append, short replication, long blockSize)
			throws IOException {
		if (NameNode.stateChangeLog.isDebugEnabled()) {
			NameNode.stateChangeLog.debug("DIR* NameSystem.startFile: src="
					+ src + ", holder=" + holder + ", clientMachine="
					+ clientMachine + ", replication=" + replication
		...
		try {
			...
			if (append) {
				...
				INodeFileUnderConstruction cons = new INodeFileUnderConstruction(
						node.getLocalNameBytes(), node.getReplication(), node
								.getModificationTime(), node
								.getPreferredBlockSize(), node.getBlocks(),
	...
	LocatedBlock appendFile(String src, String holder, String clientMachine)
		...
		if (auditLog.isInfoEnabled()) {
			logAuditEvent(UserGroupInformation.getCurrentUGI(), Server
					.getRemoteIp(), "append", src, null, null);
		}
		return lb;
	}
	...
	synchronized void commitBlockSynchronization(Block lastblock,
		...
		if (deleteblock) {
			...
			// blocks map
			lastblock
					.set(lastblock.getBlockId(), newlength, newgenerationstamp);
		...
		public void run() {
			while (fsRunning) {
				try {
					...
					LOG
							.warn("ReplicationMonitor thread received InterruptedException."
									+ ie);
					break;
				} catch (IOException ie) {
					...
					LOG
							.warn("ReplicationMonitor thread received Runtime exception. "
									+ t);
	...
	synchronized Block addStoredBlock(Block block, DatanodeDescriptor node,
		...
		if (block != storedBlock) {
			if (block.getNumBytes() >= 0) {
				...
				INodeFile file = (storedBlock != null) ? storedBlock.getINode()
						: null;
   ...
   */
	...
	public void refreshNodes(Configuration conf) throws IOException {
		...
		hostsReader.updateFileNames(conf.get("dfs.hosts", ""), conf.get(
				"dfs.hosts.exclude", ""));
		hostsReader.refresh();
		synchronized (this) {
			for (Iterator<DatanodeDescriptor> it = datanodeMap.values()
