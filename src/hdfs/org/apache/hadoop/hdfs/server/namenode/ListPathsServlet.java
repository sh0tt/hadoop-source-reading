...
public class ListPathsServlet extends DfsServlet {
	...
	static void writeInfo(FileStatus i, XMLOutputter doc) throws IOException {
		...
		doc
				.attribute("modified", ldf.format(new Date(i
						.getModificationTime())));
		...
		doc
				.attribute("permission", (i.isDir() ? "d" : "-")
						+ i.getPermission());
	...
	protected Map<String, String> buildRoot(HttpServletRequest request,
		...
		final String path = request.getPathInfo() != null ? request
				.getPathInfo() : "/";
		final String exclude = request.getParameter("exclude") != null ? request
				.getParameter("exclude")
				: "\\..*\\.crc";
		final String filter = request.getParameter("filter") != null ? request
				.getParameter("filter") : ".*";
	 ...
	 * {@code <listing path="..." recursive="(yes|no)" filter="..."
	 * time="yyyy-MM-dd hh:mm:ss UTC" version="..."> <directory path="..."
	 * modified="yyyy-MM-dd hh:mm:ss"/> <file path="..."
	 * modified="yyyy-MM-dd'T'hh:mm:ssZ" accesstime="yyyy-MM-dd'T'hh:mm:ssZ"
	 * blocksize="..." replication="..." size="..."/> </listing> * }
	 */
				...
				try {
					for (FileStatus i : nnproxy.getListing(p)) {
						if (exclude.matcher(i.getPath().getName()).matches()
								|| !filter.matcher(i.getPath().getName())
										.matches()) {
							continue;
						}
						if (recur && i.isDir()) {
