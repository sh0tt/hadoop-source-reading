...
public class FileChecksumServlets {
	...
	public static class RedirectServlet extends DfsServlet {
		...
		public void doGet(HttpServletRequest request,
			...
			final NameNode namenode = (NameNode) context
					.getAttribute("name.node");
			final DatanodeID datanode = namenode.namesystem.getRandomDatanode();
			try {
				final URI uri = createRedirectUri("/getFileChecksum", ugi,
						datanode, request);
	...
	public static class GetServlet extends DfsServlet {
		...
		public void doGet(HttpServletRequest request,
			...
			final Configuration conf = new Configuration(DataNode.getDataNode()
					.getConf());
			...
			try {
				final MD5MD5CRC32FileChecksum checksum = DFSClient
						.getFileChecksum(filename, nnproxy, socketFactory,
								socketTimeout);
