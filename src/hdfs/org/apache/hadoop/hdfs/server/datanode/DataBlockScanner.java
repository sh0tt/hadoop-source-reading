...
class DataBlockScanner implements Runnable {
	...
	static class BlockScanInfo implements Comparable<BlockScanInfo> {
		...
		public int compareTo(BlockScanInfo other) {
			...
			return (t1 < t2) ? -1 : ((t1 > t2) ? 1 : block
					.compareTo(other.block));
		}
	}
	...
	private synchronized void updateScanStatus(Block block, ScanType type,
		...
		if (!scanOk
				|| (type == ScanType.REMOTE_READ && diff < scanPeriod / 3 && diff < ONE_DAY)) {
			return;
		}
	...
	private synchronized void adjustThrottler() {
		long timeLeft = currentPeriodStart + scanPeriod
	...
	private synchronized void startNewPeriod() {
		LOG.info("Starting a new period : work left in prev period : "
				+ String.format("%.2f%%", (bytesLeft * 100.0)
						/ totalBytesToScan));
	...
	synchronized void printBlockReport(StringBuilder buffer, boolean summaryOnly) {
		...
		for (Iterator<BlockScanInfo> it = blockInfoSet.iterator(); it.hasNext();) {
			...
			if (!summaryOnly) {
				...
				buffer.append(String.format("%-26s : status : %-6s type : %-6s"
						+ " scan time : " + "%-15d %s\n", info.block,
						(info.lastScanOk ? "ok" : "failed"), scanType,
						scanTime, (scanTime <= 0) ? "not yet verified"
								: dateFormat.format(date)));
			}
		}

		double pctPeriodLeft = (scanPeriod + currentPeriodStart - now) * 100.0
				/ scanPeriod;
		...
		buffer.append(String.format("\nTotal Blocks                 : %6d"
				...
				inOneHour, inOneDay, inOneWeek, inFourWeeks, inScanPeriod,
				neverScanned, totalVerifications, totalScans, totalScanErrors,
				totalTransientErrors, Math
						.round(throttler.getBandwidth() / 1024.0), pctProgress,
				pctPeriodLeft));
	}
	...
	private static class LogFileHandler {
		...
		private void rollIfRequired() throws IOException {
			...
			if (!curFile.renameTo(prevFile)) {
				openCurFile();
				throw new IOException("Could not rename " + curFile + " to "
						+ prevFile);
			}
	...
	public static class Servlet extends HttpServlet {
		...
		public void doGet(HttpServletRequest request,
			...
			if (blockScanner == null) {
				buffer
						.append("Periodic block scanner is not running. "
