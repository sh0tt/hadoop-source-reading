...
public class DFSClient implements FSConstants, java.io.Closeable {
	...
	public BlockLocation[] getBlockLocations(String src, long start, long length)
		...
		for (LocatedBlock blk : blocks.getLocatedBlocks()) {
			...
			for (int hCnt = 0; hCnt < locations.length; hCnt++) {
				...
				NodeBase node = new NodeBase(names[hCnt], locations[hCnt]
						.getNetworkLocation());
				racks[hCnt] = node.toString();
			}
			blkLocations[idx] = new BlockLocation(names, hosts, racks, blk
	...
	public OutputStream create(String src, FsPermission permission,
		...
		OutputStream result = new DFSOutputStream(src, masked, overwrite,
				replication, blockSize, progress, buffersize, conf.getInt(
   ...
   */
	...
	public static MD5MD5CRC32FileChecksum getFileChecksum(String src,
		...
		final List<LocatedBlock> locatedblocks = callGetBlockLocations(
				namenode, src, 0, Long.MAX_VALUE).getLocatedBlocks();
		...
		for (int i = 0; i < locatedblocks.size(); i++) {
			...
			for (int j = 0; !done && j < datanodes.length; j++) {
				...
				NetUtils.connect(sock, NetUtils.createSocketAddr(datanodes[j]
						.getName()), timeout);
				...
				DataInputStream in = new DataInputStream(NetUtils
						.getInputStream(sock));
			...
			while (clientRunning && !Thread.interrupted()) {
				if (System.currentTimeMillis() - lastRenewed > (LEASE_SOFTLIMIT_PERIOD / 2)) {
					try {
						...
						LOG
								.warn("Problem renewing lease for "
										+ clientName, ie);
					}
				}
		...
		DFSOutputStream(String src, int buffersize, Progressable progress,
			...
			if (lastBlock != null) {
				...
				if (usedInCksum > 0 && freeInCksum > 0) {
					...
					// smaller size packet.
					//
					computePacketChunkSize(Math.min(writePacketSize,
				...
				if (nodes.length < 1) {
					throw new IOException(
							"Unable to retrieve blocks locations "
									+ " for last block " + block + "of file "
									+ src);
								...
								if (System.currentTimeMillis() - localstart > 5000) {
									LOG
											.info("Waiting for replication for "
													+ (System
															.currentTimeMillis() - localstart)
													/ 1000 + " seconds");
								}
								try {
									LOG
											.warn("NotReplicatedYetException sleeping "
													+ src
													+ " retries left "
													+ retries);
