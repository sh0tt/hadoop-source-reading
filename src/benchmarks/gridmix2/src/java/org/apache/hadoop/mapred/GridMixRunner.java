...
public class GridMixRunner {
		...
		STREAMSORT("streamSort") {
			public void addJob(int numReducers, boolean mapoutputCompressed,
				...
				final String prop = String.format(
						"streamSort.%sJobs.inputFiles", size);
				final String indir = getInputDirsFor(prop, size
						.defaultPath(VARINFLTEXT));
				final String outdir = addTSSuffix("perf-out/stream-out-dir-"
						+ size);
				...
				try {
					...
					jobconf.setBoolean("mapred.output.compress",
							outputCompressed);
		...
		JAVASORT("javaSort") {
			public void addJob(int numReducers, boolean mapoutputCompressed,
				...
				try {
					...
					jobConf
							.setOutputFormat(org.apache.hadoop.mapred.TextOutputFormat.class);

					jobConf.setOutputKeyClass(org.apache.hadoop.io.Text.class);
					jobConf
							.setOutputValueClass(org.apache.hadoop.io.Text.class);
					jobConf.setCompressMapOutput(mapoutputCompressed);
					jobConf.setBoolean("mapred.output.compress",
							outputCompressed);
	...
	public void addjobs() throws IOException {
		...
		System.out
				.println("total " + gridmix.getWaitingJobs().size() + " jobs");
	}
	...
	private TreeMap<String, String> getStatForJob(Job job) {
		...
		try {
			...
			TaskReport[] maps = jc
					.getMapTaskReports(JobID.forName(mapreduceID));
