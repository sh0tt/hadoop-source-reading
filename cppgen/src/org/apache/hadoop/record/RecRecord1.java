...
public class RecRecord1 extends org.apache.hadoop.record.Record {
	...
	static {
		_rio_recTypeInfo = new org.apache.hadoop.record.meta.RecordTypeInfo(
				"RecRecord1");
		_rio_recTypeInfo.addField("boolVal",
				org.apache.hadoop.record.meta.TypeID.BoolTypeID);
		_rio_recTypeInfo.addField("byteVal",
				org.apache.hadoop.record.meta.TypeID.ByteTypeID);
		_rio_recTypeInfo.addField("intVal",
				org.apache.hadoop.record.meta.TypeID.IntTypeID);
		_rio_recTypeInfo.addField("longVal",
				org.apache.hadoop.record.meta.TypeID.LongTypeID);
		_rio_recTypeInfo.addField("floatVal",
				org.apache.hadoop.record.meta.TypeID.FloatTypeID);
		_rio_recTypeInfo.addField("doubleVal",
				org.apache.hadoop.record.meta.TypeID.DoubleTypeID);
		_rio_recTypeInfo.addField("stringVal",
				org.apache.hadoop.record.meta.TypeID.StringTypeID);
		_rio_recTypeInfo.addField("bufferVal",
				org.apache.hadoop.record.meta.TypeID.BufferTypeID);
		_rio_recTypeInfo.addField("vectorVal",
				new org.apache.hadoop.record.meta.VectorTypeID(
						org.apache.hadoop.record.meta.TypeID.StringTypeID));
		_rio_recTypeInfo.addField("mapVal",
				new org.apache.hadoop.record.meta.MapTypeID(
						org.apache.hadoop.record.meta.TypeID.StringTypeID,
						org.apache.hadoop.record.meta.TypeID.StringTypeID));
		_rio_recTypeInfo.addField("recordVal",
	...
	public RecRecord1(final boolean boolVal, final byte byteVal,
			final int intVal, final long longVal, final float floatVal,
			final double doubleVal, final String stringVal,
	...
	public void serialize(final org.apache.hadoop.record.RecordOutput _rio_a,
		...
		}
		{
			...
			java.util.Set<java.util.Map.Entry<String, String>> _rio_es1 = mapVal
					.entrySet();
			for (java.util.Iterator<java.util.Map.Entry<String, String>> _rio_midx1 = _rio_es1
					.iterator(); _rio_midx1.hasNext();) {
				java.util.Map.Entry<String, String> _rio_me1 = _rio_midx1
						.next();
	...
	public int compareTo(final Object _rio_peer_) throws ClassCastException {
		...
		if (_rio_ret != 0)
			return _rio_ret;
		_rio_ret = (byteVal == _rio_peer.byteVal) ? 0
				: ((byteVal < _rio_peer.byteVal) ? -1 : 1);
		if (_rio_ret != 0)
			return _rio_ret;
		_rio_ret = (intVal == _rio_peer.intVal) ? 0
				: ((intVal < _rio_peer.intVal) ? -1 : 1);
		if (_rio_ret != 0)
			return _rio_ret;
		_rio_ret = (longVal == _rio_peer.longVal) ? 0
				: ((longVal < _rio_peer.longVal) ? -1 : 1);
		if (_rio_ret != 0)
			return _rio_ret;
		_rio_ret = (floatVal == _rio_peer.floatVal) ? 0
				: ((floatVal < _rio_peer.floatVal) ? -1 : 1);
		if (_rio_ret != 0)
			return _rio_ret;
		_rio_ret = (doubleVal == _rio_peer.doubleVal) ? 0
				: ((doubleVal < _rio_peer.doubleVal) ? -1 : 1);
		if (_rio_ret != 0)
			return _rio_ret;
	...
	public Object clone() throws CloneNotSupportedException {
		...
		_rio_other.bufferVal = (org.apache.hadoop.record.Buffer) this.bufferVal
				.clone();
		_rio_other.vectorVal = (java.util.ArrayList<String>) this.vectorVal
				.clone();
		_rio_other.mapVal = (java.util.TreeMap<String, String>) this.mapVal
				.clone();
		_rio_other.recordVal = (org.apache.hadoop.record.RecRecord0) this.recordVal
				.clone();
		return _rio_other;
	}
	...
	public int hashCode() {
		...
		_rio_result = 37 * _rio_result + _rio_ret;
		_rio_ret = (int) (Double.doubleToLongBits(doubleVal) ^ (Double
				.doubleToLongBits(doubleVal) >>> 32));
		_rio_result = 37 * _rio_result + _rio_ret;
		...
		static public int slurpRaw(byte[] b, int s, int l) {
			try {
				int os = s;
				{
					if (l < 1) {
						throw new java.io.IOException(
								"Boolean is exactly 1 byte. Provided buffer is smaller.");
					}
					s++;
					l--;
				}
				...
				}
				{
					if (l < 8) {
						throw new java.io.IOException(
								"Double is exactly 8 bytes. Provided buffer is smaller.");
					}
					s += 8;
					l -= 8;
				}
		...
		static public int compareRaw(byte[] b1, int s1, int l1, byte[] b2,
			...
			try {
				...
				}
				{
					...
					double d1 = org.apache.hadoop.record.Utils.readDouble(b1,
							s1);
					double d2 = org.apache.hadoop.record.Utils.readDouble(b2,
							s2);
					if (d1 != d2) {
						return ((d1 - d2) < 0) ? -1 : 0;
					}
				...
				}
				{
					...
					for (int vidx1 = 0; vidx1 < vi11 && vidx1 < vi21; vidx1++) {
						...
						l1 -= z1;
						l2 -= z2;
						int r1 = org.apache.hadoop.record.Utils.compareBytes(
								b1, s1, i1, b2, s2, i2);
						if (r1 != 0) {
							return (r1 < 0) ? -1 : 0;
						}
	...
	static {
		org.apache.hadoop.record.RecordComparator.define(RecRecord1.class,
				new Comparator());
	}
}
